...
function stubStatusUtils() {
	...
	sinon
		.stub(helpers, 'fetchDataFromDB')
		.yields(null, true);

	sinon
		.stub(helpers, 'mergeToTemplate')
		.yields(null, true);

	sinon
		.stub(helpers, 'populateTemplate');

	sinon
		.stub(helpers, 'checkMandatoryTemplateSchema')
		.yields(null, true);
	sinon
		.stub(helpers, 'checkDuplicate')
		.yields(contextFunction);
	sinon
		.stub(helpers, 'saveContent')
		.yields(null, true);

	sinon
		.stub(helpers, 'generateDeploymentTemplate')
		.yields(null, true);

	sinon
		.stub(helpers, 'cleanUp')
		.yields(null, true);
}
	...
	findEntry: function (soajs, opts, cb) {
		if (opts.collection === 'templates') {
			template.deploy.deployments.pre = {
                "infra__dot__cluster__dot__deploy": {"imfv" : [
                    {
                        "command":{
                            "method" : "post",
