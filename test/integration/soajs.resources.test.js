...
let sampleResourceCopy = {}, sampleResourceCopyId = '';
let sampleResource = {
	"name": "cluster1",
	"type": "cluster",
	"category": "mongo",
		...
		it.skip("fail - adding resource record with additional parameters", function (done) { // additional properties currently turned off, for vm
			...
			executeMyRequest(params, 'resources', 'post', function (body) {
				...
				assert.deepEqual(body.errors.details[0], {
					code: 173,
					message: 'Validation failed for field: resource -> The parameter \'resource\' failed due to: instance additionalProperty \"invalidInput\" exists in instance when not allowed'
				});
				done();
			});
		});
