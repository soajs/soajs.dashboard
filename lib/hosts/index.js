							...
							hostModel.getEnvInfo(soajs, BL.model, { envList: envsList }, function (error, result) {
								checkReturnError(soajs, cbMain, {
									...
									result.forEach(function (oneEnvRec) {
										...
										if(oneEnvRec.domain === ''){
											let deployerInfo = oneEnvRec.deployer.selected.split(".");
											if(oneEnvRec.deployer[deployerInfo[0]][deployerInfo[1]][deployerInfo[2]]){
												if(oneEnvRec.deployer[deployerInfo[0]][deployerInfo[1]][deployerInfo[2]].nodes && oneEnvRec.deployer[deployerInfo[0]][deployerInfo[1]][deployerInfo[2]].nodes !== ''){
													output[oneEnvRec.code.toLowerCase()]['domain'] = oneEnvRec.nodes;
												}
											}
										}
										else {
