						...
						async.map(envs, function (oneEnv, callback) {
							...
							checkReturnError(soajs, cbMain, { config: config, error: error, code: 600 }, function () {
								...
								hostModel.getEnvInfo(soajs, BL.model, { envList: envsList }, function (error, result) {
									checkReturnError(soajs, cbMain, { config: config, error: error, code: 600 }, function () {
										var response = {};
										async.each(result, (oneEnvRec, mCb) => {
											...
											if(process.env.SOAJS_SAAS){
												soajs.log.debug("checking for access token in saas mode");
												helpers.checkAccessToken(soajs, oneEnvRec.code.toLowerCase(), BL.model, (error, accessToken) => {
													checkReturnError(soajs, cbMain, { config: config, error: error, code: 600 }, function () {
														if(accessToken && accessToken.token){
															soajs.log.debug("found token:", accessToken.token);
															if(response[oneEnvRec.code.toLowerCase()]){
																response[oneEnvRec.code.toLowerCase()]['access_token'] = accessToken.token;
															}
														}
														else{
															soajs.log.debug("no token found");
															if(response[oneEnvRec.code.toLowerCase()]){
																response[oneEnvRec.code.toLowerCase()]['access_token'] = null;
															}
														}
					...
					checkReturnError(soajs, cbMain, { config: config, error: !options, code: 825 }, function () {
						...
						deployer.listServices(options, function (error, services) {
							checkReturnError(soajs, cbMain, { config: config, error: error }, function () {
								...
								if(process.env.SOAJS_SAAS){
									soajs.log.debug("checking for access token in saas mode");
									helpers.checkAccessToken(soajs, oneEnv.code.toLowerCase(), BL.model, (error, accessToken) => {
										checkReturnError(soajs, cbMain, { config: config, error: error, code: 600 }, function () {
											if(accessToken && accessToken.token){
												soajs.log.debug("found token:", accessToken.token);
												if(output[oneEnv.code.toLowerCase()]){
													output[oneEnv.code.toLowerCase()]['access_token'] = accessToken.token;
												}
											}
											else{
												soajs.log.debug("no token found");
												if(output[oneEnv.code.toLowerCase()]){
													output[oneEnv.code.toLowerCase()]['access_token'] = null;
												}
											}
											return mCb(null, true);
										});
