...
var BL = {
	 ...
	 * @param cbMain
	 */
	"awareness": function (config, soajs, model, cbMain) {
		let opts = {
			collection: 'environment',
			conditions: {code: soajs.inputmaskData.env.toUpperCase()}
		};
		
		let controllersModel = model.controllersModel;
		BL.model.findEntry(soajs, opts, function (err, envRecord) {
			checkReturnError(soajs, cbMain, {config: config, error: err || !envRecord, code: 600}, function () {
				opts = {
				...
				controllersModel.findEntries(soajs, BL.model, opts, function (error, controllers) {
					checkReturnError(soajs, cbMain, {config: config, error: err, code: 600}, function () {
		...
		async.auto({
			"validateRequest": (mCb) => {
				...
				if(soajs.inputmaskData.serviceName === 'controller'){
					...
					allowedOperations = ["heartbeat", "reloadRegistry", "loadProvision", "resourceInfo"];
				}
				else if(soajs.inputmaskData.serviceType === 'daemon'){
					allowedOperations = ["heartbeat", "reloadRegistry", "loadProvision", 'daemonStats', 'reloadDaemonConf', "resourceInfo"];
				}
				
				if(allowedOperations && allowedOperations.length > 0 && allowedOperations.indexOf(soajs.inputmaskData.operation) === -1){
				...
				let opts = {
					...
					conditions: {
						"$or": [{
							...
							version: soajs.inputmaskData.serviceVersion.toString()
						},
							{
								name: serviceName,
								env: soajs.inputmaskData.env.toLowerCase(),
								version: parseFloat(soajs.inputmaskData.serviceVersion)
							}
						]
					}
				};
			...
			"executeMaintenanceOperation": ["getServiceHost", (info, mCb) =>{
				...
				if (!soajs.inputmaskData.portType || soajs.inputmaskData.portType === "maintenance"){
					port = info.getServiceHost.port + maintenanceInc;
				}
				if (soajs.inputmaskData.portType === "inherit"){
					port = info.getServiceHost.port;
				}
				else if (soajs.inputmaskData.portType === "custom" && soajs.inputmaskData.portValue){
					port = soajs.inputmaskData.portValue;
				}
				let requestOptions = {
	...
	"start": function(config, soajs, cbMain){
		async.auto({
			...
			"getServiceHost": ["getEnvironment", (info, mCb) => {
				let serviceName = soajs.inputmaskData.serviceName;
				if(!serviceName){
					return mCb({ config: config, error: config.errors[524], code: 524 });
				}
				...
				async.auto({
					...
					"getServiceHost": ["startService", (info, mCb) => {
						function checkAgain(vCb){
							...
							BL.model.findEntry(soajs, opts, vCb);
						}
						
						function myCallback(error, hosRecord) {
							checkReturnError(soajs, mCb, { config: config, error: error, code: 600 }, () => {
								if(hosRecord){
									setTimeout(() => {
										return mCb(null, hosRecord);
									}, 1000);
								}
								else{
									counter ++;
