...
var BL = {
	 ...
	 * @ param response
	 */
	"listHostEnv": function (config, soajs, registry, deployer, helpers, cbMain) {
		if (process.env.SOAJS_DEPLOY_HA) {
			BL.listHAhostEnv(config, soajs, registry, deployer, helpers, cbMain);
		}
		else {
	...
	"listHAhostEnv": function (config, soajs, registry, deployer, helpers, cbMain) {
		var output = {};
			...
			checkReturnError(soajs, cbMain, { config: config, error: error, code: 600 }, function () {
				...
				async.each(environments, function (oneEnv, mCb) {
					...
					registry.loadByEnv({ envCode: oneEnv.code.toLowerCase() }, function(error, envRegistry) {
						checkReturnError(soajs, cbMain, { config: config, error: error || !envRegistry, code: 446 }, function () {
							var options = helpers.buildDeployerOptions(envRegistry, soajs, BL.model);
