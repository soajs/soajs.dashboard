...
var helpers = {
		...
		if (checkACL(soajs)) {
			...
			async.each(envCodes, function (oneEnvCode, mCb) {
				...
				model.findEntries(soajs, opts, function (error, tenants) {
					...
					async.each(tenants, (oneTenant, vCb) => {
						if(process.env.SOAJS_SAAS){
							helpers.checkAccessToken(soajs, oneEnvCode, oneTenant._id, model, (error, accessToken) => {
								if(error){
									return vCb(error);
								}
										...
										oneKey.extKeys.forEach(function (oneExtKey) {
											if (oneExtKey.env === oneEnvCode.toUpperCase()) {
												output[ oneEnvCode ][ 'tenants' ].push({
													...
													access_token: (accessToken && accessToken.token) ? accessToken.token : null
												});
											}
										});
									});
	...
	checkAccessToken: function(soajs, envCode, tenantId, model, cb){
		var opts = {
