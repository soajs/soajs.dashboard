...
module.exports = {
	...
	"extKeyGet": function (config, mongo, req, res) {
		...
		if (req.soajs.inputmaskData.main) {
			//get main dashboard key
			var extKey = req.soajs.servicesConfig.dashboard.key;
			findExtKeyMain(req.soajs.tenant.code, extKey, function (error, extKeyCheck) {
				if (error) {
		...
		function findExtKeyMain(tenantCode, requestedKey, cb) {
			mongo.findOne(colName, {"code": tenantCode}, function (error, record) {
				if (error || !record) {
					...
					record.applications.forEach(function (oneApplication) {
						if (oneApplication.keys) {
							oneApplication.keys.forEach(function (oneKey) {
								if (oneKey.extKeys) {
									oneKey.extKeys.forEach(function (oneExtKey) {
										if (oneExtKey.extKey === requestedKey) {
											found = true;
										}
									});
								}
	...
	"addDashboardKey": function (config, mongo, req, res) {
		...
		function doAdd() {
			mongo.count(dbKeysColl, {'code': record.code}, function (error, count) {
				if (error) {
					return res.jsonp(req.soajs.buildResponse({"code": 456, "msg": config.errors[456]}));
				}

				if (count > 0) {
					return res.jsonp(req.soajs.buildResponse({"code": 705, "msg": config.errors[705]}));
				}
