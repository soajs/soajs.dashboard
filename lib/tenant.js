	...
	"permissionsGet": function (config, req, res) {
		...
		checkIfError(req, res, {
			...
			if (req.soajs.inputmaskData.envCode) {
				getUserAclPerEnv(req.soajs.inputmaskData.envCode,function(ACL){
		...
		/**
		 * look in the tenants' collection, and try to get the ACL
		 * If it wasn't found, call getAclFromProduct
		 *
		 * @param {string} tenantCode
		 * @param {string} applicationId
		 * @param {ACL object} callBack
		 */
		function getAclFromTenantOrProduct(tenantCode, applicationId, forceFromProduct, callBack) {
			var output = {}; // will have acl defined if found, if not, product and package
			...
			BL.model.findEntries(req.soajs, combo, function (error, records) {
				if (error) {
					req.soajs.log.error(error);
					return res.jsonp(req.soajs.buildResponse({"code": 454, "msg": 'Tenant does not exist'}));
				}
				
				var applications = records[0].applications;
				
				applications.forEach(function (application) {
					if (application.appId.toString() === applicationId) {
						if (application.acl && !forceFromProduct) {
							output.acl = application.acl;
						} else {
							output.product = application.product;
							output.package = application.package;
						}
					}
				});
				...
				if (output.acl) {
					...
					getAclFromProduct(output, function (ACL) {
						callBack(ACL);
					});
				}
			});
		...
		/**
		 * look in products' collection, and try to get the ACL
		 *
		 * @param {object} tenantsData - must have {string}product & {string} package
		 * @param {ACL object} callback
		 */
		function getAclFromProduct(tenantsData, callback) {
			var prod = tenantsData.product;
			var pack = tenantsData.package;
			...
			BL.model.findEntries(req.soajs, combo, function (error, records) {
				if (error) {
					req.soajs.log.error(error);
					return res.jsonp(req.soajs.buildResponse({"code": 460, "msg": 'Unable to find product'}));
				}
				
				var packages = records[0].packages;
				var packFound = false;
				
				packages.forEach(function (eachPack) {
					if (eachPack.code.toString() === pack) {
						packFound = true;
						return callback(eachPack.acl);
					}
				});
				
				if (!packFound) {
					return res.jsonp(req.soajs.buildResponse({"code": 461, "msg": 'Unable to find package'}));
				}
			});
		}
		
		// NOT FULLY TESTED -=-=-=-=-=-=-
		function getUserAclPerEnv(envCode, cb) {
			...
			if (!ACL) {
				...
				var applicationId = tenant.application.appId;
				getAclFromTenantOrProduct(tenantCode, applicationId, true, function (ACL) {
					return cb(ACL[envCode.toLowerCase()]);
				});
			} else {
		...
		function getUserAllAclPerKey(cb) {
			...
			if (!ACL) {
				...
				getAclFromTenantOrProduct(tenantCode, applicationId, false, function (ACL) {
					var opts = {
