...
module.exports = {
	...
	"permissionsGet": function (config, mongo, req, res) {
		...
		//todo: remove the below two lines and accomodate code in getUserAclPerKey
		var ACL = {
			"dev": {
				"urac": {
					"apis": {
		...
		var c = process.env.SOAJS_ENV.toLowerCase();
		...
		checkIfError(req, res, {
			...
			if (req.soajs.inputmaskData.envCode) {
				c = req.soajs.inputmaskData.envCode.toLowerCase();
				ACL = ACL[c];
		...
		function getUserAclPerKey(cb) {
			mongo.find(envColName, {}, {"code": 1, "dbs": 1}, function (err, records) {
