	...
	"go": function (config, mongo, req, res) {
		...
		function logoutRequest(oneEnvCode) {
			...
			if (requestedEnv === process.env.SOAJS_ENV.toLowerCase()) {
				constructProxyRequest(req, requestedEnv, sessionObj.envAuth[requestedEnv], "/urac/logout", null,  res);
			}
			else {
				mongo.findOne("dashboard_extKeys", {
					"code": req.soajs.tenant.code.toUpperCase(),
					"env": requestedEnv.toUpperCase()
					...
					if (error || !keyRecord) {
						if (error) {
							...
							return res.json(req.soajs.buildResponse({"code": 903, "msg": config.errors[903].replace("%envCode%", requestedEnv)}));
						}
					}
					var extKey = keyRecord.key;
					constructProxyRequest(req, requestedEnv, sessionObj.envAuth[requestedEnv], "/urac/logout", extKey);
				});
			}
		}

		function constructProxyRequest(req, envCode, soajsauth, requestedRoute, extKey, res) {
