			...
			async.eachSeries(ci, (oneRecipe, cb) => {
				...
				if (!status.valid) {
					status.errors.forEach(function (err) {
						context.errors.push({code: 173, msg: `<b>${oneRecipe.name}</b>: ` + err.stack, group: "Continuous Integration Recipes"})
					});
					return cb();
				}
				else {
					...
					BL.model.countEntries(req.soajs, opts, function (error, count) {
						lib.checkReturnError(req, cb, {config: context.config, error: error, code: 600}, () => {
							if (count && count === 1) {
								context.errors.push({
									"code": 967,
									"msg": `<b>${oneRecipe.name}</b> already exists for provider <b>${oneRecipe.provider}</b> => ${oneRecipe.provider}/${oneRecipe.name}`,
									'entry': {
										'name': oneRecipe.name,
										'provider': oneRecipe.provider,
										'type': 'ci'
	...
	"merge": function (req, context, lib, async, BL, callback) {
		...
		if (req.soajs.inputmaskData.correction && req.soajs.inputmaskData.correction.ci) {
			req.soajs.inputmaskData.correction.ci.forEach((oneCiInput) => {
				if (context.template.content.recipes && context.template.content.recipes.ci) {
			...
			lib.initBLModel('ci', (error, ciModel) => {
				lib.checkReturnError(req, callback, {config: context.config, error: error, code: 600}, () => {
					...
					async.eachSeries(ci, (oneRecipe, cb) => {
						...
						req.soajs.inputmaskData.recipe = oneRecipe.recipe;
