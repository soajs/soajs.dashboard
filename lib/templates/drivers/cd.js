				...
				if (!status.valid) {
					...
					BL.model.countEntries(req.soajs, opts, function (error, count) {
						lib.checkReturnError(req, cb, {config: context.config, error: error, code: 600}, () => {
							...
							async.series({
								"checkPorts": (mCb) => {
                                    if (oneCatalog.restriction && oneCatalog.restriction.deployment && oneCatalog.restriction.deployment.length === 1 && oneCatalog.restriction.deployment[0] === 'vm') {
                                        if (oneCatalog.recipe.deployOptions && oneCatalog.recipe.deployOptions.ports && oneCatalog.recipe.deployOptions.ports.length > 0) {
                                            var vmPorts = oneCatalog.recipe.deployOptions.ports;
                                            async.each(vmPorts, function (oneVmPort, fCb) {
                                                if (oneVmPort.published && oneVmPort.published < 0 || oneVmPort.published > 65535) {
                                                    context.errors.push({
                                                        code: 173,
                                                        msg: `<b>${oneCatalog.name}</b>: The port chosen is outside the range of valid exposed ports (0 , 65535)`,
                                        ...
                                        if (oneCatalog.recipe.deployOptions && oneCatalog.recipe.deployOptions.ports && oneCatalog.recipe.deployOptions.ports.length > 0) {
                                            ...
                                            async.each(ports, function (onePort, callback) {
                                                ...
                                                if (onePort.published && onePort.published < 0 || onePort.published > 2767) {
                                                    context.errors.push({
                                                        code: 173,
                                                        msg: `<b>${oneCatalog.name}</b>: The port chosen is outside the range of valid exposed ports (0 , 2767)`,
                                                        group: "Catalog Deployment Recipes"
                                                    });
                                                }
                                                return callback();
								...
								"checkSourceCode": (mCb) => {
									if (oneCatalog.recipe.deployOptions.sourceCode) {
										...
										if (oneCatalog.recipe.deployOptions.sourceCode.custom) {
											...
											if (allowedTypes.indexOf(oneCatalog.type) === -1) {
												context.errors.push({
													code: 173,
													msg: `<b>${oneCatalog.name}</b>: Custom repos are only available for catalogs of type server`,
													group: "Catalog Deployment Recipes"
												})
											}
