                                ...
                                async.forEach(template.content.tenant.data, (oneTenant, mainCb) => {
                                    async.forEach(oneTenant.applications, (oneApplication, appCb) => {
                                        ...
                                        if (product.indexOf(oneApplication.product) !== -1) {
                                            async.forEach(productization, (oneProduct, prodCb) => {
                                                if (oneProduct.product === oneApplication.product) {
                                                    ...
                                                    if (oneProduct.package.indexOf(oneApplication.package) !== -1) {
                                                        ...
                                                        context.errors.push({
                                                            code: 173,
                                                            msg: `<b>${oneTenant.code}</b>, Invalid template, <b>${oneApplication.package}</b> Package that belongs to <b>${oneApplication.product}</b> product, not found.`,
                                                            group: "Tenants"
                                                        });
                                                        return prodCb();
                                                    }
                                            ...
                                            context.errors.push({
                                                code: 173,
                                                msg: `<b>${oneTenant.code}</b>, Invalid template, <b>${oneApplication.product}</b> product not found.`,
                                                group: "Tenants"
                                            });
                                            return appCb();
                                        }
