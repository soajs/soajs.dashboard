								...
								async.forEach(infraRecords, (oneProvider, lCb) => {
									...
									//if template is external, call cloostro and send template to cloud provider cdn
										...
										if (oneProvider.name === oneRecipe.infra.name) {
											...
											fs.writeFile(tempfilepath, JSON.stringify(content, null, 2), (err) => {
												if (err) {
												...
												deployer.execute({
													...
													lib.checkReturnError(req, lCb, {
														...
														if (oneRecipe.inputs) {
															...
															if (oneRecipe.inputs) {
																if (typeof oneRecipe.inputs === 'string') {
																	try {
                                                                        req.soajs.inputmaskData.inputs = JSON.parse(oneRecipe.inputs);
																	} catch (e) {
																		return lCb(e);
                                                                    }
                                                                }
                                                                if(oneRecipe.inputs.length > 0){
                                                                    req.soajs.inputmaskData.inputs = oneRecipe.inputs;
                                                                }
                                                                else{
	                                                                req.soajs.inputmaskData.inputs = [];
                                                                }
															}
															if (oneRecipe.display) {
                                                                if (typeof oneRecipe.display === 'string') {
                                                                    try {
                                                                        req.soajs.inputmaskData.display = JSON.parse(oneRecipe.display);
                                                                    } catch (e) {
                                                                        return lCb(e);
                                                                    }
                                                                }
                                                                if(Object.keys(oneRecipe.display).length > 0){
                                                                    req.soajs.inputmaskData.display = oneRecipe.display;
                                                                }
                                                                else{
	                                                                req.soajs.inputmaskData.display = {};
                                                                }
															}
															if (oneRecipe.imfv) {
                                                                if (typeof oneRecipe.imfv === 'string') {
                                                                    try {
                                                                        req.soajs.inputmaskData.imfv = JSON.parse(oneRecipe.imfv);
                                                                    } catch (e) {
                                                                        return lCb(e);
                                                                    }
                                                                }
                                                                if(Object.keys(oneRecipe.imfv).length > 0){
                                                                    req.soajs.inputmaskData.imfv = oneRecipe.imfv;
                                                                }
                                                                else{
	                                                                req.soajs.inputmaskData.imfv = {};
                                                                }
															}
                                                            let copiedIMFV = soajsUtils.cloneObj(req.soajs.inputmaskData);
                                                            req.soajs.inputmaskData = soajsUtils.cloneObj(copiedIMFV);
										...
										async.map(records, (oneRecord, mCb) => {
											...
											BL.model.findEntries(req.soajs, {
												...
												lib.checkReturnError(req, mCb, {
													...
													if (oneRecord.content && typeof oneRecord.content !== 'string') {
														oneRecord.content = JSON.stringify(oneRecord.content)
													}
													
                                                    if (oneRecord.inputs && typeof oneRecord.inputs !== 'string') {
                                                        oneRecord.inputs = JSON.stringify(oneRecord.inputs)
                                                    }
                                                    if (oneRecord.display && typeof oneRecord.display !== 'string') {
                                                        oneRecord.display = JSON.stringify(oneRecord.display)
                                                    }
                                                    if (oneRecord.imfv && typeof oneRecord.imfv !== 'string') {
                                                        oneRecord.imfv = JSON.stringify(oneRecord.imfv)
                                                    }
													oneRecord.infra = {
				...
				if (req.soajs.inputmaskData.external && req.soajs.inputmaskData.external && req.soajs.inputmaskData.external.iac && req.soajs.inputmaskData.external.iac.length > 0) {
					...
					async.forEach(external, (oneExternal, eCb) => {
						let opt = {
							collection: 'infra',
							conditions: {
								label: oneExternal.split('__')[1] // name of infra
							}
						};
										...
										if (myInfra.templates && myInfra.templates.length > 0) {
											async.forEach(myInfra.templates, (oneTemplate, tCb) => {
												if (oneTemplate.name === oneExternal.split('__')[0]) {
													...
													if (oneTemplate.inputs && typeof oneTemplate.inputs !== 'string') {
														oneTemplate.inputs = JSON.stringify(oneTemplate.inputs)
													}
													if (oneTemplate.display && typeof oneTemplate.display !== 'string') {
														oneTemplate.display = JSON.stringify(oneTemplate.display)
													}
													if (oneTemplate.imfv && typeof oneTemplate.imfv !== 'string') {
														oneTemplate.imfv = JSON.stringify(oneTemplate.imfv)
													}
													oneTemplate.infra = {
														name: oneInfra.name
													};
