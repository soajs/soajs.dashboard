					...
					"validateSchema": (mCb) => {
						...
						if (!status.valid) {
							...
							status.errors.forEach(function (err) {
								errors.push({code: 173, msg: `Endpoint ${oneEndpoint.serviceName}: ` + err.stack})
							});
							return mCb(errors);
						}
						...
						BL.model.countEntries(req.soajs, opts, function (error, count) {
							lib.checkReturnError(req, cb, {config: context.config, error: error, code: 600}, () => {
								if (count && count === 1) {
									return mCb({
										"code": 173,
										"msg": `An entry with the same name or port as endpoint ${oneEndpoint.serviceName} already exists => ${oneEndpoint.serviceName}`,
										'entry':{
