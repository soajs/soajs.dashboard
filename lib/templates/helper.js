				...
				writeStream.on('close', function () {
					...
					try {
						fs.createReadStream(path + fileName + ".zip").pipe(unzip2.Extract({path: path})).on('error', (error) => {
							...
							let f1 = (fileName.indexOf(".json") !== -1) ? fileName : fileName + ".json";
							fs.exists(path + f1, (exists) =>{
								if(!exists){
									let f2 = (fileName.indexOf(".js") !== -1) ? fileName : fileName + ".js";
									fs.exists(path + f2, (exists) =>{
										if(!exists){
											return mCb({code: 600, msg: "Unable to Load or Parse Uploaded Template!"});
										}
										else{
											parseTheFile(path + f2);
										}
									});
								}
								else{
									parseTheFile(path + f1);
								}
							});
	...
	"checkMandatoryTemplateSchema": function (req, BL, lib, context, validator, mCb) {
		...
		BL.model.countEntries(req.soajs, {
			...
			lib.checkReturnError(req, mCb, {config: context.config, error: error, code: 600}, () => {
				lib.checkReturnError(req, mCb, {config: context.config, error: count > 0, code: 480}, () => {
					return mCb(null, true);
				});
			});
		});
