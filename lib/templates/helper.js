        ...
        BL.model.findEntries(req.soajs, opts, function (error, result) {
            lib.checkReturnError(req, mCb, {
                ...
                async.forEach(result, (oneResult, prodCb) => {
                    ...
                    if (myTemplate.content.productization && (myTemplate.content.productization.data && Array.isArray(myTemplate.content.productization.data) && myTemplate.content.productization.data.length > 0 )) {
                        async.forEach(myTemplate.content.productization.data, (oneProd, cb) => {
                ...
                async.forEach(result, (oneProduct, mainCb) => {
                    ...
                    if (myTemplate.content.tenant && (myTemplate.content.tenant.data && Array.isArray(myTemplate.content.tenant.data) && myTemplate.content.tenant.data.length > 0 )) {
                        async.forEach(myTemplate.content.tenant.data, (oneTenant, mainCb) => {
                            async.forEach(oneTenant.applications, (oneApplication, appCb) => {
                                ...
                                if (product.indexOf(oneApplication.product) !== -1) {
                                    async.forEach(productization, (oneProduct, prodCb) => {
                                        ...
                                        if (oneProduct.product === oneApplication.product) {
                                            ...
                                            if (oneProduct.package.indexOf(oneApplication.package) !== -1) {
                                                ...
                                                errors.push({
                                                    code: 173,
                                                    msg: `Invalid template, <b>${oneApplication.package}</b> Package that belongs to <b>${oneApplication.product}</b> product not found.`,
                                                    group: "Tenant"
                                                });
                                            }
                                        } else {
                                    ...
                                    errors.push({
                                        code: 173,
                                        msg: `Invalid template, <b>${oneApplication.product}</b> product not found.`,
                                        group: "Tenant"
                                    });
                                    return appCb();
                                }
