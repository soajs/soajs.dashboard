				...
				writeStream.on('close', function () {
					...
					fs.createReadStream(path + fileName + ".zip").pipe(unzip2.Parse()).on('entry', (entry) => {
						if (entry.type === 'File' && !entry.path.includes("__MACOSX")&& !entry.path.includes("DS_Store")) {
							...
							filtered = filtered + "_" + currentDate;
							entry.pipe(fs.createWriteStream(path + filtered));
							filesWithin.push(path + filtered);
	...
	"checkMandatoryTemplateSchema": function (req, BL, lib, context, validator, ts, mCb) {
		...
		if(myTemplate.restriction && myTemplate.restriction.driver){
			if(!myTemplate.restriction.deployment){
				errors.push({code: 173, msg: "Detected restriction drivers configuration but template is missing restriction parent deployment configuration!", group: "General"});
			}
			else{
				myTemplate.restriction.driver.forEach((oneDriver) => {
					...
					if(myTemplate.restriction.deployment.indexOf(driverPrefix) === -1){
						errors.push({code: 173, msg: `Restriction driver ${driverName} found, but either its associated parent restriction deployment ${driverPrefix} is not supported or not provided!`, group: "General"});
					}
				});
			}
		}
		...
		async.forEach(content, (oneContent,cb) => {
			if (oneContent !== 'recipes' && oneContent !== 'deployments' && oneContent !== 'endpoints' && oneContent !== 'daemonGroups') {
        ...
        async.forEach(deploy, (oneDeploy, cb) => {
        	    if(['infra.cluster.deploy', 'infra.dns'].indexOf(oneDeploy) === -1){
		...
		if(!myTemplate.restriction){
			myTemplate.restriction={};
		}

		if(forceContainer){
			if(!myTemplate.restriction.deployment){
				myTemplate.restriction.deployment = [];
			}
			myTemplate.restriction.deployment = ['container'];
		}
