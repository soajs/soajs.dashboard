...
var helpers = {
        ...
        async.concat(envs, function (oneEnv, cback) {
            ...
            async.concat(oneEnv.services, function (oneService, callback) {
                ...
                if (cdEnvConfig[serviceName]) {
                    ...
                    req.soajs.log.debug("no specific entry for service ", serviceName, "found in CD configuration, ignoring CD update/notify for service.");
                    // check if service matches the global CD configuration
                    //Global entry per environment is no longer supported.
                }
	...
	callDeployer: function (config, req, registry, deployer, opName, cb) {
		var cloudDeploy = require("../../cloud/deploy.js");
