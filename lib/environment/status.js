...
var statusChecker = {
	...
	"startDeployment": function (req, BL, config, environmentRecord, template, cbMain) {
		...
		BL.model.findEntry(req.soajs, {collection: 'templates', conditions: {code: environmentRecord.code} }, (error, dbTemplate) => {
				...
				BL.model.findEntry(req.soajs, {collection: 'templates', conditions: {type: "_" + template.type} }, (error, schema) =>{
					...
					for(let section in template){
						if(template[section].wf && template[section].wf.deploy){
							let injectedStep = {
								"pre": function(fCb){
									statusUtils.redirectTo3rdPartyDeploy(req, {BL, environmentRecord, template, config}, section, fCb);
								},
					...
					async.eachSeries(stack, (oneStep, bCb) => {
						...
						if (error) {
							...
							updateTemplate((error) => {
								...
								environmentRecord.error = template.error.message;
								BL.model.saveEntry(req.soajs, {collection: 'environment', record: environmentRecord}, (error) => {
									if (error) {
										req.soajs.log.error(error);
									}
								});
			...
			if(req.soajs.inputmaskData.rollback){
						...
						let oneStep = {
							"pre": function (fCb){
								...
								//step is not completed but has a post function that needs execution
									...
									predefinedSchemaSteps[schemaStepName].post(req, {BL, config, environmentRecord, template}, () => {
				...
				for(let section in template){
					if(template[section].wf && template[section].wf.status){
						...
						let injectedStep = {
							"pre": function(fCb){
								statusUtils.redirectTo3rdPartyStatus(req, {BL, environmentRecord, template, config}, section, (error, data) => {
									if(error){
										req.soajs.log.error(error);
									}
									else{
				...
				async.eachSeries(stack, (oneStackStep, vCb) => {
					...
					if(error){
						...
						delete environmentRecord.pending;
						environmentRecord.error = error.message;
						BL.model.saveEntry(req.soajs, {collection: 'environment', record: environmentRecord}, (error) => {
							if (error) {
								req.soajs.log.error(error);
							}
	...
	"rollbackDeployment": function(req, context, cbMain){
		...
		for(let i = context.schemaOptions.length -1; i >=0; i--){
			if(context.template[context.schemaOptions[i]] && context.template[context.schemaOptions[i]]._id && Object.hasOwnProperty.call(predefinedSchemaSteps[context.schemaOptions[i]], 'rollback')) {
				let oneStep = {
					"pre": function (fCb) {
		...
		async.eachSeries(stack, (oneStackStep, vCb) => {
			...
			if(error){
				delete context.environmentRecord.pending;
				context.environmentRecord.error = error.message;
				context.BL.model.saveEntry(req.soajs, {collection: 'environment', record: context.environmentRecord}, cbMain);
			}
			else{
