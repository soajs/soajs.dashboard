					...
					for(let section in template){
						if(template[section].wf && template[section].wf.deploy){
							...
							if(template[section].position){
								stack.splice(template[section].position, 0, injectedStep);
							}
							else {
		...
		BL.model.findEntry(req.soajs, {collection: 'templates', conditions: {type: "_" + template.type} }, (error, schema) => {
			...
			for(let section in template){
				if(template[section].wf && template[section].wf.status){
					let injectedStep = statusUtils.redirectTo3rdParty(req, {BL, environmentRecord, template, config}, section, (error, response) => {
						console.log("***********");
						console.log(error);
					...
					if(template[section].position){
						schemaOptions.splice(template[section].position, 0, injectedStep);
					}
					else {
						schemaOptions.push(injectedStep);
					}
				}
			}
			...
			schemaOptions.forEach((schemaStepName) => {
				...
				if(template[schemaStepName]){
					stack[schemaStepName] = JSON.parse(JSON.stringify(predefinedSchemaSteps[schemaStepName].status));
					if (template[schemaStepName]._id) {
						...
						if(template[schemaStepName].exception){
							stack[schemaStepName].exception = template[schemaStepName].exception;
						}
					}
		...
		for(let section in context.template){
			if(context.template[section].wf && context.template[section].wf.rollback){
				...
				if(context.template[section].position){
					stack.splice(context.template[section].position, 0, injectedStep);
				}
				else {
					stack.push(injectedStep);
				}
