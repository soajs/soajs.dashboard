...
var statusChecker = {
	...
	"startDeployment": function (req, BL, config, environmentRecord, template, cbMain) {
		...
		statusUtils.uploadCertificates(req, {BL, config, environmentRecord, template}, (error) => {
			...
			BL.model.findEntry(req.soajs, {
				...
				let project = template.gi.project ? template.gi.project.name : null;
				if (project){
				...
				schemaOptions.forEach((schemaStepName) => {
					let oneStep = {
						"templateStatusPre": function(fCb){
							if(predefinedSchemaSteps[schemaStepName].modifyTemplateStatus){
								template.ready = false;
								req.soajs.log.debug(schemaStepName + " setting template to: NOT READY.");
								updateTemplate(fCb);
							}
							else return fCb();
						},
						...
						"templateStatusPost": function(fCb){
							if(predefinedSchemaSteps[schemaStepName].modifyTemplateStatus){
								template.ready = true;
								req.soajs.log.debug(schemaStepName + " setting template to: READY.");
								updateTemplate(fCb);
							}
							else return fCb();
						},
				...
				for (let section in template) {
					if (template[section] && template[section].wf && template[section].wf.deploy) {
						let injectedStep = {
							"templateStatusPre": function(fCb){
								if(template[section].wf && template[section].wf.modifyTemplateStatus){
									template.ready = false;
									req.soajs.log.debug(section + " setting template to: NOT READY.");
									updateTemplate(fCb);
								}
								else return fCb();
							},
							...
							"templateStatusPost": function(fCb){
								if(template[section].wf && template[section].wf.modifyTemplateStatus){
									template.ready = true;
									req.soajs.log.debug(section + " setting template to: READY.");
									updateTemplate(fCb);
								}
								else return fCb();
							},
				...
				async.eachSeries(stack, (oneStep, bCb) => {
					...
					if (error) {
						 ...
						 */
						req.soajs.log.error("Error Deploying Environment, check Deployment Status for Information.");
	...
	"checkProgress": function (req, BL, config, environmentRecord, template, cbMain) {
		...
		/**
		 * return API response if valid
		 * @param stack
		 * @param response
		 * @returns {*}
		 */
		function returnValidResponse(stack, response){
			...
			stack.forEach((oneEntry) => {
				for(let stepName in oneEntry){
					if(stepName !== 'update'){
						if(predefinedSchemaSteps[stepName] && !response[stepName]){
							response[stepName] = predefinedSchemaSteps[stepName].status;
						}
						
						if (!response[stepName] || !response[stepName].done) {
							overallProgress = false;
						}
						else if(!response[stepName]){
							response[stepName] = { done: false, id: null};
						}
					}
				}
			});
		...
		BL.model.findEntry(req.soajs, {
			...
			if (rollbackGo) {
				...
				schemaOptions.forEach((schemaStepName) => {
					...
					if (template[schemaStepName]) {
						...
						oneStep[schemaStepName] = function (fCb) {
								req.soajs.log.debug("Checking:", schemaStepName);
				 ...
				 */
				for (let section in template) {
					if (template[section] && template[section].wf && template[section].wf.status) {
						response[section] = {done: false, id: null};
						let injectedStep = {};
						injectedStep[section] = function (fCb) {
							req.soajs.log.debug("Checking:", section);
							
							if(template[section] && template[section]._id){
								response[section] = {
									done: true,
								...
								req.soajs.log.debug(section, "checking ...");
								statusUtils.redirectTo3rdPartyStatus(req, {
									...
									if (error) {
										req.soajs.log.error(error);
										if(!template[section].errorCount){
											template[section].errorCount =0;
										}
										template[section].errorCount++;
										
										if(template[section].errorCount > 5){
											return fCb(error);
										}
									}
									else {
				...
				if(!template.ready){
					returnValidResponse(stack, response)
				}
				else{
	...
	"rollbackDeployment": function (req, context, cbMain) {
		...
		let upToStep;
		if (req.soajs.inputmaskData.rollback > 0) {
