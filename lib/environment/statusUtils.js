	...
	deployNginx: function (req, context, cb) {
		...
		initBLModel(deployBL, modelName, (error, deployBL) => {
			...
			deployBL.deployService(context.config, req.soajs, deployer, (error, out) => {
				...
				if(context.template.nginx.http && context.template.infra){
					let ports = {};
					ports.http = context.template.nginx.http;
					if(context.template.nginx.https){
						ports.https = context.template.nginx.https;
					}
					
					req.soajs.awareness.getHost('controller', function (host) {
						let opts = {
							"uri": 'http://' + host + ':' + req.soajs.registry.services.controller.port + "/bridge/deployService",
							"headers": {
							...
							"body": {
								deployParams: {
									serviceId: context.template.nginx._id,
									env : context.template.gi.code.toUpperCase()
								},
								recipe: context.template.nginx.recipe
							},
