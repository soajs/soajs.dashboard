...
var lib = {
						...
						async.eachSeries(certificatesNames, (oneCertificateName, mCb) => {
							...
							fs.writeFile(filePath, oneCertificate.data, 'utf8', (error) => {
								...
								writeStream.on('error', function (error) {
									return lib.checkReturnError(req, cb, {
		...
		function multitenancyApiCall(mCb) {
			...
			function addApplicationKey(tenantBL, appId, tenantId, packageName, addKeyCb) {
				...
				tenantBL.createApplicationKey(context.config, provision, req, {}, (error, response) => {
					...
					tenantBL.addApplicationExtKeys(context.config, soajsCore, req, {}, (error, response) => {
						if (error) return cb(error);
	...
	deployservice: function (req, context, serviceName, version, cb) {
		...
		if (context.template[serviceName].custom) {
			...
			//handle all custom information related to extra config repo
			if (context.template[serviceName].custom.sourceCode && context.template[serviceName].custom.sourceCode.configuration && Object.keys(context.template[serviceName].custom.sourceCode.configuration).length > 0) {
				data.custom.sourceCode = {
					configuration: context.template[serviceName].custom.sourceCode.configuration
				};
			}
		}
			...
			"deployService": function (mCb) {
				...
				lib.initBLModel(deployBL, modelName, (error, deployBL) => {
					...
					deployBL.deployService(context.config, req.soajs, deployer, (error, out) => {
						...
						if(out && out.service){
							context.template[serviceName]._id = out.service.id;
						}
						return mCb(null, true);
					});
				});
	...
	createNginxRecipe: function (req, context, cb) {
		...
		let catalog = {
			...
			name: "Deployer Recipe for " + context.template.gi.code.toUpperCase() + " Nginx /" + timestamp,
			recipe: recipe,
			type: "server",
			subtype: "nginx"
		};
	...
	deployNginx: function (req, context, cb) {
		...
		if (context.template.nginx.custom) {
			...
			//handle all custom information related to extra config and/or ui repo
			if (context.template.nginx.custom.sourceCode) {
				if (context.template.nginx.custom.sourceCode.configuration && Object.keys(context.template.nginx.custom.sourceCode.configuration).length > 0) {
					if(!data.custom.sourceCode) {
						data.custom.sourceCode = {};
					}
					data.custom.sourceCode.configuration = context.template.nginx.custom.sourceCode.configuration;
				}
				
				if (context.template.nginx.custom.sourceCode.custom && Object.keys(context.template.nginx.custom.sourceCode.custom).length > 0) {
					if(!data.custom.sourceCode) {
						data.custom.sourceCode = {};
					}
					data.custom.sourceCode.custom = context.template.nginx.custom.sourceCode.custom;
				}
			}
		}
		...
		lib.initBLModel(deployBL, modelName, (error, deployBL) => {
			...
			deployBL.deployService(context.config, req.soajs, deployer, (error, out) => {
				...
				if(out && out.service) {
					context.template.nginx._id = out.service.id;
				}
				return cb(null, true);
			});
		});
	...
	createUserAndGroup: function (req, context, cb) {
		...
		request.post(options, (error, response, body) => {
			...
			if (error) {
				...
				if (context.template.user.count > 15) {
					//max attempts
