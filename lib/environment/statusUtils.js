		...
		if(context.template[section] && context.template[section].wf && context.template[section].wf.status){
			if(Array.isArray(context.template[section].wf.status)){
				async.mapSeries(context.template[section].wf.status, (oneStep, mCb) => {
					...
					return cb(null, context.template[section]._id);
				});
			}
			else if(typeof(context.template[section].wf.status) === 'object'){
				if(context.template[section].wf.status.recursive){
					lib.repeatCheckCall(req, context.template[section].wf.status, (error, response) => {
						if(error){
							return cb(error);
						}
						console.log(response);
