...
var lib = {
	...
	redirectTo3rdPartyStatus: function (req, context, section, cb) {
		if(context.template[section] && context.template[section].wf && context.template[section].wf.status){
			...
			//if there is _id and it matches the check schema, no need  to make a call anymore
			if(context.template[section]._id && context.template[section].wf.status.check){
				let valid = false;
				for(let field in context.template[section].wf.status.check){
					if(context.template[section].wf.status.check[field].type && Object.hasOwnProperty.call(context.template[section]._id, field) && typeof(context.template[section]._id[field]) === context.template[section].wf.status.check[field].type){
						valid = true;
					}
					else if(context.template[section].wf.status.check[field].value && Object.hasOwnProperty.call(context.template[section]._id, field) && context.template[section]._id[field] === context.template[section].wf.status.check[field].value){
						valid = true;
					}
				}
