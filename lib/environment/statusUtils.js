...
var lib = {
	...
	uploadCertificates: function (req, context, cb) {
		if (context.template.deploy.selectedDriver === 'docker') {
			...
			else if(context.template.deploy.deployment.docker.certificates){
	...
	deployNginx: function (req, context, cb) {
		...
		initBLModel(deployBL, modelName, (error, deployBL) => {
			...
			deployBL.deployService(context.config, req.soajs, deployer, (error, data) => {
				...
				context.template.nginx._id = data.id;
				return cb(null, true);
			});
		});
	},
	...
	redirectTo3rdPartyDeploy: function (req, context, section, cb) {
		if(context.template[section] && context.template[section].wf && context.template[section].wf.deploy){
			if(Array.isArray(context.template[section].wf.deploy)){
				async.mapSeries(context.template[section].wf.deploy, (oneStep, mCb) => {
					if(oneStep.recursive){
						lib.repeatCheckCall(req, oneStep, mCb)
					}
					else{
						lib.generateAndRunRequest(req, oneStep, mCb);
					}
				}, (error, response) => {
					if(error){
						return cb(error);
					...
					for(let f in response){
						context.template[section]._id[f] = response[f];
					}
					return cb(null, true);
				});
			}
			else if(typeof(context.template[section].wf.deploy) === 'object'){
				if(context.template[section].wf.deploy.recursive){
					lib.repeatCheckCall(req, context.template[section].wf.deploy, (error, response) => {
						if(error){
							return cb(error);
						}
					...
					lib.generateAndRunRequest(req, context.template[section].wf.deploy, (error, response) => {
						if(error){
							return cb(error);
						}
						if(!context.template[section]._id){
	...
	redirectTo3rdPartyStatus: function (req, context, section, cb) {
		if(context.template[section] && context.template[section].wf && context.template[section].wf.status){
			if(Array.isArray(context.template[section].wf.status)){
				async.mapSeries(context.template[section].wf.status, (oneStep, mCb) => {
					lib.generateAndRunRequest(req, oneStep, mCb);
				}, (error, response) => {
					if(error){
						return cb(error);
					...
					let checkResponse;
					if(context.template[section].wf.status.check){
						let valid = true;
						for(let field in context.template[section].wf.status.check){
							...
							else if(context.template[section].wf.status.check[field].value && Object.hasOwnProperty.call(response, field) !== context.template[section].wf.status.check[field].value){
								valid = false;
							}
						}
						if(valid){
				...
				lib.generateAndRunRequest(req, context.template[section].wf.status, (error, response) => {
					if(error){
						return cb(error);
					}
	...
	repeatCheckCall : function (req, oneStep, callback){
		lib.generateAndRunRequest(req, oneStep, function(error, response){
			...
			if(valid){
				...
				setTimeout(() => {
					lib.repeatCheckCall(req, oneStep, callback);
				}, 10000);
			}
		});
	},
	
	generateAndRunRequest: function(req, options, cb){
