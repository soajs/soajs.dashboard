...
var environmentHelper = {
	...
	prepareEnvRecord : function(config, data, envDbRecord, infraProvider){
		...
		//deployer
		if(!data.deploy || data.deploy.selectedDriver === "ondemand"){
		...
		else if(!data.deploy || data.deploy.selectedDriver === "manual"){
			record.deployer.type = "manual";
			record.deployer.selected = "manual";
			if(data.deploy) {
		...
		else if(['manual','ondemand'].indexOf(data.deploy.selectedDriver) === -1){
			...
			else if(infraProvider && infraProvider.name === 'local'){
