		...
		if(data.envType === "manual"){
			...
			if(data.deploy && data.deploy.previousEnvironment){
				...
				&& record.deployer.container.kubernetes.local.namespace){
					record.deployer.container.kubernetes.local.namespace.default = data.code.toLowerCase();
					record.deployer.container.kubernetes.remote.namespace.default = data.code.toLowerCase();
				}
				valid = true;
			}
			else {
