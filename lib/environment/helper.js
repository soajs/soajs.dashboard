...
var environmentHelper = {
	...
	prepareEnvRecord : function(config, data, envDbRecord, prefix, infraProvider){
		...
		if(data.deploy.selectedDriver === "manual"){
			...
			if (data.deploy.extras) {
				for (let i in data.deploy.extras) {
					let label = (i === 'groups') ? 'group' : i;
					record.restriction[data.deploy.selectedInfraProvider._id][data.deploy.region][label] = data.deploy.extras[i];
				}
			}
		}
