...
var environmentHelper = {
	...
	//todo: no more dockerremote or dockerlocal
	getDefaultRegistryServicesConfig : function(dashboardEnvRecord, data){
		...
		if(data.deploy && data.deploy.previousEnvironment){
			config.deployer = dashboardEnvRecord.deployer;
		}
		return config;
	},
	...
	prepareEnvRecord : function(config, data, dashboardEnvRecord){
		...
		//deployer
		if(!data.deploy || data.deploy  === {} ||(data.deploy && data.deploy.selectedDriver === "manual")){
			record.deployer.type = "manual";
			if(data.deploy) {
				if(data.deploy.previousEnvironment){
					record.deployer.manual.nodes = dashboardEnvRecord.deploy.deployment.manual.nodes;
				}
			...
			if(data.deploy.selectedDriver === 'docker'){
				if(data.deploy && data.deploy.previousEnvironment){
					record.deployer.selected = dashboardEnvRecord.deployer.selected;
					record.deployer.container = dashboardEnvRecord.deployer.container;
				}
				else {
					...
					if(data.deploy.deployment.docker.auth.token){
						record.deployer.container.docker.remote.auth = {
							token: data.deploy.deployment.docker.auth.token
						};
					}
				}
			}
