				...
				checkReturnError(req, cbMain, { config: config, error: error, code: 400 }, () => {
					checkReturnError(req, cbMain, { config: config, error: count > 0, code: 403 }, () => {
						...
						async.auto({
							...
							checkReturnError(req, cbMain, { config: config, error: error, code: error.code || 400 }, function () {
			...
			BL.model.findEntry(req.soajs, {collection:templatesColName, conditions: { code: req.soajs.inputmaskData.code } }, function(error, template) {
				checkReturnError(req, cbMain, { config: config, error: error, code: 600 }, function () {
					checkReturnError(req, cbMain, { config: config, error: !template, code: 600 }, function () {
						envDeployStatus.checkProgress(req, BL, config, environmentRecord, template, cbMain);
					});
				});
			});
		}
