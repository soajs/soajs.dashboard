...
var BL = {
	...
	"add": function (config, service, provision, dbModel, req, res, cbMain) {
						...
						checkReturnError(req, cbMain, { config: config, error: error, code: 400 }, function () {
							...
							BL.model.insertEntry(req.soajs, opts, function (err, data) {
								checkReturnError(req, cbMain, { config: config, error: err, code: 400 }, function () {
									let environmentId = data[0]._id;
									updateOwnerSettings(environmentId);
								});
							});
						});
					});
		...
		function updateOwnerSettings(environmentId){
			...
			BL.model.findEntry(req.soajs, opts, function (error, tenantRecord) {
				checkReturnError(req, cbMain, { config: config, error: error || !tenantRecord, code: 400 }, function () {
						...
						if (tenantRecord.applications[i].appId.toString() === appId.toString()) {
							for (var j = 0; j < tenantRecord.applications[i].keys.length; j++) {
								if (tenantRecord.applications[i].keys[j].key === key) {
									tenantRecord.applications[i].keys[j].config[req.soajs.inputmaskData.data.code.toLowerCase()] = tenantRecord.applications[i].keys[j].config[process.env.SOAJS_ENV.toLowerCase()];
									//todo: replace, protocol | port | sitePrefix | domain with portal info
									break;
								}
							}
						}
						...
						checkReturnError(req, cbMain, {
							...
							tenant.init(dbModel, function (error, tenantBL) {
								checkReturnError(req, cbMain, {
									...
									req.soajs.inputmaskData.env = req.soajs.inputmaskData.data.code;
									...
									// todo: check if error
									tenantBL.addApplicationExtKeys(config, provision, service.registry, req, res, function (error) {
										return cbMain(error, environmentId);
									});
								});
							});
						});
	...
	"delete": function (config, req, registry, deployer, cbMain) {
		var opts = {};
		validateId(req.soajs, function (err) {
			checkReturnError(req, cbMain, { config: config, error: err, code: 405 }, function () {
				opts.collection = colName;
		...
		function checkContainerDeployments(envRecord, cb) {
			registry.loadByEnv({ envCode: envRecord.code.toLowerCase() }, function(error, registry) {
				checkReturnError(req, cbMain, {config: config, error: error || !registry, code: 446}, function () {
					var options = utils.buildDeployerOptions(registry, req.soajs, BL);
	 ...
	 */
	"updateDeployerConfig": function (config, req, registry, deployer, cbMain) {
		var currentPlatform = ((registry.deployer && registry.deployer.selected) ? registry.deployer.selected.split('.')[1] : '');
		checkReturnError(req, cbMain, {
			config: config,
