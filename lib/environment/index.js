	...
	"delete": function (config, req, deployer, cbMain) {
		...
		function deleteEnv() {
			async.series({
				...
				"removeIncompleteVMTemplates": (vCb) => {
					opts = {};
					opts.collection = "vm_layers";
					opts.conditions = { 'env': envRecord.code.toLowerCase(), 'error': { $exists: true } };
					BL.model.removeEntry(req.soajs, opts, vCb);
				},
				"removeEnv": (vCb) => {
					opts = {};
					...
					serviceModule.listServices(config, req.soajs, deployer, (error, services) => {
						utils.checkErrorReturn(req.soajs, cbMain, { config: config, error: error }, () => {
							...
							if (req.soajs.inputmaskData.force) {
								...
								checkReturnError(req, cbMain, { config: config, error: services.length > 0, code: 906 }, () => {
