								...
								async.series({
									...
									"checkTemplate": function(mCb){
										if(req.soajs.inputmaskData.template){
											if(req.soajs.inputmaskData.template.gi && req.soajs.inputmaskData.template.gi.deploy){
												delete req.soajs.inputmaskData.template.gi.deploy;
											}
											delete req.soajs.inputmaskData.template.error;
											
											req.soajs.inputmaskData.template.code = req.soajs.inputmaskData.data.code.toUpperCase();
											req.soajs.inputmaskData.template.ready = true;
											...
											let overrideNginxPorts = false;
											if (process.env.SOAJS_SAAS && req.soajs.servicesConfig) {
												if (req.soajs.servicesConfig.HT_PROJECT && req.soajs.servicesConfig.HT_PROJECT.name) {
													if (!req.soajs.inputmaskData.soajs_project) {
														overrideNginxPorts = true;
													}
													if (req.soajs.servicesConfig.HT_PROJECT.name === req.soajs.inputmaskData.soajs_project) {
														overrideNginxPorts = true;
													}
												}
												
												if (!req.soajs.tenant.locked && req.soajs.servicesConfig.SOAJS_SAAS) {
													if (req.soajs.inputmaskData.soajs_project && req.soajs.servicesConfig.SOAJS_SAAS[req.soajs.inputmaskData.soajs_project]) {
														overrideNginxPorts = true;
														
														req.soajs.inputmaskData.template.gi.project = {
															name: req.soajs.inputmaskData.soajs_project
														}
													}
												}
												
												if (overrideNginxPorts){
													req.soajs.inputmaskData.template.nginx.http = 80;
													if (req.soajs.inputmaskData.template.nginx.https){
														req.soajs.inputmaskData.template.nginx.http = 443;
													}
												}
											}
											BL.model.removeEntry(req.soajs, {
												...
												BL.model.insertEntry(req.soajs, {
													collection: templatesColName,
													record: req.soajs.inputmaskData.template
												}, mCb);
											});
										}
										else{
			...
			if (process.env.SOAJS_SAAS && !req.soajs.tenant.locked && req.soajs.servicesConfig) {
				...
				if(serviceConfig && serviceConfig[req.soajs.inputmaskData.soajs_project]){
					...
					BL.model.countEntries(req.soajs, opts, (error, count) => {
						checkReturnError(req, cbMain, { config: config, error: count > 0, code: 400 }, function () {
		...
		function deleteEnv() {
			...
			BL.model.removeEntry(req.soajs, opts, function (error) {
				checkReturnError(req, cbMain, { config: config, error: error, code: 404 }, function () {
					BL.model.removeEntry(req.soajs, {collection: templatesColName, conditions: {code: envRecord.code.toUpperCase() }}, (error) => {
						checkReturnError(req, cbMain, { config: config, error: error, code: 404 }, function () {
	...
	"list": function (config, req, res, cbMain) {
		...
		BL.model.findEntries(req.soajs, opts, function (err, records) {
			checkReturnError(req, cbMain, { config: config, error: err, code: 402 }, function () {
				...
				//check for templates
				BL.model.findEntries(req.soajs, {collection: templatesColName, conditions: {"type": {$in: ['_BLANK','_SOAJS','_PORTAL']} } }, function (err, templates) {
					checkReturnError(req, cbMain, {config: config, error: err, code: 402}, function () {
						if(!templates || templates.length < 3){
							let templateSchemas = require("../../templates/environment/environments");
							async.each(templateSchemas, (oneTemplateSchema, fCb) => {
								BL.model.updateEntry(req.soajs, {collection: templatesColName, conditions: {"type": oneTemplateSchema.type}, fields: {$set: oneTemplateSchema}, options: {upsert: true, multi:false} }, fCb);
							}, (err) => {
								checkReturnError(req, cbMain, {config: config, error: err, code: 402}, function () {
