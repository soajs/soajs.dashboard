					...
					checkReturnError(req, cbMain, { config: config, error: count > 0, code: 403 }, () => {
						...
						async.auto({
							...
							"getInfraProvider": ['validateEnvironmentRestrictions', (info, mCb) => {
								...
								//case of vm only
								if(!req.soajs.inputmaskData.data.deploy.selectedInfraProvider._id && req.soajs.inputmaskData.data.deploy.selectedInfraProvider.infraId){
									req.soajs.inputmaskData.data.deploy.selectedInfraProvider._id = req.soajs.inputmaskData.data.deploy.selectedInfraProvider.infraId;
								}
						...
						utils.checkErrorReturn(req.soajs, cbToUse, { config: config, error: error }, () => {
							...
							if (imfvClone.force) {
								...
								async.series({
									...
									"deleteNamespace": (mCb) => {
										if (!imfvClone.namespace) {
											return mCb();
										}
										req.soajs.inputmaskData = soajsUtils.cloneObj(imfvClone);
										initBLModel(require(__dirname + "/../cloud/namespaces/index.js"), 'mongo', (error, namesModule) => {
											checkReturnError(req, mCb, {
												config: config,
												error: error,
												code: 600
											}, () => {
												req.soajs.inputmaskData.namespaceId = imfvClone.namespace;
												namesModule.delete(config, req.soajs, deployer, (error, secrets) => {
													checkReturnError(req, mCb, {
														config: config,
														error: error,
														code: 600
					...
					checkReturnError(req, cbMain, {
						...
						BL.model.findEntry(req.soajs, opts, function (error, environment) {
							checkReturnError(req, cbMain, { config: config, error: error, code: 405 }, function () {
								...
								environment.port = req.soajs.inputmaskData.port;
								environment.protocol = req.soajs.inputmaskData.protocol;
	...
	"list": function (config, req, res, cbMain) {
		...
		if (found){
			...
			if (req.soajs.urac && req.soajs.urac.groupsConfig){
				...
				if (req.soajs.urac.groupsConfig.allowedEnvironments){
