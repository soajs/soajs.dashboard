					...
					checkReturnError(req, cbMain, { config: config, error: count > 0, code: 403 }, () => {
						...
						async.auto({
							...
							"getInfraProvider": ['validateEnvironmentRestrictions', (info, mCb) => {
								...
								if(req.soajs.inputmaskData.data.deploy.selectedInfraProvider._id){
									req.soajs.inputmaskData.data.infraId = req.soajs.inputmaskData.data.deploy.selectedInfraProvider._id;
								}
								
								if (!req.soajs.inputmaskData.data.infraId && req.soajs.inputmaskData.data.deploy.selectedInfraProvider.infraId){
									req.soajs.inputmaskData.data.infraId = req.soajs.inputmaskData.data.deploy.selectedInfraProvider.infraId;
								}
