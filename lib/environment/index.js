...
var platformsHelper = require("./platform.js");
				...
				checkReturnError(req, cbMain, { config: config, error: error, code: 400 }, () => {
					checkReturnError(req, cbMain, { config: config, error: count > 0, code: 403 }, () => {
						...
						async.auto({
							...
							"getInfraProvider": (mCb) => {
								if(!req.soajs.inputmaskData.data.infraId){
									return mCb();
								}
								let opts = {
									collection: "infra",
									conditions: { "_id": new BL.model.getDb(req.soajs).ObjectId(req.soajs.inputmaskData.data.infraId) }
								};
								BL.model.findEntry(req.soajs, opts, (error, infraRecord) => {
									checkReturnError(req, mCb, { config: config, error: error, code: 400 }, () => {
										checkReturnError(req, mCb, { config: config, error: !infraRecord, code: 400 }, () => {
							...
							"getRequestedTemplate": (mCb) =>{
								...
								BL.model.findEntry(req.soajs, opts, (error, template) => {
									checkReturnError(req, mCb, { config: config, error: error, code: 400 }, () => {
										checkReturnError(req, mCb, { config: config, error: !template, code: 400 }, () => {
											if (template.reusable === false) {
                                                ...
                                                BL.model.countEntries(req.soajs, envOpts, (error, envCount) => {
                                                    checkReturnError(req, mCb, { config: config, error: error, code: 400 }, () => {
                                                        if (envCount > 0) {
                                                            let error = [];
                                                            error.push({code: 173, msg: `Template ${template.name}: Already used to deploy another environment`});
                                                            return mCb (error);
                                                        } else {
							...
							"prepareEnvRecord": ["checkPreviousEnvironment", "getInfraProvider", (info, mCb) => {
								...
								let record = envHelper.prepareEnvRecord(config, req.soajs.inputmaskData.data, envRecord, info.getInfraProvider);
				...
				checkReturnError(req, cbMain, { config: config, error: error, code: 600 }, function () {
					checkReturnError(req, cbMain, { config: config, error: !template, code: 600 }, function () {
						...
						if(template.infra){
							...
							BL.model.findEntry(req.soajs, {collection:'infra', conditions: { _id: infraId } }, function(error, selectedProvider) {
								checkReturnError(req, cbMain, { config: config, error: error, code: 600 }, function () {
									checkReturnError(req, cbMain, { config: config, error: !selectedProvider, code: 600 }, function () {
	...
	"delete": function (config, req, deployer, cbMain) {
			...
			async.series({
				...
				"cleanUpInfra": (vCb) =>{
					...
					req.soajs.inputmaskData = {
						envCode: envRecord.code.toUpperCase()
					};
		...
		function checkContainerDeployments(envRecord, cb) {
			...
			initBLModel(require(__dirname + "/../cloud/services/index.js"), 'mongo', (error, serviceModule) => {
				checkReturnError(req, cbToUse, { config: config, error: error, code: (error && error.code) ? error.code : 600 }, () => {
					req.soajs.inputmaskData.env = envRecord.code;
					let imfvClone = JSON.parse(JSON.stringify(req.soajs.inputmaskData));
					serviceModule.listServices(config, req.soajs, deployer, (error, services) => {
						utils.checkErrorReturn(req.soajs, cbToUse, { config: config, error: error }, () => {
							...
							if (imfvClone.force) {
								...
								async.series({
									"deleteServices": (mCb) => {
										async.eachSeries(services, (oneService, sCb) => {
											req.soajs.inputmaskData = JSON.parse(JSON.stringify(imfvClone));
								...
								//don't wait for async to finish, return the cb
								return cb(null, true);
							}
							else{
								checkReturnError(req, cbToUse, { config: config, error: services.length > 0, code: 906 }, () => {
		...
		validateId(req.soajs, function (err) {
			checkReturnError(req, cbMain, { config: config, error: err, code: 405 }, function () {
				checkCanEdit(req.soajs, function(errCode) {
					checkReturnError(req, cbMain, { config: config, error: config.errors[errCode], code: errCode}, function () {
						let opts = {};
						opts.collection = colName;
						opts.conditions = {
