...
var BL = {
	...
	"add": function (config, service, provision, dbModel, req, res, cbMain) {
		...
		BL.model.countEntries(req.soajs, opts, function (error, count) {
			checkReturnError(req, cbMain, { config: config, error: error, code: 400 }, function () {
				checkReturnError(req, cbMain, { config: config, error: count > 0, code: 403 }, function () {
					...
					if(req.soajs.inputmaskData.data.deploy.previousEnvironment){
						condition = {
						...
						checkReturnError(req, cbMain, { config: config, error: error, code: 400 }, function () {
							...
							BL.model.insertEntry(req.soajs, opts, function (err, data) {
								checkReturnError(req, cbMain, { config: config, error: err, code: 400 }, function () {
									...
									if(record.deployer.selected === 'container.docker.remote' && req.soajs.inputmaskData.data.deploy.previousEnvironment){
										//update certificates, push new environment
										attachCertificates(record, req.soajs.inputmaskData.data, data[0]._id);
									}
									else{
		...
		function attachCertificates(record, data, _id){
			let opts = {
				collection: "fs.files",
				conditions: {
				...
				checkReturnError(req, cbMain, {
					...
					checkIfEnvIsDeployed(envRecord, currentPlatform, function () {
						if(currentPlatform === 'docker'){
							envRecord.deployer.container[currentPlatform][req.soajs.inputmaskData.driver] = req.soajs.inputmaskData.config;
						}
						else{
							if(!Object.hasOwnProperty.call(req.soajs.inputmaskData.config, 'namespace')){
								...
								if(envRecord.deployer.container[currentPlatform][req.soajs.inputmaskData.driver].namespace.perService){
									envRecord.deployer.container[currentPlatform][req.soajs.inputmaskData.driver].namespace.perService = 'perService';
								}
								else{
									envRecord.deployer.container[currentPlatform][req.soajs.inputmaskData.driver].namespace.perService = 'perService';
								}
