...
var BL = {
	...
	"add": function (config, service, provision, dbModel, req, res, cbMain) {
		...
		BL.model.countEntries(req.soajs, opts, function (error, count) {
			checkReturnError(req, cbMain, { model: BL.model, config: config, error: error, code: 400 }, function () {
				checkReturnError(req, cbMain, { model: BL.model, config: config, error: count > 0, code: 403 }, function () {
					...
					BL.model.findEntry(req.soajs, opts, function (error, envRecord) {
						checkReturnError(req, cbMain, { model: BL.model, config: config, error: error, code: 400 }, function () {
							...
							if(req.soajs.inputmaskData.data.soajsFrmwrk){
								...
								record.services.config.session.secret = req.soajs.inputmaskData.data.secret;
							}
							else{
							...
							BL.model.insertEntry(req.soajs, opts, function (err, data) {
								checkReturnError(req, cbMain, { model: BL.model, config: config, error: err, code: 400 }, function () {
	...
	"get": function (config, req, res, cbMain) {
		...
		if (req.soajs.inputmaskData.code) {
			...
			validateId(req.soajs, function (err) {
				checkReturnError(req, cbMain, { model: BL.model, config: config, error: err, code: 405 }, function () {
		...
		function checkManualDeployments(envRecord, cb) {
			...
			BL.model.countEntries(req.soajs, opts, function (error, count) {
				checkReturnError(req, cbMain, {
					...
					checkReturnError(req, cbMain, { model: BL.model, config: config, error: count > 0, code: 906 }, cb);
				});
			});
		}
	...
	"list": function (config, req, res, cbMain) {
		...
		BL.model.findEntries(req.soajs, opts, function (err, records) {
			checkReturnError(req, cbMain, { model: BL.model, config: config, error: err, code: 402 }, function () {
	...
	"keyUpdate": function (config, provision, req, res, cbMain) {
		...
		checkCanEdit(req.soajs, function (error) {
			checkReturnError(req, cbMain, { model: BL.model, config: config, error: error, code: error }, function () {
				validateId(req.soajs, function (err) {
					checkReturnError(req, cbMain, {
						model: BL.model,
						config: config,
	...
	"removeCert": function (config, req, res, cbMain) {
		...
		validateId(req.soajs, function (error) {
			checkReturnError(req, cbMain, { model: BL.model, config: config, error: error, code: 701 }, function () {
	...
	"chooseExistingCerts": function (config, req, res, cbMain) {
		...
		opts.conditions = { code: req.soajs.inputmaskData.env };
		BL.model.countEntries(req.soajs, opts, function (error, count) {
			checkReturnError(req, cbMain, { model: BL.model, config: config, error: error, code: 600 }, function () {
				checkReturnError(req, cbMain, {
					model: BL.model,
					config: config,
					error: count === 0,
	...
	"changeSelectedDriver": function (config, req, res, cbMain) {
		...
		BL.model.updateEntry(req.soajs, opts, function (error, result) {
			checkReturnError(req, cbMain, { model: BL.model, config: config, error: error, code: 735 }, function () {
				return cbMain(null, true);
			});
		});
	},
