...
var BL = {
			...
			BL.model.countEntries(req.soajs, opts, function (error, count) {
				checkReturnError(req, cbMain, { config: config, error: error, code: 400 }, () => {
					checkReturnError(req, cbMain, { config: config, error: count > 0, code: 403 }, () => {
						...
						async.auto({
							"validateEnvironmentRestrictions": (mCb) => {
								restrictions.validateEnvironmentRestrictions(req, (error) => {
	...
	"unlockEnvToCloudProvider": function(config, req, cbMain){
		cloudProviderHelper.unlockEnvToCloudProvider(config, req, BL, cbMain);
	}
};
