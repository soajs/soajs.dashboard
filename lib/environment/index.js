		...
		BL.model.countEntries(req.soajs, opts, function (error, count) {
			checkReturnError(req, cbMain, { config: config, error: error, code: 400 }, function () {
				checkReturnError(req, cbMain, { config: config, error: count > 0, code: 403 }, function () {
					...
					let myEnvCode = process.env.SOAJS_ENV.toUpperCase();
					let condition = { 'code': myEnvCode };
					if(req.soajs.inputmaskData.data.deploy.previousEnvironment){
						myEnvCode = req.soajs.inputmaskData.data.deploy.previousEnvironment.toUpperCase();
						condition = {
							'code': myEnvCode,
