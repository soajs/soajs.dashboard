						...
						async.auto({
							...
							"getInfraProvider": (mCb) => {
								...
								BL.model.findEntry(req.soajs, opts, (error, infraRecord) => {
									checkReturnError(req, cbMain, { config: config, error: error, code: 400 }, () => {
										checkReturnError(req, cbMain, { config: config, error: !infraRecord, code: 400 }, () => {
				...
				"cleanUpInfra": (vCb) =>{
					...
					initBLModel(require("../cloud/infra/index.js"), "mongo", (error, infraModule) => {
						checkReturnError(req, vCb, { config: config, error: error, code: 600 }, function () {
							infraModule.removeEnvFromDeployment(config, req.soajs, deployer, (error) =>{
								if(error){
				...
				utils.checkErrorReturn(req.soajs, cbMain, { config: config, error: error }, function () {
					checkReturnError(req, cbMain, { config: config, error: services.length > 0, code: 907 }, () =>{
						if(envRecord.deployer.container[currentPlatform][req.soajs.inputmaskData.driver].namespace.default !== req.soajs.inputmaskData.config.namespace.default){
							...
							deployer.execute({'type': 'container', 'driver': options.strategy}, 'createNameSpace', options, (error) =>{
