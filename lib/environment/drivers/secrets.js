...
const secrets = {
	validate: function(req, context, lib, async, BL, modelName, callback){
		...
		if(!context.opts.input || !Array.isArray(context.opts.input) || context.template.content.custom_registry.data.length !== context.opts.input.length){
		...
		if(context.environmentRecord.deployer.selected.indexOf("kubernetes") !== -1){
