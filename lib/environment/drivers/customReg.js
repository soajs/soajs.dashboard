...
const customReg = {
	validate: function (req, context, lib, async, BL, modelName, callback) {
		...
		if (!context.template.content.custom_registry
			|| !context.template.content.custom_registry.data
			|| !Array.isArray(context.template.content.custom_registry.data)
			|| context.template.content.custom_registry.data.length === 0) {
		...
		if (!context.opts.inputs || !Array.isArray(context.opts.inputs) || context.template.content.custom_registry.data.length !== context.opts.inputs.length) {
			context.errors.push({
				code: 172,
				msg: `Mismatch between the number of inputs provided and the template entries in custom registries!`
			});
			return callback();
		}
			...
			if (!dataImfv || typeof dataImfv !== 'object') {
				...
				if (!status.valid) {
					status.errors.forEach(function (err) {
						context.errors.push({code: 173, msg: `Custom Registry ${oneEntry}: ` + err.stack});
					});
				}
			}
			return fCb();
