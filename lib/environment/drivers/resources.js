                    ...
                    lib.initBLModel(BL.resources.module, modelName, (error, cdModule) => {
                        lib.initBLModel(BL.cloud.services.module, modelName, function (serviceBL) {
                            ...
                            if (dataImfv.deploy && Object.keys(dataImfv.deploy).length > 0) {
                                ...
                                if(dataImfv.deploy.options.deployConfig && dataImfv.deploy.options.deployConfig.vmConfiguration && dataImfv.deploy.options.deployConfig.vmConfiguration.securityGroups){
	                                for( let i =0; i < dataImfv.deploy.options.deployConfig.vmConfiguration.securityGroups.length; i++){
	                                	if(dataImfv.deploy.options.deployConfig.vmConfiguration.securityGroups[i].v && dataImfv.deploy.options.deployConfig.vmConfiguration.securityGroups[i].selected){
			                                dataImfv.deploy.options.deployConfig.vmConfiguration.securityGroups[i] = dataImfv.deploy.options.deployConfig.vmConfiguration.securityGroups[i].v;
		                                }
	                                }
                                }
	
	                            if(req.soajs.inputmaskData["config"] && req.soajs.inputmaskData["config"].options && req.soajs.inputmaskData["config"].options.deployConfig && req.soajs.inputmaskData["config"].options.deployConfig.vmConfiguration && req.soajs.inputmaskData["config"].options.deployConfig.vmConfiguration.securityGroups){
		                            for( let i =0; i < req.soajs.inputmaskData["config"].options.deployConfig.vmConfiguration.securityGroups.length; i++){
			                            if(dataImfv.deploy.options.deployConfig.vmConfiguration.securityGroups[i].v && req.soajs.inputmaskData["config"].options.deployConfig.vmConfiguration.securityGroups[i].selected){
				                            req.soajs.inputmaskData["config"].options.deployConfig.vmConfiguration.securityGroups[i] = req.soajs.inputmaskData["config"].options.deployConfig.vmConfiguration.securityGroups[i].v;
			                            }
		                            }
	                            }
                            ...
                            //set by the wizard to alter the default flow of the api callback
	                        req.soajs.inputmaskData.wizard = true;
                            req.soajs.log.debug("resource driver in wizard:", req.soajs.inputmaskData);
