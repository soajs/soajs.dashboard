...
const repos = {
	...
	validate: function(req, context, lib, async, BL, modelName, callback){
		...
		if (!context.template.content.deployments || !context.template.content.deployments.resources || !context.template.content.deployments.resources[resourceSection]) {
			//this deployment entry is not found in the template content
			context.errors.push({code: 172, msg: `The template does not support deploying resources for ${resourceSection}`});
			return callback();
		}
		...
		if(!context.opts.input || !Array.isArray(context.opts.input) || context.opts.input.length !== context.template.content.deployments.resources[resourceSection].limit){
			context.errors.push({code: 172, msg: `Mismatch between the number of inputs provided and the template entries in resources ${resourceSection}!`});
			return callback();
		}
