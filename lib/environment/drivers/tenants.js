	...
	tenantsModule.createApplicationKey(context.config, provision, req, {}, (error, appInfo) =>{
		lib.checkReturnError(req, callback, {error: error, code: (error && error.code) ? error.code: 600}, () => {
			...
			addApplicationConfig(req, context, tenantsModule, tenantRecord, appInfo, oneKeyInput, (error) => {
				lib.checkReturnError(req, exCb, {error: error, code: (error && error.code) ? error.code: 600}, () => {
		...
		if(domain && context.template && context.template.deploy && context.template.deploy.deployments){
			...
			for(let group in context.template.deploy.deployments){
				for(let step in context.template.deploy.deployments[group]){
					if(step.indexOf("deployments.resources") !== -1 && step.imfv){
						let input = step.imfv[0];
						
						//if nginx resource
						if(input && input.type ==='server' && input.category === 'nginx' && input.deploy){
							if(input.deploy.custom){
								
								//check if https is turned on or not
								if(input.deploy.custom.secrets && input.deploy.custom.secrets.length > 0){
									input.deploy.custom.secrets.forEach((oneSecret) => {
								...
								if(input.deploy.custom.ports && input.deploy.custom.ports.length > 0){
									input.deploy.custom.ports.forEach((onePort) => {
