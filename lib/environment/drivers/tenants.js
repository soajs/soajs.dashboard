...
function updateOauth(req, context, oneTenantInput, tenantRecord, tenantsModule, callback){
	if(oneTenantInput.oauth && Object.keys(oneTenantInput.oauth).length > 0){
		...
		if(oneTenantInput.oauth.oauthType){
			req.soajs.inputmaskData.oauthType = oneTenantInput.oauth.oauthType;
		}
...
function addApplication(req, context, async, lib, oneApplicationInput, tenantRecord, tenantsModule, callback){
	...
	let packageCode = (oneApplicationInput.package.indexOf("_") !== -1) ? oneApplicationInput.package.split("_")[1] : oneApplicationInput.package;
	req.soajs.inputmaskData = {
		id: tenantRecord._id.toString(),
		productCode: oneApplicationInput.product,
		packageCode: packageCode,
		description: oneApplicationInput.description,
		_TTL: calculateTTL(oneApplicationInput._TTL)
	};
...
function addKey(req, context, async, lib, tenantsModule, tenantRecord, appInfo, oneKeyInput, callback){
	...
	if(oneKeyInput.key){
		req.soajs.inputmaskData.internalKey = oneKeyInput.key;
	}
					...
					tenantsModule.get(context.config, req, {}, (error, tenantRecord) =>{
						lib.checkReturnError(req, fCb, {error: error, code: (error && error.code) ? error.code: 600}, () => {
							...
							if(tenantRecord){
								...
								updateOauth(req, context, oneTenantInput, tenantRecord, tenantsModule, (error) => {
									lib.checkReturnError(req, fCb, {error: error, code: error && error.code ? error.code : 600}, () => {
										...
										async.eachSeries(applications, (oneApplicationInput, vCb) => {
											let appFound;
											let myPackage = oneApplicationInput.package;
											if(myPackage.indexOf(oneApplicationInput.product) == -1){
												myPackage = oneApplicationInput.product + "_" + oneApplicationInput.package;
											}
											
											for(let i = tenantRecord.applications.length -1; i >= 0; i--) {
												if (tenantRecord.applications[i].package === myPackage) {
								...
								tenantsModule.add(context.config, req, {}, (error, tenant) => {
									lib.checkReturnError(req, fCb, {error: error, code: (error && error.code) ? error.code: 600}, () => {
										req.soajs.inputmaskData = {};
										req.soajs.inputmaskData.id = tenant.id;
										tenantsModule.get(context.config, req, {}, (error, tenantRecord) =>{
											lib.checkReturnError(req, fCb, {error: error, code: (error && error.code) ? error.code: 600}, () => {
									...
									if (tenantRecord) {
										...
										for (let i = tenantRecord.applications.length - 1; i >= 0; i--) {
											oneTenantInput.applications.forEach((oneApplicationInput) => {
												if (tenantRecord.applications[i] && tenantRecord.applications[i].package === oneApplicationInput.package) {
										...
										if (tenantRecord.applications.length === 0) {
											tenantsModule.model.removeEntry(req.soajs, {
												collection: 'tenants',
												conditions: {
										...
										//update tenant record in database
											...
											tenantsModule.model.saveEntry(req.soajs, {
												collection: 'tenants',
												record: tenantRecord
											}, (error) => {
												if (error) {
