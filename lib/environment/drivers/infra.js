...
const infra = {
	validate: function (req, context, lib, async, BL, modelName, callback) {
		...
		let myValidator = new req.soajs.validator.Validator();
		async.mapSeries(context.opts.input, (oneInfraInput, fCb) => {
	...
	deploy: function(req, context, lib, async, BL, modelName, callback){
		...
		let remoteStack = [];
		if (Array.isArray(context.opts.input)) {
			remoteStack = context.opts.input;
		}
		else{
			remoteStack.push(context.opts.input);
		}
