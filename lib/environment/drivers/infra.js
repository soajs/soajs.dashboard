...
function generateAndRunRequest(req, context, infraModule, oneEntry, cb) {
	...
	function makeTheCall() {
		...
		let soajs_project_imfv = req.soajs.inputmaskData.soajs_project;
		req.soajs.inputmaskData = entryOptions || {};
		if (['onboardVM'].indexOf(oneEntry.command) !== -1) {
			// req.soajs.inputmaskData.infraId = context.infraProvider._id.toString(); //infraProviderId
		}
