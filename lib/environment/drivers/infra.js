				...
				async.mapSeries(remoteStack, (oneStep, mCb) => {
					...
					lib.checkReturnError(req, callback, {error: error, code: 600}, () => {
						...
						if (infraInfo.command === 'deployVM'){
							console.log(JSON.parse(JSON.stringify(finalResponse, null, 2)));
							//clean up the final Response
							delete outputResponse[1].inputs;
							delete outputResponse[1].vms;
							
							for(let stage in context.template.deploy.deployments){
								for(let group in context.template.deploy.deployments[stage]){
									for(let step in context.template.deploy.deployments[stage][group]){
										if(step.includes("deployments.resources")){
											let oneStep = context.template.deploy.deployments[stage][group][step];
											if(oneStep.imfv[0].deployOptions && oneStep.imfv[0].deployOptions.vmConfiguration){
												if(oneStep.imfv[0].deployOptions.infra === finalResponse[0].infraId){
													if(oneStep.imfv[0].deployOptions.vmConfiguration.vmLayer && oneStep.imfv[0].deployOptions.vmConfiguration.vmLayer === finalResponse[0].name){
														...
														oneStep.imfv[0].deployOptions.vmConfiguration.group = finalResponse[1].inputs.group;
														...
														context.template.deploy.deployments[stage][group][step] = oneStep;
													}
												}
											}
										}
