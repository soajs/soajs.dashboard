...
function  generateAndRunRequest(req, context, infraModule, oneEntry, cb) {
	...
	if(['deployVM', 'getDeployVMStatus'].indexOf(oneEntry.command) !== -1 && entryOptions && (!existingProvider || (existingProvider._id.toString() !== entryOptions.infraId))){
	...
	function makeTheCall(){
		...
		infraModule[oneEntry.command](context.config, req, req.soajs, deployer, (error, response) => {
			...
			if(['deployVM', 'getDeployVMStatus'].indexOf(oneEntry.command) !== -1 && !error && response && response.id){
