			...
			lib.checkReturnError(req, callback, {error: error, code: 600}, () => {
				...
				async.mapSeries(remoteStack, (oneStep, mCb) => {
					...
					lib.checkReturnError(req, callback, {error: error, code: 600}, () => {
						...
						console.log(infraInfo);
						
						if (infraInfo.command === 'deployVM'){
							console.log(infraInfo.command);
							// if(context.template.deploy.deployments){
							// 	for(let stage in context.template.deploy.deployments){
							// 		if()
							// 	}
							// }
							// context.template.deploy[context.opts.stage][context.opts.group][context.opts.stepPath].imfv.deploy.options.deployConfig = {
							//  region: finalResponse[1].inputs.region,
							//  infra: finalResponse[1].infraId,
							//  type: 'vm',
							//  vmConfiguration: {
							//      vmLayer: finalResponse[1].inputs.vmLayer,
							//      group: finalResponse[1].inputs.group
							//  }
							// };
						}
						console.log("---------")
						console.log(JSON.stringify(context.template, null, 2))
						console.log("---------")
						console.log(JSON.stringify(finalResponse, null, 2))
						//todo modify incase of create vm layer
						//(infraInfo.command === 'deployVM')
						//look inside context.template.deploy for resources that use this vm layer
						//update their imfv and inject the vm instances array
						//so that when the next driver tries to deploy these resources in this vm layer
						//---> we don't change a thing in that driver
