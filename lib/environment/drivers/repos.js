...
const repos = {
	...
	validate: function(req, context, lib, async, BL, modelName, callback){
		...
		if(!context.opts.input || !Array.isArray(context.opts.input) || context.opts.input.length !== context.template.content.deployments.repo[repoSection].limit){
			context.errors.push({code: 172, msg: `Mismatch between the number of inputs provided and the template entries in repos ${repoSection}!`});
			return callback();
		}
