...
module.exports = {
    ...
    'logout': function (mongo, config, req, res) {
        validateId(mongo, req, function (error) {
            checkIfError(req, res, {config: config, error: error, code: 600}, function () {
                mongo.findOne(collName, {_id: req.soajs.inputmaskData.id}, {authId: 1, _id: 0}, function (error, authInfo) {
                    checkIfError(req, res, {config: config, error: error || !authInfo, code: 600}, function () {
    ...
    'listAccounts': function (mongo, config, req, res) {
        mongo.find(collName, {}, {token: 0}, function (error, accounts) {
            checkIfError(req, res, {config: config, error: error, code: 600}, function () {
    ...
    'getRepos': function (mongo, config, req, res) {
        ...
        function addReposStatus (allRepos, activeRepos, cb) {
            ...
            //todo: if repo was not found remotely, add it to response and mark it as deleted
    ...
    'activateRepo': function (mongo, config, req, res) {
        ...
        function addNewStaticContent (staticContentInfo) {
    ...
    'deactivateRepo': function (mongo, config, req, res) {
        ...
        function removeServiceOrDaemon (type, cb) {
            ...
            mongo.findOne(coll, {'src.repo': req.soajs.inputmaskData.repo}, {name: 1, _id: 0}, function (error, record) {
                if (error || !record) {
                ...
                mongo.count(hostsColl, {name: record.name}, function (error, count) {
                    ...
                    mongo.remove(coll, {'src.repo': req.soajs.inputmaskData.repo}, function (error, result) {
                        if (error || !result) {
                            return cb (error || !result);
                        }
        ...
        function removeMulti (cb) {
            ...
            function removeService (callback) {
                mongo.remove(servicesColl, {'src.repo': req.soajs.inputmaskData.repo}, function (error) {
                    return callback (error);
                });
            }

            function removeDaemon (callback) {
                mongo.remove(daemonsColl, {'src.repo': req.soajs.inputmaskData.repo}, function (error) {
                    return callback (error);
                });
            }
            ...
            function getRepoServices (callback) {
                mongo.find(servicesColl, {'src.repo': req.soajs.inputmaskData.repo}, {name: 1, _id: 0}, function (error, records) {
                    return callback (error, records);
                });
            }

            function getRepoDaemons (callback) {
                mongo.find(daemonsColl, {'src.repo': req.soajs.inputmaskData.repo}, {name: 1, _id: 0}, function (error, records) {
                    return callback (error, records);
                });
            }
        }
