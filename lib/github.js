...
function assurePath (pathTo, path) {
    if (pathTo === 'config' && path === 'config.js') {
        return '/' + path;
    }
...
module.exports = {
    ...
    'activateRepo': function (mongo, config, req, res) {
        ...
        function addNewStaticContent (staticContentInfo, cb) {
            mongo.count(staticContentColl, {name: staticContentInfo.name, 'src.owner': req.soajs.inputmaskData.owner, 'src.repo': req.soajs.inputmaskData.repo}, function (error, count) {
                if (error) {
                    return cb (error);
                }
    ...
    'syncRepo': function (mongo, config, req, res) {
                                        ...
                                        checkIfError(req, res, {config: config, error: error, code: (error && error.code) || 768}, function () {
                                            ...
                                            syncRepo(result.type, result.info, result.sha, function (error, newSHA) {
                                                checkIfError(req, res, {config: config, error: error, code: 768}, function () {
                                                    var repoLabel = req.soajs.inputmaskData.user + '/' + req.soajs.inputmaskData.repo;
                                                    mongo.update(collName, {_id: req.soajs.inputmaskData.id, 'repos.name': repoLabel}, {'$set': {'repos.$.configSHA': newSHA.sha}}, function (error, result) {
                                                        checkIfError(req, res, {config: config, error: error || !result, code: 600}, function () {
                                                            return res.jsonp(req.soajs.buildResponse(null, true));
                                                        });
                                                    });
        ...
        function syncMultiRepo(paths, token, cb) {
            ...
            mongo.findOne(collName, {_id: req.soajs.inputmaskData.id, 'repos.name': repoLabel}, {'repos.$': 1}, function (error, record) {
                ...
                async.map(paths, function (path, callback) {
                    var tempPath = assurePath('config', path);
                    tempPath = tempPath.slice(1, tempPath.length);
                    if (tempPath !== '/config.js') { //excluding root config file
                        var sha;
                        var found = false;
                        for (var i = 0; i < configSHA.length; i++) {
                            if (tempPath === configSHA[i].path) {
                                sha = configSHA[i].sha;
                                found = true;
                            }
                        }
                        ...
                        if (!found) {
                            req.soajs.log.error('Repository is out of sync. Failed to sync: ' + tempPath);
                            return callback ('outOfSync');
                        }

                        getConfigFile(token, sha, path, {multi: true}, callback);
                    }
