...
function getAllRepos (info, cb) {
	...
	if (info && info.token) {
		...
		if (info.type === 'personal') {
			options = {
				url: 'https://api.github.com/users/' + info.owner + '/repos',
				headers: {
					'user-agent': 'SOAJS GitHub App'
				},
				qs: {
			...
			options = {
				url: 'https://api.github.com/orgs/' + info.owner + '/repos',
				headers: {
					'user-agent': 'SOAJS GitHub App'
				},
				qs: {
...
module.exports = {
				...
				mongo.findOne(collName, {_id: req.soajs.inputmaskData.id}, function (error, accountRecord) {
					checkIfError(req, res, {config: config, error: error || !accountRecord, code: 757}, function () {
						if (accountRecord.token) { //private account with token
							getAllRepos({token: accountRecord.token}, function (error, response) {
							...
							getAllRepos({owner: accountRecord.owner, type: accountRecord.type}, function (error, response) {
								checkIfError(req, res, {config: config, error: error, code: 758}, function () {
									addReposStatus(response, accountRecord.repos, function (repos) {
										return res.jsonp(req.soajs.buildResponse(null, repos));
									});
	...
	'activateRepo': function (mongo, config, req, res) {
		...
		function validateFileContents(repoConfig, cb) {
			...
			else if (repoConfig.type && repoConfig.type === 'static' && !repoConfig.name) {
				return cb({'message': 'Missing field name for static repo in config.js'});
			}
			else {
				// TODO: check
				//req.soajs.log.error('Invalid or no type provided in config');
		...
		function validateFileContents(repoConfig, cb) {
			if (repoConfig.type && repoConfig.type === 'multi') {
				...
				if (!check.valid) {
					...
					return cb({'code': 769, "message": new Error(errMsgs.join(" - ")).message});
				}
			}
			else if (repoConfig.type && repoConfig.type === 'static' && !repoConfig.name) {
				return cb({'code': 770, 'message': 'Missing field name for static repo in config.js'});
			}
			else {
				// TODO: check
