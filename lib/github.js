...
function assurePath(pathTo, path) {
	if (pathTo === 'config' && path.indexOf(config.gitAccounts.github.defaultConfigFilePath) !== -1) {
		if (path.indexOf('/') === 0) {
			return path;
		}
		else {
	...
	if (pathTo === 'main' && path.indexOf(config.gitAccounts.github.defaultConfigFilePath) !== -1) {
		path = path.substring(0, path.indexOf(config.gitAccounts.github.defaultConfigFilePath));
	}
	...
	if (pathTo === 'main') {
		...
		path = path + config.gitAccounts.github.defaultConfigFilePath;
	}
...
function getAllRepos(req, info, cb) {
	var options = {
		url: '',
		headers: {
			'user-agent': config.gitAccounts.github.userAgent,
	...
	if (info && info.token) {
		options.url = config.gitAccounts.github.urls.getReposAuthUser;
		options.headers.Authorization = 'token ' + info.token;
		// options.qs.affiliation = 'owner, collaborator';

		request.get(options, function (error, response, body) {
			return cb(error, (body) ? JSON.parse(body) : []);
		});
	} else if (info && info.owner && info.type) {
		if (info.type === 'personal') {
			options.url = config.gitAccounts.github.urls.getReposNonAuthUser.replace('%OWNER%', info.owner);
			options.qs.type = 'all';
			...
			options.url = config.gitAccounts.github.urls.getReposPublicOrg.replace('%OWNER%', info.owner);
			options.qs.type = 'all';
				...
				mongo.findOne(collName, {_id: req.soajs.inputmaskData.id}, function (error, accountRecord) {
					checkIfError(req, res, {config: config, error: error || !accountRecord, code: 757}, function () {
						if (accountRecord.token) { //private account with token
							getAllRepos(req, {token: accountRecord.token}, function (error, response) {
			...
			checkIfError(req, res, {config: config, error: error, code: 701}, function () {
				getAuthToken(mongo, req.soajs.inputmaskData.id, function (error, token) {
					checkIfError(req, res, {config: config, error: error, code: 755}, function () {
						getConfigFile(token, config.gitAccounts.github.defaultConfigFilePath, null, function (error, repoInfo) {
							checkIfError(req, res, {
								config: config,
	...
	'syncRepo': function (mongo, config, req, res) {
			...
			checkIfError(req, res, {config: config, error: error, code: 701}, function () {
				mongo.findOne(collName, {_id: req.soajs.inputmaskData.id}, function (error, accountRecord) {
					checkIfError(req, res, {config: config, error: error, code: 767}, function () {
						...
						getConfigFile(accountRecord.token, configSHA, config.gitAccounts.github.defaultConfigFilePath, null, function (error, result) {
		...
		function comparePaths(remote, local, callback) {
			...
			local.forEach(function (onePath) {
				if (onePath.path !== config.gitAccounts.github.defaultConfigFilePath) { //excluding root config.js file
