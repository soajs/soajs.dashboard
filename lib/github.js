...
module.exports = {
    ...
    'syncRepo': function (mongo, config, req, res) {
            ...
            checkIfError(req, res, {config: config, error: error, code: 600}, function () {
                mongo.findOne(collName, {_id: req.soajs.inputmaskData.id}, function (error, accountRecord) {
                    checkIfError(req, res, {config: config, error: error, code: 600}, function () {
                        ...
                        getConfigFile(accountRecord.token, configSHA, 'config.js', function (error, repoInfo) {
                            checkIfError(req, res, {config: config, error: error, code: 600}, function () {
                                clearConfigDir(function (error) {
                                    checkIfError(req, res, {config: config, error: error, code: 600}, function () {
                                        return res.jsonp(req.soajs.buildResponse(null, true));
        ...
        function getConfigFile (token, configSHA, path, cb) {//todo: pass configSHA here and compare
            if (token) {
                github.authenticate({type: 'oauth', token: token});
            }
            github.repos.getContent({
                ...
                // if (configSHA === response.sha) {//todo: not applicable in case of multi repo
                //     return cb (null, true);
                // }
        ...
        function analyzeConfigFile (path, token, cb) {
            ...
            validateFileContents(repoConfig, function (error) {
                ...
                if (repoConfig.type === 'service') {
                    ...
                    syncRepo(repoConfig.type, info, cb);
                } else if (repoConfig.type === 'daemon') {
                    info = {
                        name: repoConfig.serviceName,
                        port: repoConfig.servicePort,
        ...
        function syncRepo (type, info, cb) {
            var coll;
            if (type === 'service') {
                coll = servicesColl;
            } else if (type === 'daemon') {
            ...
            mongo.count(coll, criteria, function (error, count) {
                ...
                if (count === 1) {
                    ...
                    mongo.update(coll, criteria, {'$set': info}, function (error) {
                        ...
                        //todo: update github account record, set new configSHA
                        return cb (null, true);
                    });
                } else {
                    mongo.insert(coll, info, function (error, result) {
                        if (error || !result) {
                            return cb (error || !result);
                        }

                        var repoLabel = req.soajs.inputmaskData.user + "/" + req.soajs.inputmaskData.repo;
                        findAndFlag(repoLabel, cb); //search for old service/daemon/staticContent and flag them as deleted//todo: check if extra params are needed
                    });
                }
            });
        }

        function syncStaticContentRepo (info, cb) {
            mongo.count(staticContentColl, {name: info.name, 'src.repo': req.soajs.inputmaskData.repo, 'src.owner': req.soajs.inputmaskData.user}, function (error, count) {
                if (error) {
                    return cb (error);
                }
                ...
                if (count === 1) {
                    delete info.name;
                    mongo.update(staticContentColl, criteria, {'$set': info}, function (error) {
                        if (error) {
                            return cb (error);
                        }
                    ...
                    //insert, findAndFlag
                    mongo.insert(staticContentColl, info, function (error, result) {
                        if (error || !result) {
                            return cb (error || !result);
                        }
        ...
        function syncMultiRepo (paths, token, cb) {
            //get config files SHAs and call async parallel to sync
            mongo.findOne(collName, {_id: req.soajs.inputmaskData.id}, function (error, record) {
                if (error) {
                    return cb (error);
                }
                ...
                for (var i = 0; i < record.repos.length; i++) {
                    if (record.repos[i].name === repoLabel) {
                        configData = record.repos[i].configSHA;
                        break;
                    }
                }
                //from record get the list of configSHAs, then call getConfigFile for each item and specify its SHA from list
                async.each(paths, function (onePath, callback) {
                    var sha;
                    for (var i = 0; i < configData.length; i++) {
                        if (configData[i].path === onePath) {
                            sha = configData[i].sha;
                            break;
                        }
                    }//todo: what happens if path has been removed
                    getConfigFile(token, sha, onePath, callback);
                }, cb);
            });
        }

        function findAndFlag (repoLabel, cb) {
