...
var BL = {
	...
	"list": function (config, req, cb) {
		...
		if (Object.hasOwnProperty.call(req.soajs.inputmaskData, 'specifyGit')) {
			if (req.soajs.inputmaskData.specifyGit) {
				opts.conditions = {
					"recipe.deployOptions.specifyGitConfiguration": true
				};
			}
			else {
				opts.conditions = {
					"$or": [
						{
							"recipe.deployOptions.specifyGitConfiguration": false
						},
						{
							"recipe.deployOptions.specifyGitConfiguration": {
								"$exists": false
							}
						}
					]
	...
	"upgrade": function(config, req, cb){
			...
			"catalogs": (mCb) =>{
				...
				BL.model.findEntries(req.soajs, opts, (error, records) => {
					checkIfError(req, mCb, {config: config, error: error, code: 600}, () => {
						async.map(records, (oneRecord, callback) => {
							migrateOneRecord(opts, oneRecord, callback);
						}, mCb);
					});
				});
			},
		...
		function migrateOneRecord(opts, oneRecord, callback){
			...
			BL.model.saveEntry(req.soajs, opts, true, callback);
		}
	}
