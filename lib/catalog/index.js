	...
	"add": function (config, req, cb) {
		...
		async.series({
			...
			"checkPorts": function (mCb) {
				if (req.soajs.inputmaskData.catalog && req.soajs.inputmaskData.catalog.restriction && req.soajs.inputmaskData.catalog.restriction.deployment && req.soajs.inputmaskData.catalog.restriction.deployment[0] && req.soajs.inputmaskData.catalog.restriction.deployment[0] === 'vm') {
					return mCb();
				}
