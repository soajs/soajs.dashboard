...
var BL = {
	...
	"add": function (config, req, cb) {
		...
		async.series({
			...
			checkIfError(req, cb, {config: config, error: error, code: error.code || 701}, function () {
	...
	"upgrade": function (config, req, cb) {
		...
		function migrateOneRecord(opts, oneRecord, versioning, callback) {
			...
			//upgrade volumes
			if(oneRecord.recipe.deployOptions.voluming.volumes){
				let voluming = [];
				if(oneRecord.recipe.deployOptions.voluming.volumeMounts){
					//kubernetes
					for(let i =0; i < oneRecord.recipe.deployOptions.voluming.volumes.length -1; i++){
