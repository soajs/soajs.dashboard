...
function validateSourceCodeAttachment(config, catalog, cb){
	if (catalog.sourceCode) {
		if (catalog.sourceCode.configuration) {
			...
			if (catalog.sourceCode.configuration.repo && !catalog.sourceCode.configuration.branch) {
		...
		if (catalog.sourceCode.custom) {
			...
			if (catalog.sourceCode.custom.repo && !catalog.sourceCode.custom.branch) {
				// if repository is selected, a branch must be selected too
				return cb({ error: config.errors[793], code: 793 });
			}
		}
	...
	"add": function (config, req, cb) {
		...
		validateSourceCodeAttachment(config, req.soajs.inputmaskData.catalog, (error) => {
			...
			checkPorts(config, req.soajs.inputmaskData.catalog, (error) => {
                ...
                if (req.soajs.inputmaskData.catalog.recipe && req.soajs.inputmaskData.catalog.recipe.deployOptions && req.soajs.inputmaskData.catalog.recipe.deployOptions.voluming && req.soajs.inputmaskData.catalog.recipe.deployOptions.voluming.length > 0){
                    req.soajs.inputmaskData.catalog.recipe.deployOptions.voluming.forEach((oneVolume) => {
                    	if (oneVolume.docker && oneVolume.docker.volume && (!oneVolume.docker.volume.type || oneVolume.docker.volume.type === '')) {
                            error = {'code': "949" , "msg": config.errors['949']};
                    	}
					});
                }
