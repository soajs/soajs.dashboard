	...
	"generate": function (config, req, res) {
		...
		function checkService(cb) {
			...
			BL.model.findEntry(req.soajs, opts, function (err, oneRecord) {
				checkReturnError(req, res, {config: config, error: err, code: 600}, function () {
					checkReturnError(req, res, {config: config, error: (oneRecord), code: 614}, function () {
						return cb(null, true);
					});
				});
			});
		...
		function generateModule(cb) {
			...
			function buildDirectories(mCb) {
				...
				async.eachSeries(directories, function (dirPath, mCb) {
					...
					checkReturnError(req, res, {config: config, error: error, code: 853}, function () {
			...
			function writeFiles(mCb) {
				...
				async.each(files, function (fileObj, mCb) {
					...
					checkReturnError(req, res, {config: config, error: error, code: 854}, function () {
						return mCb(null, true);
					});
				});
			}
		...
		function cleanUp(cb) {
			...
			zipFolder(tempDir + "/", execDir + zipFileName + ".zip", function (error) {
				checkReturnError(req, res, {config: config, error: error, code: 852}, function () {
