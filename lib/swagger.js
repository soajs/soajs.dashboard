...
var BL = {
	...
	"generate": function (config, req, res) {
		...
		function cleanUp(cb) {
			...
			req.soajs.log.debug("into", execDir + serviceInfo.serviceName + ".zip");
			
			zipFolder(tempDir + "/", execDir + serviceInfo.serviceName + ".zip", function (error) {
		...
		checkService(function () {
			validateYaml(function () {
				generateModule(function () {
					cleanUp(function () {
						var stat = fs.statSync(execDir + serviceInfo.serviceName + ".zip");
						var readStream = fs.createReadStream(execDir + serviceInfo.serviceName + ".zip");
