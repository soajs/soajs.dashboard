...
var schemas = require(__dirname + "/../schemas/serviceSchema");
var swaggerUtils = require(__dirname + "/../utils/swagger");
...
var BL = {
	...
	"generate": function (config, req, res) {
		...
		function validateYaml(cb) {
			...
			context.soajs.config = swaggerUtils.mapConfig(serviceInfo);
			swaggerUtils.mapAPis(jsonAPISchema, function (response) {
				...
				var myValidator = new req.soajs.validator.Validator();
				var check = myValidator.validate(context.soajs.config, schemas.service);
				if (check.valid) {
					return cb(null, true);
				}
				else {
		...
		function generateModule(cb) {
			...
			function writeFiles(mCb) {
				var files = [
					//scripts
					{file: tempDir + "/grunt_src/gen.js", data: fs.readFileSync(tmplDir + "gen.js", "utf8")},
					{file: tempDir + "/grunt_src/mw.txt", data: fs.readFileSync(tmplDir + "mw.txt", "utf8")},
					{file: tempDir + "/grunt_src/swagger.js", data: fs.readFileSync(__dirname + "/../utils/swagger.js", "utf8")},
					{
						file: tempDir + "/grunt_src/schema.js",
						data: "\"use strict\";" + os.EOL + "module.exports = " + JSON.stringify(schemas.service, null, 2) + ";"
					},
					//module files
					{file: tempDir + "/swagger.yaml", data: req.soajs.inputmaskData.data.yaml},
					...
					{file: tempDir + "/test/helper.js", data: fs.readFileSync(tmplDir + "helper.txt", "utf8")},
					{file: tempDir + "/README.md", data: "# " + serviceInfo.serviceName},
					{
					...
					{
						file: tempDir + "/test/unit/_server.test.js",
					...
					{
						file: tempDir + "/test/unit/" + serviceInfo.serviceName + ".test.js",
					...
					{
						file: tempDir + "/test/integration/_server.test.js",
						data: fs.readFileSync(tmplDir + "server.txt", "utf8"),
						tokens: {
							service_name: serviceInfo.serviceName,
							type: "Integration"
					...
					{
						file: tempDir + "/test/integration/" + serviceInfo.serviceName + ".test.js",
						data: fs.readFileSync(tmplDir + "mocha.txt", "utf8"),
						tokens: {
							service_name: serviceInfo.serviceName,
							type: "Integration"
				...
				if (mongo) {
					files.push({
						file: tempDir + "/lib/models/mongo.js",
						data: fs.readFileSync(tmplDir + "mongo.txt", "utf8")
					});
				}
				if (es) {
					files.push({
						file: tempDir + "/lib/models/es.js",
			...
			function generateAPIsMw() {
				...
				async.each(APIs, function (oneAPI, mCb) {
					...
					fs.writeFile(tempDir + "/lib/mw/" + oneAPI.copy, data, "utf8", mCb);
				}, function (error) {
					checkIfError(req, res, {config: config, error: error, code: 854}, function () {
						return cb(null, true);
					});
