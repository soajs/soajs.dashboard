...
var jsonUtils = require('../../utils/apiBuilder/json.js');
var swaggerUtils = require('../../utils/apiBuilder/swagger.js');
...
var BL = {
	...
	"preUpdateSchemasValidation": function (config, req, res, cbMain) {
		let mainType = req.soajs.inputmaskData.mainType;
		let collection = mainType === 'endpoints' ? endpointsCollection : servicesCollection;
		...
		if ((!schemas || schemas === "") && (!swagger || swagger === "")) {
			return cbMain({"code": 922, "msg": "Please provide either a schema or a swagger object"});
		} else {
			if (schemas) {
				jsonUtils.preParseValidation(schemas, function (errorDescription) {
					if (errorDescription) {
						return cbMain({"code": 852, "msg": errorDescription});
					} else {
						return cbMain(null, {isValid: true});
					}
				});
			} else {
				let jsonAPISchema;
				try {
					jsonAPISchema = yamljs.parse(swagger);
				}
				catch (e) {
					return cbMain({"code": 851, "msg": e.message});
				}
				
				swaggerUtils.preMapApisValidation(jsonAPISchema, function (errorDescription) {
					if (errorDescription) {
						return cbMain({"code": 853, "msg": errorDescription});
					} else {
						return cbMain(null, {isValid: true});
					}
				});
