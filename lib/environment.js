...
function getEnvCerts(mongo, envRecords, counter, cb) {
	...
	var envId = envRecords[counter]._id.toString();
	mongo.find("fs.files", {'metadata.envId': envId}, function (error, certs) {
		if (error) {
			...
			counter++;
			getEnvCerts(mongo, envRecords, counter);

			if (cb) return cb(null, envRecords);
		}
	});
}
...
module.exports = {
					...
					checkIfError(req, res, {config: config, error: error, code: 404}, function () {
						checkIfError(req, res, {config: config, error: record, code: 500}, function () { // return error msg that this record is locked
							...
							mongo.remove(colName, criteria, function(error) {
								checkIfError(req, res, {config: config, error: error, code: 404}, function () {
									mongo.remove("fs.files", {metadata: {envId: req.soajs.inputmaskData.id}}, function (error) {
	...
	"uploadCerts": function (config, mongo, req, res) {
		//Steps////////////
		//1. Check for mongo and GridFs
		//2. Check if file already exists and remove it
		//3. Create write stream and stream file to mongo
		//4. Reply
		///////////////////
		...
		mongo.getMongoSkinDB(function (error, db) {
			checkIfError(req, res, {config: config, error: error, code: 600}, function () {
				...
				removeCertIfExists(gfs, req.query.filename, req.query.envId, function (error, result) {
					if (error) {
						console.log (error);
					} else {
						form.onPart = function (part) {
							...
							var writeStream = gfs.createWriteStream({
								...
								metadata: {envId: req.query.envId}
							});
		...
		function removeCertIfExists (filename, envId, cb) {
			mongo.findOne("fs.files", {filename: filename, 'metadata.envId': envId}, function (error, cert) {
				if (error) {
					return cb (error);
				}

				if (cert) {
					mongo.remove("fs.files", {filename: filename, 'metadata.envId': envId}, function (error, result) {
						if (error) {
							return cb (error, null);
						}
		...
		validateId(mongo, req, function (error) {
			checkIfError(req, res, {config: config, error: error, code: 600}, function () {
				mongo.findOne ("fs.files", {_id: req.soajs.inputmaskData.id}, function (error, data) {
					checkIfError(req, res, {config: config, error: error, code: 600}, function () {
						checkIfError(req, res, {config: config, error: !data, code: 600}, function () {
