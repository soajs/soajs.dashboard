...
function getEnvCerts(mongo, envRecords, counter, cb) {
	var envCode = envRecords[counter].code;
	mongo.find("fs.files", {'metadata.envCode': envCode}, function (error, certs) {
		if (error) {
			return cb (error, null);
		} else {
			if (certs && certs.length > 0) {
				certs.forEach(function (oneCert) {
					delete oneCert.contentType;
					delete oneCert.chunkSize;
					delete oneCert.aliases;
					delete oneCert.metadata;
					delete oneCert.md5;
				});
			} else {
				certs = [];
			...
			envRecords[counter].deployer['certificates'] = certs;

			counter++;
			if (counter === envRecords.length) {
				return cb(null, envRecords);
			} else {
				getEnvCerts(mongo, envRecords, counter, cb);
			}
		}
	});
}
...
module.exports = {
	"add": function(config, mongo, req, res) {
		...
		if(type !== 'manual'){
			var driver = req.soajs.inputmaskData.deployer.selected.split(".");
			checkIfError(req, res, {
				config: config,
				error: !req.soajs.inputmaskData.deployer[driver[0]][driver[1]] || JSON.stringify(req.soajs.inputmaskData.deployer[driver[0]][driver[1]]) === '{}',
				code: 407
			});
		}
			...
			checkIfError(req, res, {config: config, error: err, code: 405}, function () {
				...
				mongo.findOne(colName, criteria1, function(error, record) {
					checkIfError(req, res, {config: config, error: error, code: 404}, function () {
						checkIfError(req, res, {config: config, error: record.locked === true, code: 500}, function () { // return error msg that this record is locked
							var criteria = {
								'_id': req.soajs.inputmaskData.id, 'locked': {$ne: true}
							};
							mongo.remove(colName, criteria, function(error) {
								checkIfError(req, res, {config: config, error: error, code: 404}, function () {
									mongo.remove("fs.files", {metadata: {envCode: record.code}}, function (error) {
	...
	"list": function(config, mongo, req, res, cb) {
		mongo.find(colName, function(err, records) {
			checkIfError(req, res, {config: config, error: err, code: 402}, function () {
				var counter = 0;
				getEnvCerts(mongo, records, counter, function (error, envRecords) {
					checkIfError(req, res, {config: config, error: error, code: 728}, function () {
						if(cb && typeof(cb) === 'function') {
							return cb(envRecords);
						}
						else {
							return res.jsonp(req.soajs.buildResponse(null, envRecords));
						}
					});
				});
			});
	...
	"uploadCerts": function (config, mongo, req, res) {
		...
		mongo.getMongoSkinDB(function (error, db) {
			checkIfError(req, res, {config: config, error: error, code: 600}, function () {
				...
				removeCertIfExists(req.query.filename, req.query.envCode, function (error, result) {
					checkIfError(req, res, {config: config, error: error, code: 729}, function () {
						form.onPart = function (part) {
							...
							var writeStream = gfs.createWriteStream({
								...
								metadata: {envCode: req.query.envCode}
							});

							part.pipe(writeStream);
							writeStream.on('error', function (error) {
								checkIfError(req, res, {config: config, error: error, code: 727});
							});
		...
		function removeCertIfExists (filename, envCode, cb) {
			mongo.findOne("fs.files", {filename: filename, 'metadata.envCode': envCode}, function (error, cert) {
				if (error) {
					return cb (error, null);
				}

				if (cert) {
					mongo.remove("fs.files", {filename: filename, 'metadata.envCode': envCode}, function (error) {
						if (error) {
							cb (error, null);
						} else {
							cb (null, true);
	...
	"removeCert": function (config, mongo, req, res) {
		validateId(mongo, req, function (error) {
			checkIfError(req, res, {config: config, error: error, code: 701}, function () {
				mongo.findOne ("fs.files", {_id: req.soajs.inputmaskData.id}, function (error, data) {
					checkIfError(req, res, {config: config, error: error, code: 729}, function () {
						checkIfError(req, res, {config: config, error: !data, code: 730}, function () {
							mongo.remove("fs.files", {_id: req.soajs.inputmaskData.id}, function (error, data) {
								checkIfError(req, res, {config: config, error: error, code: 729}, function () {
