...
module.exports = {
	...
	"uploadCerts": function (config, mongo, req, res) {
		...
		mongo.getMongoSkinDB(function (error, db) {
			checkIfError(req, res, {config: config, error: error, code: 600}, function () {
				...
				form.onPart = function (part) {
					...
					//removeCertIfExists(gfs, part.filename, req.query.envId, function (error, result) {
					//	if (error) {
					//		console.log (error);
					//	}
		...
		function removeCertIfExists (gfs, filename, envId, cb) {
			gfs.exist({filename: filename, 'metadata.envId': envId}, function (error, cert) {
				if (error) {
					return cb (error);
				}
				...
				if (cert) {
					...
					gfs.remove ({filename: filename, 'metadata.envId': envId}, function (error, result) {
						if (error) {
							return cb (error);
						}
