...
var BL = {
	...
	"keyUpdate": function (config, provision, req, res) {
		...
		checkIfError(req, res, {
			config: config,
			error: myUrac.tenant.id !== req.soajs.tenant.id,
			code: 781
		}, function () {
			validateId(req.soajs, function (err) {
				checkIfError(req, res, {config: config, error: err, code: 405}, function () {
		...
		function generateKeysForTenantApps(tenantRecord, envRecord, dashExtKey, cb) {
			...
			async.each(tenantRecord.applications, function (oneApp, callback) {
				//generate new key
				console.log (tenantRecord, oneApp);
				generateNewKey(tenantRecord, oneApp, function (error, newKeys, updatedTenantRecord) {
					if (error) {
						console.log (error);
						return callback(error);
					}
					console.log ('------------------');
					console.log (updatedTenantRecord, envRecord, oneApp, newKeys, dashExtKey);
					//generate new external key
					generateNewExtKey(updatedTenantRecord, envRecord, oneApp, newKeys, dashExtKey, function (error, newKeys) {
						if (error) {
							console.log (error);
							return callback(error);
						}
		...
		function generateNewKey(tenantRecord, oneApp, cb) {
			var newKeys = [];
		...
		function generateNewExtKey(tenantRecord, envRecord, oneApp, keys, dashExtKey, cb) {
					...
					if (oneExtKey.env === envRecord.code.toUpperCase()) {
						...
						provision.generateExtKey(newInternalKey, {
							...
							if (dashExtKey && oneExtKey.extKey === dashExtKey.key && oneExtKey.env === dashExtKey.env) {
								newExtKey.dashboardAccess = true;
							}
			...
			tenantRecord.applications.forEach(function(oneApplication){
				oneApplication.keys.forEach(function(oneKey){
					oneKey.extKeys.forEach(function(oneExtKey){
						if(oneExtKey.env === envRecord.code.toUpperCase() && oneExtKey.dashboardAccess){
							key = oneExtKey.extKey;
						}
					});
				});
			});
			if(!key){
				return cb(new Error("No Dashboard external key found for environment: ", envRecord.code));
			}
			return cb(null, key);
		}
	},
