...
var BL = {
	...
	"add": function (config, service, dbModel, req, res, cb) {
		...
		BL.model.countEntries(req.soajs, opts, function (error, count) {
			checkReturnError(req, cb, {config: config, error: error, code: 400}, function () {
				checkReturnError(req, cb, {config: config, error: count > 0, code: 403}, function () {
					...
					BL.model.findEntry(req.soajs, opts, function (error, envRecord) {
						checkReturnError(req, cb, {config: config, error: error, code: 400}, function () {
							...
							BL.model.insertEntry(req.soajs, opts, function (err, data) {
								checkReturnError(req, cb, {config: config, error: err, code: 400}, function () {
	...
	"update": function (config, req, res, cb) {
		validateId(req.soajs, function (err) {
			checkReturnError(req, cb, {config: config, error: err, code: 405}, function () {
				switch (req.soajs.inputmaskData.services.config.session.proxy) {
					case "true":
						req.soajs.inputmaskData.services.config.session.proxy = true;
						break;
				...
				BL.model.updateEntry(req.soajs, opts, function (err, data) {
					checkReturnError(req, cb, {config: config, error: err, code: 401}, function () {
	...
	"list": function (config, req, res, cb) {
		...
		BL.model.findEntries(req.soajs, opts, function (err, records) {
			checkReturnError(req, cb, {config: config, error: err, code: 402}, function () {
	...
	"keyUpdate": function (config, provision, req, res, cb) {
		...
		checkCanEdit(req.soajs, function (error) {
			checkReturnError(req, cb, {config: config, error: error, code: error}, function () {
				validateId(req.soajs, function (err) {
					checkReturnError(req, cb, {config: config, error: err, code: 405}, function () {
		...
		function updateTenants(tenantRecords, envRecord) {
			...
			async.map(tenantRecords, function (oneTenant, tntCb) {
				...
				checkReturnError(req, cb, {config: config, error: error, code: 600}, function () {
	...
	"listDbs": function (config, req, res, cb) {
		...
		BL.model.findEntry(req.soajs, opts, function (err, envRecord) {
			checkReturnError(req, cb, {config: config, error: err || !envRecord, code: 402}, function () {
					...
					if (req.soajs.inputmaskData.name === 'session') {
						checkReturnError(req, cb, {
							...
							BL.model.saveEntry(req.soajs, opts, function (err) {
								checkReturnError(req, cb, {config: config, error: err, code: 503}, function () {
				...
				checkReturnError(req, cb, {
					...
					if (req.soajs.inputmaskData.name === 'session') {
						checkReturnError(req, cb, {
							...
							opts.record = envRecord;
							BL.model.saveEntry(req.soajs, opts, function (err) {
								checkReturnError(req, cb, {config: config, error: err, code: 513}, function () {
			...
			checkReturnError(req, cb, {config: config, error: err || !envRecord, code: 402}, function () {
				if (req.soajs.inputmaskData.name === 'session') {
					checkReturnError(req, cb, {
						...
						opts.record = envRecord;
						BL.model.saveEntry(req.soajs, opts, function (err) {
							checkReturnError(req, cb, {config: config, error: err, code: 514}, function () {
		...
		BL.model.findEntry(req.soajs, opts, function (err, envRecord) {
			checkReturnError(req, cb, {config: config, error: err || !envRecord, code: 402}, function () {
				...
				checkReturnError(req, cb, {
					...
					opts.record = envRecord;
					BL.model.saveEntry(req.soajs, opts, function (err) {
						checkReturnError(req, cb, {config: config, error: err, code: 505}, function () {
		...
		BL.model.findEntry(req.soajs, opts, function (err, envRecord) {
			checkReturnError(req, cb, {config: config, error: err || !envRecord, code: 402}, function () {
				...
				checkReturnError(req, cb, {
					...
					opts.record = envRecord;
					BL.model.saveEntry(req.soajs, opts, function (err) {
						checkReturnError(req, cb, {config: config, error: err, code: 506}, function () {
	...
	"deleteCluster": function (config, req, res, cb) {
		...
		opts.conditions = {'code': req.soajs.inputmaskData.env.toUpperCase()};
		BL.model.findEntry(req.soajs, opts, function (error, envRecord) {
			checkReturnError(req, cb, {config: config, error: error || !envRecord, code: 402}, function () {
				checkReturnError(req, cb, {
					config: config,
		...
		BL.model.findEntry(req.soajs, opts, function (error, envRecord) {
			checkReturnError(req, cb, {config: config, error: error || !envRecord, code: 402}, function () {
				listCerts(req.soajs, function (error, certs) {
					checkReturnError(req, cb, {config: config, error: error, code: 732}, function () {
		...
		validateInput(function (error) {
			checkReturnError(req, maincb, {
				...
				BL.model.getDb(req.soajs).getMongoDB(function (error, db) {
					checkReturnError(req, maincb, {config: config, error: error, code: 727}, function () {
						...
						form.onPart = function (part) {
							...
							writeStream.on('error', function (error) {
								return checkReturnError(req, maincb, {config: config, error: error, code: 727});
							});
		...
		validateId(req.soajs, function (error) {
			checkReturnError(req, cb, {config: config, error: error, code: 701}, function () {
				...
				BL.model.findEntry(req.soajs, opts, function (error, cert) {
					checkReturnError(req, cb, {config: config, error: error, code: 729}, function () {
						checkReturnError(req, cb, {config: config, error: !cert, code: 730}, function () {
	...
	"chooseExistingCerts": function (config, req, res, cb) {
		...
		BL.model.countEntries(req.soajs, opts, function (error, count) {
			checkReturnError(req, cb, {config: config, error: error, code: 600}, function () {
				checkReturnError(req, cb, {config: config, error: count === 0, code: 446}, function () {
					async.map(req.soajs.inputmaskData.certIds, function (oneCert, callback) {
						...
						checkReturnError(req, cb, {config: config, error: error, code: 701}, function () {
							...
							BL.model.findEntries(req.soajs, opts, function (error, certs) {
								checkReturnError(req, cb, {config: config, error: error, code: 728}, function () {
									checkReturnError(req, cb, {
										config: config,
	...
	"changeSelectedDriver": function (config, req, res, cb) {
		...
		BL.model.updateEntry(req.soajs, opts, function (error, result) {
			checkReturnError(req, cb, {config: config, error: error, code: 735}, function () {
				return cb(null, true);
			});
		});
	},
	...
	"changeDeployerType": function (config, req, res, cb) {
		...
		BL.model.updateEntry(req.soajs, opts, function (error, result) {
			checkReturnError(req, cb, {config: config, error: error || !result, code: 738}, function () {
				cb(null, true);
			});
		});
	},
	...
	"updateDeployerConfig": function (config, req, res, maincb) {
		...
		opts.conditions = {code: req.soajs.inputmaskData.env.toUpperCase()};
		BL.model.findEntry(req.soajs, opts, function (error, envRecord) {
			checkReturnError(req, maincb, {config: config, error: error, code: 600}, function () {
				checkReturnError(req, maincb, {config: config, error: !envRecord, code: 402}, function () {
		...
		function checkIfEnvIsDeployed(cb) {
			...
			BL.model.findEntry(req.soajs, opts, function (error, envRecord) {
				checkReturnError(req, cb, {config: config, error: error, code: 600}, function () {
					checkReturnError(req, cb, {config: config, error: !envRecord, code: 402}, function () {
						var options = utils.buildDeployerOptions(envRecord, req.soajs, BL);
						options.params = {env: req.soajs.inputmaskData.env.toUpperCase()};
						deployer.listServices(options, function (error, services) {
							utils.checkIfError(req.soajs, cb, {config: config, error: error}, function () { //utils.checkIfError() is used to handle driver errors
								checkReturnError(req, cb, {config: config, error: services.length > 0, code: 907}, cb);
							});
						});
					});
				});
