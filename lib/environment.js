...
module.exports = {
	...
	"keyUpdate": function (config, mongo, req, res) {
		/*
			...
			2. Generate New Key
				2.1 Get the current appId and key being used
				2.2 Generate a new key via provision.generateInternalKey()
				2.3 Clone the config of the current key to the newly generated one
			3. Update Tenant - add the new key
			4. Generate New External Key
				4.1 Get the current external key (search using value and env that it gives access to)
				4.2 Clone the device, geo, and expDate values from the current ext key to the new one
				4.3 Generate a new external key using the newly set algorithm and password
					4.3.1 If it fails, rollback() to delete the new key that was saved in step number 3
					4.3.2 Proceed with step number 6
				4.4 Prepare the new ext key object and add to it the env code
			5. Update Tenant - add the new external key
			6. Update Environment
				6.1 Update the env record to include the new algorithm and password
			7. Update Dashboard External Keys
				7.1 If exists, replace with the newly generated key for this environment
			8. Load Provision
		...
		*/
									...
									checkIfError(req, res, {config: config, error: error, code: error}, function () {
										...
										updateTenantRecord(tenantRecord, function (error) {
											checkIfError(req, res, {config: config, error: error, code: error}, function () {
												...
												generateNewExtKey(provision, tenantRecord, envRecord, function (error) {
													if (error) {
													...
													updateTenantRecord(tenantRecord, function (error) {
														checkIfError(req, res, {
															...
															//update dashboard external keys
															updateDashboardExtKey(tenantRecord, envRecord, newKey, function (error) {
																checkIfError(req, res, {config: config, error: error, code : error}, function () {
																	//update environment
																		...
																		checkIfError(req, res, {
																			...
																			provision.loadProvision(function (loaded) {
																				...
																				if (envRecord.code.toLowerCase() === 'dashboard') {
																					data = {'newKey': newKey.extKeys[0].extKey};
																				}
																				else {
																					data = true;
																				}
		...
		function generateNewExtKey(provision, tenantRecord, envRecord, cb) {
			var app = position[0];
			var key = position[1];
			...
			for (var extKey = 0; extKey < tenantRecord.applications[app].keys[key].extKeys.length; extKey++) {
				if (tenantRecord.applications[app].keys[key].extKeys[extKey].extKey === req.soajs.tenant.key.eKey && tenantRecord.applications[app].keys[key].extKeys[extKey].env.toLowerCase() === envRecord.code.toLowerCase()) {
					...
					var newExtKey = {
						...
						env: tenantRecord.applications[app].keys[key].extKeys[extKey].env
					};
		...
		function updateDashboardExtKey(tenantRecord, envRecord, newKey, cb) {
			var criteria = {
				code: tenantRecord.code.toUpperCase(),
				env: envRecord.code.toUpperCase()
			};
			mongo.count(dashExtKeysColName, criteria, function (error, count) {
				if (count === 0) {
					return cb(null, true);
				}
