...
var BL = {
	...
	"add": function (config, service, dbModel, req, res, cb) {
		...
		BL.model.countEntries(req.soajs, opts, function (error, count) {
			checkIfError(req, cb, {config: config, error: error, code: 400}, function () {
				checkIfError(req, cb, {config: config, error: count > 0, code: 403}, function () {
					...
					BL.model.findEntry(req.soajs, opts, function (error, envRecord) {
						checkIfError(req, cb, {config: config, error: error, code: 400}, function () {
							...
							BL.model.insertEntry(req.soajs, opts, function (err, data) {
								checkIfError(req, cb, {config: config, error: err, code: 400}, function () {
	...
	"delete": function (config, req, res, cb) {
		...
		validateId(req.soajs, function (err) {
			checkIfError(req, cb, {config: config, error: err, code: 405}, function () {
				opts.collection = colName;
				opts.conditions = {'_id': req.soajs.inputmaskData.id};
				BL.model.findEntry(req.soajs, opts, function (error, record) {
					checkIfError(req, cb, {config: config, error: error || !record, code: 404}, function () {
						checkIfError(req, cb, {config: config, error: record.locked, code: 500}, function () { // return error msg that this record is locked
		...
		function deleteEnv() {
			...
			BL.model.removeEntry(req.soajs, opts, function (error) {
				checkIfError(req, cb, {config: config, error: error, code: 404}, function () {
					return cb(null, "environment delete successful");
				});
			});
		}
		...
		function checkManualDeployments(envRecord, cb) {
			...
			BL.model.countEntries(req.soajs, opts, function (error, count) {
				checkIfError(req, cb, {config: config, error: error, code: 402}, function () {
					checkIfError(req, cb, {config: config, error: count > 0, code: 906}, cb);
				});
			});
		}
		...
		function checkContainerDeployments(envRecord, cb) {
			...
			deployer.listServices(options, function (error, services) {
				utils.checkIfError(req, cb, {config: config, error: error}, function () { //utils.checkIfError() is used to handle driver errors
					checkIfError(req, cb, {config: config, error: services.length > 0, code: 906}, cb);
				});
			});
		}
	},
		...
		validateId(req.soajs, function (err) {
			checkIfError(req, cb, {config: config, error: err, code: 405}, function () {
				...
				BL.model.updateEntry(req.soajs, opts, function (err, data) {
					checkIfError(req, cb, {config: config, error: err, code: 401}, function () {
						return cb(null, "environment update successful");
					});
				});
			});
		});
	...
	"list": function (config, req, res, cb) {
		...
		BL.model.findEntries(req.soajs, opts, function (err, records) {
			checkIfError(req, cb, {config: config, error: err, code: 402}, function () {
	...
	"keyUpdate": function (config, provision, req, res, cb) {
		...
		checkCanEdit(req.soajs, function (error) {
			checkIfError(req, cb, {config: config, error: error, code: error}, function () {
				validateId(req.soajs, function (err) {
					checkIfError(req, cb, {config: config, error: err, code: 405}, function () {
	...
	"listDbs": function (config, req, res, cb) {
		var opts = {};
		opts.collection = colName;
		opts.conditions = {'code': req.soajs.inputmaskData.env.toUpperCase()};
		BL.model.findEntry(req.soajs, opts, function (err, envRecord) {
			checkIfError(req, cb, {config: config, error: err || !envRecord, code: 402}, function () {
				return cb(null, envRecord.dbs);
			});
		});
	},
	
	"addDb": function (config, req, res, cb) {
		if (req.soajs.inputmaskData.name === 'session') {
			if (!req.soajs.inputmaskData.sessionInfo || JSON.stringify(req.soajs.inputmaskData.sessionInfo) === '{}') {
				return cb({"code": 507, "msg": config.errors[507]});
			}
		}
		var opts = {};
		opts.collection = colName;
					...
					if (req.soajs.inputmaskData.name === 'session') {
						checkIfError(req, cb, {
							...
							BL.model.saveEntry(req.soajs, opts, function (err) {
								checkIfError(req, cb, {config: config, error: err, code: 503}, function () {
									return cb(null, "environment database added successful");
								});
							});
						});
					}
	...
	"updateDb": function (config, req, res, cb) {
		if (req.soajs.inputmaskData.name === 'session') {
			if (!req.soajs.inputmaskData.sessionInfo || JSON.stringify(req.soajs.inputmaskData.sessionInfo) === '{}') {
				return cb({"code": 507, "msg": config.errors[507]});
			}
				...
				checkIfError(req, cb, {
					...
					if (req.soajs.inputmaskData.name === 'session') {
						checkIfError(req, cb, {
							...
							opts.record = envRecord;
							BL.model.saveEntry(req.soajs, opts, function (err) {
								checkIfError(req, cb, {config: config, error: err, code: 513}, function () {
									return cb(null, "environment database updated successful");
								});
							});
						});
					}
	...
	"deleteDb": function (config, req, res, cb) {
		var opts = {};
		opts.collection = colName;
		opts.conditions = {'code': req.soajs.inputmaskData.env.toUpperCase()};
		BL.model.findEntry(req.soajs, opts, function (err, envRecord) {
			checkIfError(req, cb, {config: config, error: err || !envRecord, code: 402}, function () {
				if (req.soajs.inputmaskData.name === 'session') {
					checkIfError(req, cb, {
						...
						opts.record = envRecord;
						BL.model.saveEntry(req.soajs, opts, function (err) {
							checkIfError(req, cb, {config: config, error: err, code: 514}, function () {
								return cb(null, "environment database removed successful");
							});
						});
					});
				}
	...
	"updateDbsPrefix": function (config, req, res, cb) {
		...
		BL.model.updateEntry(req.soajs, opts, function (err) {
			checkIfError(req, cb, {config: config, error: err, code: 402}, function () {
				return cb(null, "environment Database prefix update successful");
			});
		});
	},
	
	"listClusters": function (config, req, res, cb) {
		var opts = {};
		opts.collection = colName;
		opts.conditions = {'code': req.soajs.inputmaskData.env.toUpperCase()};
		BL.model.findEntry(req.soajs, opts, function (err, envRecord) {
			checkIfError(req, cb, {config: config, error: err || !envRecord, code: 402}, function () {
				return cb(null, envRecord.dbs.clusters);
			});
		});
	},
	
	"addCluster": function (config, req, res, cb) {
		var opts = {};
		opts.collection = colName;
		opts.conditions = {'code': req.soajs.inputmaskData.env.toUpperCase()};
		BL.model.findEntry(req.soajs, opts, function (err, envRecord) {
			checkIfError(req, cb, {config: config, error: err || !envRecord, code: 402}, function () {
				...
				checkIfError(req, cb, {
					...
					opts.record = envRecord;
					BL.model.saveEntry(req.soajs, opts, function (err) {
						checkIfError(req, cb, {config: config, error: err, code: 505}, function () {
							return cb(null, "environment cluster added successful");
						});
					});
				});
			});
	...
	"updateCluster": function (config, req, res, cb) {
		var opts = {};
		opts.collection = colName;
		opts.conditions = {'code': req.soajs.inputmaskData.env.toUpperCase()};
		BL.model.findEntry(req.soajs, opts, function (err, envRecord) {
			checkIfError(req, cb, {config: config, error: err || !envRecord, code: 402}, function () {
				...
				checkIfError(req, cb, {
					...
					opts.record = envRecord;
					BL.model.saveEntry(req.soajs, opts, function (err) {
						checkIfError(req, cb, {config: config, error: err, code: 506}, function () {
							return cb(null, "environment cluster updated successful");
						});
					});
				});
			});
	...
	"deleteCluster": function (config, req, res, cb) {
		var opts = {};
		opts.collection = colName;
		opts.conditions = {'code': req.soajs.inputmaskData.env.toUpperCase()};
		BL.model.findEntry(req.soajs, opts, function (error, envRecord) {
			checkIfError(req, cb, {config: config, error: error || !envRecord, code: 402}, function () {
				checkIfError(req, cb, {
					config: config,
					...
					BL.model.saveEntry(req.soajs, opts, function (err) {
						checkIfError(req, cb, {config: config, error: err, code: 402}, function () {
							return cb(null, "environment cluster removed successful");
						});
					});
				});
			});
	...
	"listPlatforms": function (config, req, res, cb) {
		...
		BL.model.findEntry(req.soajs, opts, function (error, envRecord) {
			checkIfError(req, cb, {config: config, error: error || !envRecord, code: 402}, function () {
				listCerts(req.soajs, function (error, certs) {
					checkIfError(req, cb, {config: config, error: error, code: 732}, function () {
	...
	"uploadCerts": function (config, req, res, maincb) {
		...
		validateInput(function (error) {
			checkIfError(req, maincb, {
				config: config,
				error: error,
				code: ((error && error.code) ? error.code : 739)
			}, function () {
				BL.model.getDb(req.soajs).getMongoDB(function (error, db) {
					checkIfError(req, maincb, {config: config, error: error, code: 727}, function () {
						...
						form.onPart = function (part) {
							...
							writeStream.on('error', function (error) {
								return checkIfError(req, maincb, {config: config, error: error, code: 727});
							});
	...
	"removeCert": function (config, req, res, cb) {
		var opts = {};
		validateId(req.soajs, function (error) {
			checkIfError(req, cb, {config: config, error: error, code: 701}, function () {
				...
				BL.model.findEntry(req.soajs, opts, function (error, cert) {
					checkIfError(req, cb, {config: config, error: error, code: 729}, function () {
						checkIfError(req, cb, {config: config, error: !cert, code: 730}, function () {
	...
	"chooseExistingCerts": function (config, req, res, cb) {
		var opts = {};
		opts.collection = colName;
		opts.conditions = {code: req.soajs.inputmaskData.env};
		BL.model.countEntries(req.soajs, opts, function (error, count) {
			checkIfError(req, cb, {config: config, error: error, code: 600}, function () {
				checkIfError(req, cb, {config: config, error: count === 0, code: 446}, function () {
					async.map(req.soajs.inputmaskData.certIds, function (oneCert, callback) {
						...
						checkIfError(req, cb, {config: config, error: error, code: 701}, function () {
							...
							BL.model.findEntries(req.soajs, opts, function (error, certs) {
								checkIfError(req, cb, {config: config, error: error, code: 728}, function () {
									checkIfError(req, cb, {
										config: config,
	...
	"changeSelectedDriver": function (config, req, res, cb) {
		var opts = {};
		var criteria = {
		...
		BL.model.updateEntry(req.soajs, opts, function (error, result) {
			checkIfError(req, cb, {config: config, error: error, code: 735}, function () {
				return cb(null, true);
			});
		});
	},
	
	"changeDeployerType": function (config, req, res, cb) {
		var opts = {};
		opts.collection = colName;
		opts.conditions = {code: req.soajs.inputmaskData.env};
		opts.fields = {$set: {'deployer.type': req.soajs.inputmaskData.deployerType}};
		BL.model.updateEntry(req.soajs, opts, function (error, result) {
			checkIfError(req, cb, {config: config, error: error || !result, code: 738}, function () {
				cb(null, true);
			});
		});
	},
	 ...
	 */
	"updateDeployerConfig": function (config, req, res, maincb) {
		var opts = {};
		opts.collection = colName;
		opts.conditions = {code: req.soajs.inputmaskData.env.toUpperCase()};
		BL.model.findEntry(req.soajs, opts, function (error, envRecord) {
			checkIfError(req, maincb, {config: config, error: error, code: 600}, function () {
				checkIfError(req, maincb, {config: config, error: !envRecord, code: 402}, function () {
		...
		function checkIfEnvIsDeployed(cb) {
			...
			BL.model.findEntry(req.soajs, opts, function (error, envRecord) {
				checkIfError(req, cb, {config: config, error: error, code: 600}, function () {
					checkIfError(req, cb, {config: config, error: !envRecord, code: 402}, function () {
						var options = utils.buildDeployerOptions(envRecord, req.soajs, BL);
						...
						deployer.listServices(options, function (error, services) {
							utils.checkIfError(req.soajs, res, {config: config, error: error}, function () { //utils.checkIfError() is used to handle driver errors
								checkIfError(req, cb, {config: config, error: services.length > 0, code: 907}, cb);
							});
						});
					});
				});
