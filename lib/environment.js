...
function deleteCerts(mongo, certsArr, req, counter, cb) {
	...
	if (cert.metadata.env[envCode].length === 1 && Object.keys(cert.metadata.env).length === 1) { //only 1 available env and driver
		mongo.remove("fs.files", {_id: cert._id}, function (error, data) {
			...
			counter++;
			if (counter === certsArr.length) {
				cb (null, true);
			} else {
				deleteCerts(mongo, certsArr, req, counter, cb);
			}
		});
	} else if (cert.metadata.env[envCode].length === 1 && Object.keys(cert.metadata.env).length > 1) { //other env are available
	...
	} else if (cert.metadata.env[envCode].length > 1) {
...
module.exports = {
	...
	"uploadCerts": function (config, mongo, req, res) {
				...
				checkIfError(req, res, {config: config, error: cert, code: 731}, function () {
					mongo.getMongoSkinDB(function (error, db) {
						checkIfError(req, res, {config: config, error: error, code: 727}, function () {
							...
							removeCertIfExists(req, function (error, result) {
		...
		function removeCertIfExists (req, cb) {
			var criteria = {
				filename: '',
				metadata: {
					env: {}
			...
			criteria.metadata.env[req.query.envCode] = [req.query.driver];
			...
			mongo.findOne("fs.files", criteria, function (error, cert) {
				...
				if (cert) {
					if (cert.metadata.env[req.query.envCode].length === 1) {
						...
						cert.metadata.env[req.query.envCode].splice(cert.metadata.env[req.query.envCode].indexOf(req.query.driver), 1);
	...
	"chooseExistingCerts": function (config, mongo, req, res) {
		...
		mongo.find("fs.files", criteria, function (error, certs) {
			checkIfError(req, res, {config: config, error: error, code: 728}, function () {
				checkIfError(req, res, {config: config, error: !certs, code: 730}, function () {
	...
	"editDriver": function (config, mongo, req, res) {
		...
		if (driver === 'dockermachine - local') {
			...
			update['$set']['deployer.container.docker.scoket'] = req.soajs.inputmaskData.socket;
		}
