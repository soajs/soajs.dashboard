...
module.exports = {
		...
		mongo.findOne(colName, {code: req.soajs.inputmaskData.env.toUpperCase()}, function (error, envRecord) {
			checkIfError(req, res, {config: config, error: error, code: 733}, function () {
				checkIfError(req, res, {config: config, error: !envRecord, code: 446}, function () {
					...
					var platform = req.soajs.inputmaskData.platform;
					var driver = req.soajs.inputmaskData.driver;
					var driverConfig = req.soajs.inputmaskData.config;

					if (type === 'container' && !envRecord.deployer[type]) {
						req.soajs.log.debug("Deployer config for " + envRecord.code + " might not have been initialized, using default schema");
					...
					envRecord.deployer[type][platform][driver] = driverConfig;
					mongo.save(colName, envRecord, function (error) {
						checkIfError(req, res, {config: config, error: error, code: 733}, function () {
							return res.jsonp(req.soajs.buildResponse(null, true));
						});
					});
	...
	"editDriver": function (config, mongo, req, res) {
		...
		var criteria = {code: req.soajs.inputmaskData.env.toUpperCase()};
		var update = {	'$set': {}	};

		update.$set['deployer.' + type + '.' + platform + '.' + driver] = driverConfig;
