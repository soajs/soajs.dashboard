...
const configFileModule = {
	...
	"getFile": (config, req, options, soajsFile, cb) => {
		...
		function createConfigData (repoConfig, configSHA, fileData, cb){
			let dataNeeded = {
				content : {},
				configSHA: configSHA
			};
			options.configGenerator.generate(repoConfig, fileData, (error, config) => {
				dataNeeded.content = config;
				return cb(error, dataNeeded);
			});
		}
		function writeFiles(repoConfig, configSHA, fileData, cb) {
			createConfigData (repoConfig, configSHA, fileData, (err, data)=>{
				return cb(err, data);
