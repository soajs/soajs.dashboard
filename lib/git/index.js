...
var BL = {
		...
		gitModel.listGitAccounts(req.soajs, BL.model, function (error, records) {
			checkReturnError(req, cbMain, {config: config, error: error, code: 756}, function () {
				...
				if (req.soajs.inputmaskData.rms) {
					let soajsRMS = ['soajs/soajs.controller', 'soajs/soajs.urac', 'soajs/soajs.oauth', 'soajs/soajs.multitenant'];
		...
		validateId(req.soajs, function (error) {
			checkReturnError(req, cbMain, {config: config, error: error, code: 701}, function () {
				...
				if (req.soajs.inputmaskData.name) {
					options.name = req.soajs.inputmaskData.name;
				}
		...
		validateId(req.soajs, function (error) {
			...
			checkSAASSettings(() => {
				gitModel.getAuthToken(req.soajs, BL.model, {accountId: req.soajs.inputmaskData.id}, function (error, record) {
					checkReturnError(req, cbMain, {config: config, error: error || !record, code: 755}, function () {
									...
									checkReturnError(req, cbMain, {
										...
										if (req.soajs.inputmaskData.git && req.soajs.inputmaskData.git.branches && req.soajs.inputmaskData.git.branches.length > 0) {
											...
											if (reesult.type === 'service' || reesult.type === 'daemon') {
												newRepo.swaggerSHA = configSHA[0].swaggerSHA;
										...
										gitModel.addRepoToAccount(req.soajs, BL.model, {
											...
											checkReturnError(req, cbMain, {
												...
												if (req.soajs.noApi && reesult.type === 'service' || reesult.type === 'daemon') {
													reesult.noApi = true;
													if (req.soajs.reason) {
				...
				if (error) {
					...
					if (error.code === 761) {
						...
						if (unauthorizedUpdate) {
							return cb({
								"code": 761,
								"msg": `Failed to activate branch. This repository is of type ${typeOfService}, make sure a [ soa.js | soa.json | config.js ]  file is available in your branch and it has the right schema.
	...
	'deactivateRepo': function (config, req, git, helpers, model, deployer, cbMain) {
		...
		validateId(req.soajs, function (error) {
			checkReturnError(req, cbMain, {config: config, error: error, code: 701}, function () {
				...
				gitModel.getRepo(req.soajs, BL.model, {
					accountId: req.soajs.inputmaskData.id,
					repoLabel: repoLabel
				}, function (error, repoRecord) {
					checkReturnError(req, cbMain, {config: config, error: error, code: 765}, function () {
						var type = '';
						if (!repoRecord) type = 'custom';
						else type = repoRecord.repos[0].type;
						remove(type, repoRecord, function (error) {
							checkReturnError(req, cbMain, {
								config: config,
								error: error,
		...
		function remove(type, repoRecord, cb) {
			checkCatalogRecipes(() => {
				switch (type) {
					case 'service':
					case 'daemon':
						removeServiceOrDaemon(type, repoRecord, cb);
						break;
		...
		function removeServiceOrDaemon(type, repoRecord, cb) {
			var coll = '';
			let module;
			if (type === 'service') {
				module = model.serviceModel;
			...
			module.findEntry(req.soajs, BL.model, opts, function (error, record) {
				checkMongoError(req, error, cb, function () {
					if (!record) {
						// if service is deleted bypass
						if (repoRecord.repos){
							repoRecord.repos.forEach((oneRepo)=>{
								if (oneRepo.name === req.soajs.inputmaskData.owner + '/' + req.soajs.inputmaskData.repo){
					...
					serviceName = req.soajs.inputmaskData.record.name;
					if (req.soajs.inputmaskData.branch) {
			...
			function removeCdRecord(req, method, cb) {
				//if no service, we can't delete cicd
				if (method !== "removeEntry" || serviceName) {
					return cb(null, true);
				} else {
					let colName = 'cicd';
												...
												checkReturnError(req, cbMain, {
													...
													gitModel.updateRepoInfo(req.soajs, BL.model, opts, function (error, reesult) {
														checkReturnError(req, cbMain, {
															...
															return cbMain(null, {status: reesult ? reesult.status: 1});
														});
													});
												});
											});
	...
	'getFile': function (config, req, git, deployer, helpers, gitModel, cbMain) {
		gitModel.getAccount(req.soajs, BL.model, {
			...
			checkReturnError(req, cbMain, {config: config, error: error || !account, code: 757}, function () {
				if (process.env.SOAJS_DEPLOY_HA && req.soajs.inputmaskData.env) {
			...
			checkReturnError(req, cbMain, {
				...
				checkReturnError(req, cbMain, {
					...
					checkReturnError(req, cbMain, {
						...
						}, 'findService', options, (error, oneService) => {
	...
	'getAnyFile': function (config, req, git, helpers, gitModel, cbMain) {
		BL.model.validateCustomId(req.soajs, req.soajs.inputmaskData.accountId, (error, id) => {
			checkReturnError(req, cbMain, {config: config, error: error}, function () {
				gitModel.getAccount(req.soajs, BL.model, {
					accountId: id
				}, function (error, account) {
					if (!req.soajs.inputmaskData.owner) {
						req.soajs.inputmaskData.owner = account.owner;
					}
					checkReturnError(req, cbMain, {config: config, error: error || !account, code: 757}, function () {
						helpers.doGetFile(req, BL, git, gitModel, account, req.soajs.inputmaskData.branch, cbMain);
					});
				});
			});
		})
