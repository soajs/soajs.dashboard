				...
				checkMongoError(req, error || !record, cb, function () {
					if(process.env.SOAJS_DEPLOY_HA){
						...
						cloudBL.listServices(config, req.soajs, deployer, function(error, services){
							checkReturnError(req, cbMain, { config: config, error: error, code: 795 }, function () {
								//check if a service is using this repo
								for(var i =0; i < services.length; i++){
									var match = false;
									var oneService = services[i];
									for(var serviceLabel in oneService.labels){
										if(serviceLabel === req.soajs.inputmaskData.repo){
											match = true;
										}
									}
									if(match){
								...
								if(match){
									return cb({ 'code': 766, 'message': 'Repository has services/daemons deployed' });
								}
								else{
									//do remove
									doRemove();
