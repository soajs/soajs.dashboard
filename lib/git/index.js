...
var BL = {
	...
	'deactivateRepo': function (config, req, git, helpers, gitModel, cloudBL, registry, deployer, cbMain) {
		validateId(req.soajs, function (error) {
			checkReturnError(req, cbMain, { config: config, error: error, code: 701 }, function () {
			...
			BL.model.findEntry(req.soajs, opts, function (error, record) {
				checkMongoError(req, error || !record, cb, function () {
					if(process.env.SOAJS_DEPLOY_HA){
						helpers.checkifRepoIsDeployed(req, config, BL, cloudBL, registry, deployer, cb, function(){
							//do remove
							doRemove();
						});
					}
	 ...
	 * @param cbMain
	 */
	'getFile': function (config, req, git, registry, deployer, helpers, gitModel, cbMain) {
		gitModel.getAccount(req.soajs, BL.model, {
			owner: req.soajs.inputmaskData.owner,
			repo: req.soajs.inputmaskData.repo
			...
			checkReturnError(req, cbMain, { config: config, error: error || !account, code: 757 }, function () {
				if (process.env.SOAJS_DEPLOY_HA) {
					BL.getHaFile(account, config, req, git, registry, deployer, helpers, gitModel, cbMain);
				}
				else {
	...
	'getHaFile': function (account, config, req, git, registry, deployer, helpers, gitModel, cbMain) {
		var options = helpers.buildDeployerOptions(registry, req.soajs, BL.model);
		checkReturnError(req, cbMain, {
			config: config,
			error: !options,
			code: 825
			...
			options.params = {
				env: registry.environment.toLowerCase(),
