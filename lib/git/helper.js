							...
							checkReturnError(req, mainCb, { config: config, error: error, code: 795 }, function () {
								...
								for(var i =0; i < services.length; i++){
									...
									if(oneService.labels['service.repo'] === req.soajs.inputmaskData.repo){
										...
										if( req.soajs.inputmaskData.oldVersion){
					...
					if (error) {
						if (version) {
							...
							if (req.soajs.inputmaskData.branch){
								message = `You cannot sync/delete from branch ${req.soajs.inputmaskData.branch}
								...
								message = `You cannot delete this repository ${req.soajs.inputmaskData.repo} because: v ${req.soajs.inputmaskData.oldVersion} is deployed. Please  delete all its deployments first.`;
							}
							return mainCb({
