...
var helpers = {
	...
	syncMyRepo: function (model, soajs, type, info, sha, cb) {
		...
		opts.conditions['name'] = info.info.name;
		...
		if (type === 'service') {
			opts.collection = servicesColl;
			opts.conditions['port'] = info.info.port;
		} else if (type === 'daemon') {
			opts.collection = daemonsColl;
			opts.conditions['port'] = info.info.port;
		}
		...
		delete info.info.name;
		delete info.info.path;

		if (info.info.port) {
			delete info.info.port;
		}
		...
		for (var p in info.info) {
			if (Object.hasOwnProperty.call(info.info, p)) {
				if (p !== 'versions') {
					s.$set[ p ] = info.info[ p ];
				}
			}
		...
		if (info.info && info.info.versions) {
			for (var vp in info.info.versions) {
				if (Object.hasOwnProperty.call(info.info.versions, vp)) {
					s.$set[ "versions." + vp.toString() ] = info.info.versions[ vp ];
				}
			}
		}
		opts.fields = s;
