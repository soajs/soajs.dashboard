...
var lib = {
	...
	"populateCommonFields": function (commonFields) {
		...
		function mapSimpleField(oneField) {
			if (oneField['$ref']) {
				return utils.cloneObj(lib.getIMFVfromCommonFields(commonFields, oneField['$ref']));
			} else {
				return oneField;
			}
		}
...
var utils = {
			...
			methods.forEach(function (oneMethod) {
				...
				if (apiPath[route][oneMethod].parameters && apiPath[route][oneMethod].parameters.length > 0) {
					for (var input in apiPath[route][oneMethod].parameters) {
						...
						} else if (oneInput.schema) {
							if (oneInput.schema['$ref']) {
								var oneInputName = oneInput.schema['$ref'].split("/");
								oneInputName = oneInputName[oneInputName.length - 1];
								if (commonFields[oneInputName] && commonFields[oneInputName].source) {
									all_apis[oneMethod.toLowerCase()][soajsRoute].imfv.commonFields.push(oneInputName);
								}
							} else if (oneInput.schema.items && oneInput.schema.items['$ref'] && oneInput.schema.type === "array") {
								var oneInputName = oneInput.schema.items['$ref'].split("/");
								oneInputName = oneInputName[oneInputName.length - 1];
								if (commonFields[oneInputName] && commonFields[oneInputName].source) {
									all_apis[oneMethod.toLowerCase()][soajsRoute].imfv.commonFields.push({
										"type": "array",
										"items": oneInputName
									});
								}
	...
	"generateFiles": function (fileObj, cb) {
		...
		var data = JSON.stringify(utils.cloneObj(fileObj.data), null, 2);
		...
		if (data && data.schema) {
			for (var key in data.schema) {
				for (var subKey in data.schema[key]) {
					if (data.schema[key][subKey] && data.schema[key][subKey].mv) {
						data.schema[key][subKey].mv = data.replace(/\\"/g, '"').replace(/["]+/g, '"').replace(/"__dirname/g, '__dirname');
					}
				}
			}
		}
...
var generator = {
	...
	"generate": function (req, repoConfig, fileContent, callback) {
		let soa = null;
		try {
			soa = JSON.parse(JSON.stringify(repoConfig));
		} catch (e) {
		 ...
		 * @returns {*}
		 */
		function validateSwagger(cb) {
			utils.parseYaml(fileContent ? fileContent.content : null, context, cb);
		}
		 ...
		 * @param cb
		 */
		function generateModule(err, cb) {
			var files = {
				data: context.soajs.config,
				tokens: {
					dirname: "__dirname"
				},
				purify: true
		...
		validateSwagger((err) => {
			if (err) {
				...
				req.soajs.reason = err.code;
			}
			generateModule(err, (error, data) => {
				if (error) {
					return callback(err);
				} else {
					//add the swagger file
					if (data && typeof data === 'object') {
						if (context.yaml) {
							data.swaggerFile = context.yaml;
						}
						data.soaFile = soa;
					}
