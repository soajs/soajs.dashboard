...
var BL = {
	...
	"checkAnalytics": function (config, soajs, res) {
		BL.model.getEnvAnalyticsRecord(soajs, soajs.inputmaskData.env, function (error, record) {
			checkIfError(soajs, res, {config: config, error: error || !record, code: 600}, function () {
				return res.jsonp(soajs.buildResponse(null, record.json.activated));
			});
		});
	},
	...
	"activateAnalytics": function (config, soajs, res) {
		...
		function putIntoElastic(type, route, cb) {
			BL.model.getAnalyticsRecords(soajs, type, function (error, content) {
				...
				async.each(content, function (oneContent, callback) {
					if (type === 'visual' || type === 'dashboard') {
						route += oneContent.json.title.toLowerCase().replace(' ', '-');
					}

					var options = {
						method: 'XPUT',
						uri: 'http://' + elasticURL + route,
						data: oneContent.json
					};
		...
		function putIndexPattern(cb) {
			var indexPattern = soajs.inputmaskData.env + '-nginx-access-*';
			var options = {
				method: 'XPUT',
				uri: 'http://' + elasticURL + '/index-pattern/' + indexPattern,
				data: {
					title: soajs.inputmaskData.env.toUpperCase() + ' Index Pattern',
			...
			request(options, function (error, response, body) {
				...
				options = {
					method: 'XPUT',
					uri: 'http://' + elasticURL + '/.kibana/config/4.6.0',
					data: {
