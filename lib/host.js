...
function deployContainerServiceOrDaemon(config, soajs, res, model, type) {
	...
	function constructDeployerParams(cb){
		...
		if (context.dbRecord.src && context.dbRecord.src.main) {
			dockerParams.Cmd[2] = dockerParams.Cmd[2] + ' ' + context.dbRecord.src.main;
		}
	...
	function createContainer(cb){
		soajs.log.debug("Calling create container with params:", JSON.stringify(context.deployerConfig), JSON.stringify(dockerParams));
	...
	function registerHostInfo(cb){
		...
		registerHost(soajs, res, model, config, context.data, context.deployerConfig, info, cb);
	}
