...
var BL = {
	...
	"deployNginx": function(config, soajs, res){
		...
		BL.model.getEnvironment(soajs, soajs.inputmaskData.envCode, function (err, envRecord) {
			checkIfError(soajs, res, {config: config, error: err || !envRecord, code: 600}, function () {
				...
				soajs.log.debug("checking for old nginx container for environment: " + envRecord.code);
				BL.model.getOneContainer(soajs, envRecord.code, "nignx", function (error, oldNginx) {
					checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
						if (oldNginx) {
			...
			BL.model.getContainers(soajs, envRecord.code, "controller", true, function (error, controllers) {
				checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
					...
					if (!controllers || (controllers && controllers.length === 0)) {
						...
						BL.model.getHosts(soajs, envRecord.env, "controller", function (error, hostsData) {
							checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
					...
					checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
						...
						checkIfError(soajs, res, {
							...
							checkIfError(soajs, res, {
								...
								soajs.log.debug("Calling create nginx container with params:", JSON.stringify(deployerConfig), JSON.stringify(dockerParams));
								deployer.createContainer(deployerConfig, dockerParams, BL.model.getDB(), function (error, data) {
									checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {

										soajs.log.debug("Nginx Container Created, starting container with params:", JSON.stringify(deployerConfig), JSON.stringify(data));
										deployer.start(deployerConfig, data.Id, BL.model.getDB(), function (error, data) {
											checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {

												soajs.log.debug("Nginx Container started. Saving nginx container information in docker collection.");
												var document = {
													"env": soajs.inputmaskData.envCode.toLowerCase(),
													"cid": data.Id,
	...
	"deployController": function (config, soajs, res) {
			...
			BL.model.getEnvironment(soajs, soajs.inputmaskData.envCode, function (err, envRecord) {
				checkIfError(soajs, res, {config: config, error: err || !envRecord, code: 600}, function () {
					checkIfError(soajs, res, {
						...
						getDashDbInfo(BL.model.getDB(), soajs, function (error, data) {
							checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
								...
								BL.model.getGitAccounts(soajs, repoLabel, function (error, accountRecord) {
									checkIfError(soajs, res, {
										...
										BL.model.getService(soajs, {name: 'controller'}, function (error, ctrlRecord) {
											checkIfError(soajs, res, {
												config: config,
												...
												deployControllers(0, soajs.inputmaskData.number, envRecord, dockerParams, function () {
		...
		function deployControllers(counter, max, envRecord, dockerParams, cb) {
			...
			checkIfError(soajs, res, {
				...
				soajs.log.debug("Calling create controller container:", JSON.stringify(deployerConfig), JSON.stringify(dockerParams));
				deployer.createContainer(deployerConfig, dockerParams, BL.model.getDB(), function (error, data) {
					checkIfError(soajs, res, {
						config: config,
						error: error,
						...
						soajs.log.debug("Controller Container Created, starting container:", JSON.stringify(deployerConfig), JSON.stringify(data));
						deployer.start(deployerConfig, data.Id, BL.model.getDB(), function (error, data) {
							checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
								registerNewControllerHost(data, deployerConfig, function () {
									...
									// checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
									soajs.log.debug("Controller Container started, saving information in core_provision", JSON.stringify(data));
				...
				checkIfError(soajs, res, {
					...
					getDashDbInfo(BL.model.getDB(), soajs, function (error, data) {
						checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
							...
							serviceOrig = serviceName = soajs.inputmaskData.name;
							if (soajs.inputmaskData.gcName) {
								serviceName = soajs.inputmaskData.gcName;
								serviceOrig = 'gcs';
							}

							var repoLabel = soajs.inputmaskData.owner + '/' + soajs.inputmaskData.repo;
							BL.model.getGitAccounts(soajs, repoLabel, function (err, accountRecord) {
								checkIfError(soajs, res, {
									config: config,
									...
									BL.model.getService(soajs, {name: serviceName}, function (error, serviceRecord) {
										checkIfError(soajs, res, {
											config: config,
											...
											checkIfError(soajs, res, {
												...
												soajs.log.debug("Calling create service container with params:", JSON.stringify(deployerConfig), JSON.stringify(dockerParams));
												deployer.createContainer(deployerConfig, dockerParams, BL.model.getDB(), function (error, data) {
													checkIfError(soajs, res, {
														config: config,
														error: error,
														...
														soajs.log.debug("Service Container Created, starting container with params:", JSON.stringify(deployerConfig), JSON.stringify(data));
														var info = {
															type: "service",
															name: serviceName
														};
														registerHost(soajs, res, model, config, data, deployerConfig, info, function () {
															...
															deployer.start(deployerConfig, data.Id, BL.model.getDB(), function (error, data) {
																checkIfError(soajs, res, {
																	...
																	soajs.log.debug("Service Container started, saving information in core_provision", JSON.stringify(data));
																	updateHostWithNetworkInfo(soajs, res, model, config, data, info, deployerConfig);
																});
															});
	...
	"deployDaemon": function (config, soajs, res) {
		...
		BL.model.getEnvironment(soajs, soajs.inputmaskData.envCode, function (error, envRecord) {
				...
				checkIfError(soajs, res, {
					...
					getDashDbInfo(model, soajs, function (error, data) {
						checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
							...
							daemonOrig = daemonName = soajs.inputmaskData.name;
							if (soajs.inputmaskData.gcName) {
								daemonName = soajs.inputmaskData.gcName;
								daemonOrig = "gcs";
							}
							...
							BL.model.getGitAccounts(soajs, repoLabel, function (error, accountRecord) {
								checkIfError(soajs, res, {
									...
									BL.model.getDaemon(soajs, {name: daemonName}, function (error, daemonRecord) {
										checkIfError(soajs, res, {
											config: config,
											...
											var dockerParams = {
												...
												"variables": [
													...
													"SOAJS_DAEMON_GRP_CONF=" + soajs.inputmaskData.grpConfName,
											...
											checkIfError(soajs, res, {
												...
												soajs.log.debug("Calling create daemon container with params:", JSON.stringify(deployerConfig), JSON.stringify(dockerParams));
												deployer.createContainer(deployerConfig, dockerParams, BL.model.getDB(), function (error, data) {
													checkIfError(soajs, res, {
														config: config,
														error: error,
														...
														soajs.log.debug("Daemon Container Created, starting container with params:", JSON.stringify(deployerConfig), JSON.stringify(data));
														var info = {
															type: "daemon",
															name: daemonName
														};
														registerHost(soajs, res, model, config, data, deployerConfig, info, function () {
															deployer.start(deployerConfig, data.Id, BL.model.getDB(), function (error, data) {
																checkIfError(soajs, res, {
																	...
																	soajs.log.debug("Daemon Container started, saving information in core_provision");
																	//get the ip of the host from hosts
	...
	"listNginx": function (config, soajs, res) {
		BL.model.getContainers(soajs, soajs.inputmaskData.env, 'nginx', function (error, records) {
			checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
