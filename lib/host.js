...
var BL = {
	...
	"addNode": function (config, soajs, res) {
		...
		BL.model.listNodes(soajs, {role: 'manager'}, function (error, managerNodes) {
			checkIfError(soajs, res, {config: config, error: error || managerNodes.length === 0, code: 600}, function () {
	...
	"removeNode": function (config, soajs, res) {
		...
		BL.model.getOneNode(soajs, criteria, function (error, nodeRecord) {
			checkIfError(soajs, res, {config: config, error: error || !nodeRecord, code: 600}, function () {
				BL.model.getEnvironment(soajs, 'DASHBOARD', function (error, envRecord) { //TODO: make dynamic
					checkIfError(soajs, res, {config: config, error: error || !envRecord, code: 600}, function () {
						var deployerConfig = getDeployerConfig(envRecord);
						deployerConfig.envCode = 'DASHBOARD'; //TODO: make dynamic
	...
	"scaleHAService": function (config, soajs, res) {
		...
		BL.model.getServiceContainers(soajs, criteria, function (error, records) {
			checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
				checkIfError(soajs, res, {config: config, error: records.length === soajs.inputmaskData.scale, code: 600}, function () {
