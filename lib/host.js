...
function getMainPath (accountRecord, serviceRecord) {
    if (accountRecord.repos[0].type === 'multi' && serviceRecord.src.main) {
        return (serviceRecord.src.main);
    } else {
        return null;
    }
}
...
module.exports = {
    ...
    "deployController": function (config, mongo, req, res) {
                    ...
                    checkIfError(req, res, {config: config, error: envRecord.deployer.type === 'manual', code: 618}, function () {
                        getDashDbInfo(mongo, function (error, data) {
                            checkIfError(req, res, {config: config, error: error, code: 600}, function () {
                                ...
                                mongo.findOne("github", {'repos.name': repoLabel}, function (error, accountRecord) {
                                    checkIfError(req, res, {config: config, error: error || !accountRecord, code: 600}, function () {
                                        mongo.findOne("services", {name: 'controller'}, function (error, ctrlRecord) {
                                            checkIfError(req, res, {config: config, error: error || !ctrlRecord, code: 600}, function () {
                                                ...
                                                var mainPath = getMainPath(accountRecord, ctrlRecord);
                                                if (mainPath) {
                                                    dockerParams.Cmd[2] = dockerParams.Cmd[2] + ' ' + mainPath;
                                                }
        ...
        function deployControllers(counter, max, envRecord, dockerParams, cb) {
            ...
            deployer.createContainer(deployerConfig, dockerParams, mongo, function (error, data) {
                checkIfError(req, res, {config: config, error: error, code: (error && error.code) || 615}, function () {
                ...
                checkIfError(req, res, {config: config, error: envRecord.deployer.type === 'manual', code: 618}, function () {
                    getDashDbInfo(mongo, function (error, data) {
                        checkIfError(req, res, {config: config, error: error, code: 600}, function () {
                            ...
                            var repoLabel = req.soajs.inputmaskData.owner + '/' + req.soajs.inputmaskData.repo;
                            mongo.findOne("github", {"repos.name": repoLabel}, {token: 1, 'repos.$': 1}, function (err, accountRecord) {
                                checkIfError(req, res, {config: config, error: err || !accountRecord, code: 600}, function () {
                                    mongo.findOne("services", {name: serviceName}, function (error, serviceRecord) {
                                        checkIfError(req, res, {config: config, error: error || !serviceRecord, code: 600}, function () {
                                            var dockerParams = {
                                                "env": req.soajs.inputmaskData.envCode.toLowerCase(),
                                                "name": serviceName,
                                                "image": config.images.services,
                                            ...
                                            var mainPath = getMainPath(accountRecord, serviceRecord);
                                            if (mainPath) {
                                                dockerParams.Cmd[2] = dockerParams.Cmd[2] + ' ' + mainPath;
                                            }
                ...
                checkIfError(req, res, {config: config, error: envRecord.deployer.type === 'manual', code: 618}, function () {
                    getDashDbInfo(mongo, function (error, data) {
                        checkIfError(req, res, {config: config, error: error, code: 600}, function () {
                            ...
                            var repoLabel = req.soajs.inputmaskData.owner + '/' + req.soajs.inputmaskData.repo;
                            mongo.findOne("github", {"repos.name": repoLabel}, {token: 1, 'repos.$': 1}, function (error, accountRecord) {
                                checkIfError(req, res, {config: config, error: error || !accountRecord, code: 600}, function () {
                                    mongo.findOne("daemons", {name: daemonName}, function (error, daemonRecord) {
                                        checkIfError(req, res, {config: config, error: error || !daemonRecord, code: 600}, function () {
                                            ...
                                            //if multi service, get main path
                                            var mainPath = getMainPath(accountRecord, daemonRecord);
                                            if (mainPath) {
                                                dockerParams.Cmd[2] = dockerParams.Cmd[2] + ' ' + mainPath;
                                            }
