	...
	"redeployService": function(config, soajs, res){
		...
		function getServiceHost(env, cb){
			BL.model.getOneHost(soajs, env, soajs.inputmaskData.name, soajs.inputmaskData.ip, soajs.inputmaskData.hostname, function (error, hostRecord) {
				checkIfError(soajs, res, {config: config, error: error || !hostRecord, code: 600}, function () {
		...
		function deployerUpdateServiceCode(envRecord, containerRecord, hostRecord, cb){
			...
			if(containerRecord.info.Config.Env && containerRecord.info.Config.Env.length > 0){
				for(var i = containerRecord.info.Config.Env.length -1; i >= 0; i--){
					if(containerRecord.info.Config.Env[i].indexOf("SOAJS_GIT_BRANCH") !== -1){
						containerRecord.info.Config.Env.splice(i, 1);
					}
				}
			}
			...
			var params = {
				...
				"Cmd": [
					'bash',
					'-c',
					'./soajsDeployer.sh -T service -X redeploy -G ' + containerRecord.provider
				]
			};
			...
			if (Array.isArray(params.variables) && params.variables.length > 0) {
				var commands = [];
				params.variables.forEach(function(oneVariable){
					commands.push('export ' + oneVariable + " && ");
				});
				params.Cmd[2] = commands.join("") + params.Cmd[2];
			}
			params.Cmd[2] = "cd /opt/soajs/FILES/deployer/; " + params.Cmd[2];

			console.log("====================================");
			console.log(JSON.stringify(params, null, 2));
			console.log("====================================");

			deployer.exec(deployerConfig, containerRecord.cid, BL.model.getDB(), params, function(error, exec){
