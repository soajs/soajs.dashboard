...
function checkReturnError(soajs, res, data, cb) {
	if (data.error) {
...
var BL = {
	...
	"list": function (config, soajs, res) {
		data.getEnvironment(soajs, BL.model, soajs.inputmaskData.env, function (err, envRecord) {
			checkReturnError(soajs, res, {config: config, error: err || !envRecord, code: 600}, function () {

				data.getHosts(soajs, BL.model, envRecord.code, null, function (err, hosts) {
					checkReturnError(soajs, res, {config: config, error: err, code: 600}, function () {
	...
	"listHostEnv": function (config, soajs, res) {
		if (process.env.SOAJS_DEPLOY_HA) {
			...
			BL.model.findEntries(soajs, opts, function (error, environments) {
				checkReturnError(soajs, res, {config: config, error: error, code: 600}, function () {
					...
					async.each(environments, function (oneEnv, mCb) {
						...
						checkReturnError(soajs, res, {config: config, error: !options, code: 825}, function () {
							options.params = {
								env: oneEnv.code.toLowerCase()
							};
							BL.myDeployer.listServices(options, function (error, services) {
								checkReturnError(soajs, res, {config: config, error: error}, function () {
			...
			data.getHostEnv(soajs, BL.model, function (error, envs) {
				checkReturnError(soajs, res, {config: config, error: error, code: 600}, function () {
					checkReturnError(soajs, res, {config: config, error: envs.length === 0, code: 621}, function () {
	...
	"maintenanceOperation": function (config, soajs, res) {
			...
			checkReturnError(soajs, res, {
				...
				if (soajs.inputmaskData.serviceName === 'controller') {
					...
					data.getService(soajs, BL.model, {
						...
						checkReturnError(soajs, res, {config: config, error: error, code: 603}, function () {
							if (!record) {
		...
		function checkServiceHost() {
			data.getOneHost(soajs, BL.model, soajs.inputmaskData.env, soajs.inputmaskData.serviceName, soajs.inputmaskData.serviceHost, soajs.inputmaskData.hostname, function (error, record) {
				checkReturnError(soajs, res, {config: config, error: error, code: 603}, function () {
					checkReturnError(soajs, res, {config: config, error: !record, code: 605}, function () {
