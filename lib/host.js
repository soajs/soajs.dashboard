	...
	"scaleHAService": function (config, soajs, res) {
		...
		var scale = {
			...
			up: function (dockerRecords, serviceInfo, deployerConfig, cb) {
				var options = {
					dockerRecords: dockerRecord,
					serviceInfo: serviceInfo
				};

				deployer.getNewInstance(soajs, deployerConfig, options, BL.model, function (newInstances) {
					var newServiceInfo = {
						tasks: newInstances
					};
					var serviceType = ((soajs.inputmaskData.name.indexOf('nginx') !== -1) ? 'nginx' : 'service'); //TODO: change
				...
				// 	return findAndSaveContainers(deployerConfig, soajs, data, serviceInfo, serviceType, soajs.inputmaskData.envCode, cb);
				// });
			},
