							...
							deployer.listServices(options, function (error, services) {
								checkIfError(soajs, res, {config: config, error: error}, function () {
									...
									for(var i =0; i < services.length; i++){
										...
										if(soajs.inputmaskData.service === oneService.labels["soajsCore.service.name"]){
											if(soajs.inputmaskData.version){
												if(soajs.inputmaskData.version === oneService.labels["soajsCore.service.version"]){
									...
									services.forEach(function(oneService){
										if(output[oneEnv.code.toLowerCase()] && oneService.labels['soajsCore.service.group'] === 'nginx' && oneService.labels['soajsCore.env.code'] === oneEnv.code.toLowerCase()){
