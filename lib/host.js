...
var BL = {
	...
	"getContainerPackages": function (config, soajs, res) {
		BL.model.getEnvironment(soajs, soajs.inputmaskData.env, function (error, envRecord) {
			checkIfError(soajs, res, {config: config, error: error || !envRecord, code: 600}, function () {
				checkIfError(soajs, res, {
					config: config,
					...
					BL.model.getOneContainer(soajs, envRecord.code, soajs.inputmaskData.cid, function (error, dockerRecord) {
						checkIfError(soajs, res, {config: config, error: error || !dockerRecord, code: 600}, function () {
							var dockerParams = {
								...
								"Cmd": [
									'bash',
									'-c',
									'cd /opt/soajs/node_modules/*/ && /usr/bin/npm list --json > packages.txt && pwd'
								]
							};

							deployer.exec(dockerRecord.deployer, dockerRecord.cid, BL.model.getDB(), dockerParams, function (error, exec) {
								checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
									exec.start({hijack: true, stdin: true}, function (error, stream) {
										...
										stream.on('readable', function () {
											...
											while ((chunk = handle.read()) != null) {
												repoPath += chunk.toString("utf8");
											}
										});

										stream.on('end', function () {
											stream.destroy();
											repoPath = repoPath.substring(repoPath.indexOf('/')).replace(/\\u[0-9]{4}|#|\n/g, '');
											checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
												deployer.getFile(dockerRecord.deployer, dockerRecord.cid, soajs, repoPath + '/packages.txt', BL.model.getDB(), function (error, data) {
													checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
														return res.jsonp(soajs.buildResponse(null, {packages: data}));
													});
												});
											});
										});
