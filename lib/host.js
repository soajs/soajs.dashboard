...
var BL = {
    ...
    "nginx": function (config, soajs, deployNewNginx, res) {
        ...
        function constructDeployerParams(customData, oldVariables) {
            ...
            if (soajs.inputmaskData.haService) {
                ...
                var controllerServiceName = context.controllers[0].serviceName;
            ...
            checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
                checkIfError(soajs, res, {config: config, error: envs.length === 0, code: 621}, function () {
                    async.map(envs, function (oneEnv, callback) {
                        return callback(null, oneEnv.env.toUpperCase());
                    }, function (error, envsList) {
                    	data.getEnvInfo(soajs, BL.model, {envList: envsList}, function (error, result) {
		                    checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
			                    var response = {};
		                    	for(var i=0; i<result.length; i++){
			                    	response[result[i].code.toLowerCase()]=result[i].apiPrefix+"."+result[i].domain;
			                    }
    ...
    "addNode": function (config, soajs, res) {
        var nodeInfo = {}, swarmPort, deployerConfig = {};
        ...
        function addNodeToSwarm (envRecord, cb) {
            deployerConfig = getDeployerConfig(envRecord);
        ...
        function saveNodeRecord (managerNodes, node, cb) {
            var options = {
                ...
                node: node,
                swarmPort: swarmPort || null
            };

            deployer.buildNodeRecord(soajs, deployerConfig, options, BL.model, function (record) {
            ...
            getManagerNodes(function (managerNodes) {
                buildApiPayload(managerNodes, function () {
                    getEnv(function (envRecord) {
                        addNodeToSwarm(envRecord, function (node, nodeIndo) {
    ...
    "scaleHAService": function (config, soajs, res) {
        ...
        var scale = {
            ...
            up: function (dockerRecords, serviceInfo, deployerConfig, cb) {
                var options = {
                    dockerRecords: dockerRecords,
                    serviceInfo: serviceInfo
                };

                deployer.getNewInstances(soajs, deployerConfig, options, BL.model, function (newInstances) {
                    var newServiceInfo = {
                    ...
                    var serviceType = '';
                    if ((soajs.inputmaskData.name.indexOf('nginx') !== -1)) serviceType = 'nginx';
                    else if ((soajs.inputmaskData.name.indexOf('controller') !== -1)) serviceType = 'controller';
                    else serviceType = 'service';

                    return findAndSaveContainers(deployerConfig, soajs, data, newServiceInfo, serviceType, soajs.inputmaskData.envCode, cb);
                });
            },
            ...
            down: function (dockerRecords, serviceInfo, deployerConfig, cb) {
                ...
                deployer.getRemovedInstances(soajs, deployerConfig, options, BL.model, function (rmInstances) {
                    async.each(rmInstances, function (oneInstance, callback) {
                        data.removeContainerByTask(soajs, BL.model, soajs.inputmaskData.envCode, oneInstance.taskName, function (error) {
                            if (error) {
                            ...
                            data.removeHostByTask(soajs, BL.model, soajs.inputmaskData.envCode, oneInstance.taskName, callback);
                        });
                    }, cb);
                });
            }
    ...
    "deleteHAService": function (config, soajs, res) {
        data.getEnvironment(soajs, BL.model, soajs.inputmaskData.env, function (error, envRecord) {
            checkIfError(soajs, res, {config: config, error: error || !envRecord, code: 402}, function () {
                data.getServiceType(soajs, BL.model, {env: soajs.inputmaskData.env, serviceName: soajs.inputmaskData.name}, function (error, serviceType) {
                    checkIfError(soajs, res, {config: config, error: error, code: 813}, function () {
                        ...
                        options.serviceType = serviceType.type;
