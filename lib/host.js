...
function findAndSaveContainers(deployerConfig, soajs, model, tasks, serviceType, env, cb) {
	async.map(tasks, function (oneInstance, callback) {
		...
		deployer.inspectContainer(deployerConfig, options, model.getDB(), function (error, containerInfo) {
			...
			var labels = Object.keys(containerInfo.HostConfig.Config.Labels);
			labels.forEach(function (oneLabel) {
				containerInfo.HostConfig.Config.Labels[oneLabel.replace(/\./g, '-')] = containerInfo.HostConfig.Config.Labels[oneLabel];
				delete containerInfo.HostConfig.Config.Labels[oneLabel];
			});
						...
						initDeployer(function(){
							if (!soajs.inputmaskData.haService) {
								...
								createHAService(function () {
									inspectService(context.deployerConfig, {serviceName: context.haServiceParams.Name}, model, soajs.inputmaskData.haCount, function (error, serviceInfo) {
										checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
					...
					if (!soajs.inputmaskData.haService) {
						...
						getCustomUIConfig(function (error, customData) {
							initializeDeployer(function () {
								...
								context.haServiceParams = haServiceParams;
								createHAService(function () {
									inspectService(context.deployerConfig, {serviceName: context.haServiceParams.Name}, BL.model, soajs.inputmaskData.haCount, function (error, serviceInfo) {
										checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
		...
		BL.model.listNodes(soajs, {role: 'manager'}, function (error, managerNodes) {
			checkIfError(soajs, res, {config: config, error: error || managerNodes.length === 0, code: 800}, function () {
				async.map(managerNodes, function (oneNode, callback) {
					...
					BL.model.getEnvironment(soajs, 'DASHBOARD', function (error, envRecord) { //TODO: make dynamic
						checkIfError(soajs, res, {config: config, error: error || !envRecord, code: 402}, function () {
							...
							deployerConfig.envCode = 'DASHBOARD'; //TODO: make dynamic
								...
								deployer.addNode(deployerConfig, nodeInfo, BL.model.getDB(), function (error, node) {
									checkIfError(soajs, res, {config: config, error: error, code: 801}, function () {
										...
										var record = {
						                    ...
						                    role: node.Spec.Role,
						                    resources: {
						                        cpuCount: (node.Description.Resources.NanoCPUs / 1000000000).toFixed(2),
										...
										if (record.role === 'manager') {
											...
											record.tokens = managerNodes[0].tokens;
										}
										else {
