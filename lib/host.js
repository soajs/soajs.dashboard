							...
							deployer.listServices(options, function (error, services) {
								checkIfError(soajs, res, {config: config, error: error}, function () {
									...
									services.forEach(function(oneService){
										if(oneService.labels['soajs.service.group'] === 'nginx' && oneService.labels['soajs.env.code'] === oneEnv.code.toLowerCase()){
											output[oneEnv.code.toLowerCase()] += ":" + oneService.ports[0].published;
										}
									});
