...
function updateHostWithNetworkInfo(req, res, mongo, config, data, info, deployerConfig) {
	...
	if (req.soajs.inputmaskData.grpConfName) {
		newHost.grpConfName = req.soajs.inputmaskData.grpConfName;
	}
	...
	mongo.insert(colName, newHost, function (error) {
		checkIfError(req, res, {config: config, error: error, code: 615}, function () {
			mongo.find(colName, {
				...
				checkIfError(req, res, {config: config, error: error, code: 600}, function () {
					mongo.update("docker", {"cid": data.Id}, {'$set': {"info": data}}, {
						multi: false,
						upsert: false,
						safe: true
					}, function (error) {
...
var model = require("../models/host.js");
...
module.exports = {
	...
	"list": function (config, mongo, req, res) {
		model.getEnvironment(mongo, req.soajs.inputmaskData.env, function (err, envRecord) {
			// mongo.findOne(envColl, {code: req.soajs.inputmaskData.env.toUpperCase()}, function (err, envRecord) {
			checkIfError(req, res, {config: config, error: err || !envRecord, code: 600}, function () {
				model.getHosts(mongo, envRecord.code, null, function (err, hosts) {
					// mongo.find(colName, {env: req.soajs.inputmaskData.env.toLowerCase()}, function (err, hosts) {
					checkIfError(req, res, {config: config, error: err, code: 600}, function () {
						model.getContainers(mongo, envRecord.code, null, null, function (err, containers) {
			...
			checkIfError(req, res, {config: config, error: err || !envRecord, code: 600}, function () {
				if (envRecord.deployer.type === 'manual') {
					...
					mongo.findOne("docker", dockerColCriteria, function (error, dockerRecord) {
						checkIfError(req, res, {config: config, error: error || !dockerRecord, code: 600}, function () {
	...
	"deleteContainer": function (config, mongo, req, res) {
		//should be used in case container has no host record such as nginx containers and zombie containers
		mongo.findOne(envColl, {code: req.soajs.inputmaskData.env.toUpperCase()}, function (error, envRecord) {
			checkIfError(req, res, {config: config, error: error || !envRecord, code: 773}, function () {
				checkIfError(req, res, {
					config: config,
					error: envRecord.deployer.type === 'manual',
					code: 619
					...
					mongo.findOne('docker', criteria, function (error, dockerRecord) {
						checkIfError(req, res, {config: config, error: error || !dockerRecord, code: 773}, function () {
								...
								mongo.count('docker', {type: 'nginx'}, function (error, count) {
									checkIfError(req, res, {config: config, error: error, code: 773}, function () {
										checkIfError(req, res, {
											...
											removeDockerContainer(dockerRecord, criteria);
										});
									});
								});
							}
		...
		function removeDockerContainer(dockerRecord, criteria) {
			var deployerConfig = dockerRecord.deployer;
			deployer.remove(deployerConfig, dockerRecord.cid, mongo, function (error) {
				if (error) {
					req.soajs.log.error(error);
				}
				else {
					req.soajs.log.debug("Container has been deleted from " + req.soajs.inputmaskData.env + " environment");
				}

				mongo.remove('docker', criteria, function (error) {
					checkIfError(req, res, {config: config, error: error, code: 600}, function () {
						req.soajs.log.debug("Container has been removed from docker collection");
						return res.jsonp(req.soajs.buildResponse(null, true));
					});
				});
			});
