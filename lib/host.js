	...
	mongo.insert(colName, newHost, function (error) {
		checkIfError(req, res, {config: config, error: error, code: 615}, function () {
			mongo.find(colName, {
				...
				checkIfError(req, res, {config: config, error: error, code: 600}, function () {
					mongo.update("docker", {"cid": data.Id}, {'$set': {"info": data}}, {multi: false, upsert: false, safe: true}, function (error) {
						checkIfError(req, res, {config: config, error: error, code: 600}, function () {
							...
							if (dockerRecord.type === 'nginx') {
								mongo.count('docker', {type: 'nginx'}, function (error, count) {
									checkIfError(req, res, {config: config, error: error, code: 773}, function () {
										checkIfError(req, res, {config: config, error: count < 2, code: 777}, function () {
