...
function reloadControllerRegistry(soajs, BL, envRecord, cb) {
	BL.model.getHosts(soajs, envRecord.code, 'controller', function (error, ctrlRecords) {
...
function inspectService(deployerConfig, options, dataSource, serviceCount, cb) {
	deployer.inspectHAService(deployerConfig, options, dataSource, function (error, serviceInfo) {
		if (error) {
			return cb(error);
		}
		...
		if (runningTasks.length !== serviceCount) {
			//waiting until all new tasks are available
			setTimeout(function () {
				return inspectService(deployerConfig, options, dataSource, serviceCount, cb);
			}, 500);
		}
		else {
...
function findAndSaveContainers(deployerConfig, soajs, model, tasks, serviceType, env, cb) {
	async.map(tasks, function (oneInstance, callback) {
		...
		deployer.inspectContainer(deployerConfig, options, model.getDB(), function (error, containerInfo) {
			if (error) {
		...
		model.insertContainers(soajs, results, cb);
	});
}
						...
						initDeployer(function(){
							if (!soajs.inputmaskData.haService) {
								...
								createHAService(function () {
									inspectService(context.deployerConfig, {serviceName: context.haServiceParams.Name}, model.getDB(), soajs.inputmaskData.haCount, function (error, serviceInfo) {
										checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
											var serviceType = ((soajs.customData && soajs.customData.type) ? soajs.customData.type : 'service');
											findAndSaveContainers(context.deployerConfig, soajs, model, serviceInfo.tasks, serviceType, context.dockerParams.env, function (error) {
												checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
													model.getHosts(soajs, soajs.inputmaskData.envCode, "controller", function (error, controllers) {
														checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
...
var BL = {
					...
					if (!soajs.inputmaskData.haService) {
						...
						getCustomUIConfig(function (error, customData) {
							initializeDeployer(function () {
								...
								context.haServiceParams = haServiceParams;
								createHAService(function () {
									inspectService(context.deployerConfig, {serviceName: context.haServiceParams.Name}, BL.model.getDB(), soajs.inputmaskData.haCount, function (error, serviceInfo) {
										checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
											findAndSaveContainers(context.deployerConfig, soajs, BL.model, serviceInfo.tasks, 'nginx', context.dockerParams.env, function (error) {
												checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
							...
							checkIfError(soajs, res, {config: config, error: err, code: 600}, function () {
								...
								hosts.forEach(function (oneHost) {
									containers.forEach(function (oneContainer) {
										if (oneHost.serviceHATask.match(new RegExp(oneContainer.serviceName + '.[0-9]+', 'g'))) {
			...
			BL.model.listNodes(soajs, {role: 'manager'}, function (error, managerNodes) {
				checkIfError(soajs, res, {config: config, error: error || managerNodes.length === 0, code: 800}, function () {
					async.map(managerNodes, function (oneNode, callback) {
						...
						var swarmPort = managerNodes[0].swarmPort; //use same swarm port as standard for all nodes
						...
						BL.model.getEnvironment(soajs, soajs.inputmaskData.env, function (error, envRecord) {
							checkIfError(soajs, res, {config: config, error: error || !envRecord, code: 402}, function () {
								...
								checkIfError(soajs, res, {config: config, error: !deployerConfig, code: 743}, function () {
									...
									deployer.addNode(deployerConfig, nodeInfo, BL.model.getDB(), function (error, node) {
										checkIfError(soajs, res, {config: config, error: error, code: ((error && error.code) ? error.code : 801)}, function () {
											...
											BL.model.addNode(soajs, record, function (error) {
												checkIfError(soajs, res, {config: config, error: error, code: 802}, function () {
													//if manager node, add to list of nodes for deployer object of all environmentsConfig
													if (record.role === 'manager') {
														BL.model.updateDockerDeployerNodes(soajs, 'add', record.name, function (error) {
															checkIfError(soajs, res, {config: config, error: error, code: 819}, function () {
																return res.jsonp(soajs.buildResponse(null, true));
															});
														});
													}
	...
	"removeNode": function (config, soajs, res) {
		checkIfError(soajs, res, {config: config, error: soajs.inputmaskData.env.toLowerCase() !== 'dashboard', code: 818}, function () {
			...
			BL.model.getOneNode(soajs, criteria, function (error, nodeRecord) {
				checkIfError(soajs, res, {config: config, error: error || !nodeRecord, code: 803}, function () {
					checkIfError(soajs, res, {config: config, error: nodeRecord.role === 'manager', code: 821}, function () {
						BL.model.getEnvironment(soajs, soajs.inputmaskData.env, function (error, envRecord) {
							checkIfError(soajs, res, {config: config, error: error || !envRecord, code: 402}, function () {
								var deployerConfig = getDeployerConfig(envRecord);
								deployerConfig.envCode = soajs.inputmaskData.env;

								deployer.removeNode(deployerConfig, nodeRecord, BL.model.getDB(), function (error) {
									checkIfError(soajs, res, {config: config, error: error, code: ((error && error.code) ? error.code : 804)}, function () {
										BL.model.removeNode(soajs, criteria, function (error) {
											checkIfError(soajs, res, {config: config, error: error, code: 805}, function () {
												if (nodeRecord.role === 'manager') {
													BL.model.updateDockerDeployerNodes(soajs, 'remove', nodeRecord.name, function (error) {
				...
				checkIfError(soajs, res, {config: config, error: error || nodes.length === 0, code: 803}, function () {
					checkIfError(soajs, res, {config: config, error: nodes.length === 1, code: 823}, function () {
						async.detect(nodes, function (oneNode, callback) {
							return callback(null, (oneNode.id === soajs.inputmaskData.nodeId));
						}, function (error, nodeRecord) {
							//no need to handle above error, it does not exist
							...
							BL.model.getEnvironment(soajs, soajs.inputmaskData.env, function (error, envRecord) {
								checkIfError(soajs, res, {config: config, error: error || !envRecord, code: 402}, function () {
									...
									deployer.updateNode(deployerConfig, options, BL.model.getDB(), function (error, result) {
										checkIfError(soajs, res, {config: config, error: error, code: ((error && error.code) ? error.code : 806)}, function () {
											BL.model.updateNode(soajs, criteria, update, function (error) {
												checkIfError(soajs, res, {config: config, error: error, code: 807}, function () {
													if (soajs.inputmaskData.type === 'role') {
														...
														BL.model.updateDockerDeployerNodes(soajs, action, nodeRecord.name, function (error) {
	...
	"scaleHAService": function (config, soajs, res) {
		...
		BL.model.getServiceContainers(soajs, criteria, function (error, records) {
			checkIfError(soajs, res, {config: config, error: error || records.length === 0, code: 808}, function () {
				checkIfError(soajs, res, {config: config, error: records.length === soajs.inputmaskData.scale, code: 809}, function () {
					var scaling = (soajs.inputmaskData.scale > records.length) ? 'up' : 'down';
					BL.model.getEnvironment(soajs, soajs.inputmaskData.env.toUpperCase(), function (error, envRecord) {
						checkIfError(soajs, res, {config: config, error: error, code: 402}, function () {
							...
							deployer.scaleHAService(deployerConfig, options, BL.model.getDB(), function (error, result) {
								checkIfError(soajs, res, {config: config, error: error, code: ((error && error.code) ? error.code : 810)}, function () {
									inspectService(deployerConfig, options, BL.model.getDB(), soajs.inputmaskData.scale, function (error, serviceInfo) {
										checkIfError(soajs, res, {config: config, error: error, code: 811}, function () {
											scale[scaling](records, serviceInfo, deployerConfig, function (error) {
												checkIfError(soajs, res, {config: config, error: error, code: 812}, function () {
													if (scaling === 'down') {
														reloadControllerRegistry(soajs, BL, envRecord, function () {
															return res.jsonp(soajs.buildResponse(null, true));
														});
													}
													else {
		...
		var scale = {
			...
			up: function (dockerRecords, serviceInfo, deployerConfig, cb) {
				...
				async.each(serviceInfo.tasks, function (oneTask, callback) {
					...
					return findAndSaveContainers(deployerConfig, soajs, BL.model, newInstances, serviceType, soajs.inputmaskData.env, cb);
				});
			},
			...
			down: function (dockerRecords, serviceInfo, deployerConfig, cb) {
				...
				async.each(dockerRecords, function (oneRecord, callback) {
					...
					async.each(rmInstances, function (oneInstance, callback) {
						BL.model.removeContainerByTask(soajs, soajs.inputmaskData.env, oneInstance.taskName, function (error) {
							if (error) {
								return callback(error);
							}
							...
							BL.model.removeHostByTask(soajs, soajs.inputmaskData.env, oneInstance.taskName, callback);
						});
					}, cb);
				});
			}
	...
	"deleteHAService": function (config, soajs, res) {
		BL.model.getEnvironment(soajs, soajs.inputmaskData.env, function (error, envRecord) {
			checkIfError(soajs, res, {config: config, error: error || !envRecord, code: 402}, function () {
				...
				deployer.deleteHAService(deployerConfig, options, BL.model.getDB(), function (error, result) {
					checkIfError(soajs, res, {config: config, error: error, code: ((error && error.code) ? error.code : 813)}, function () {
						criteria = {
							serviceName: soajs.inputmaskData.name
						};
		...
		function removeDockerRecords(criteria, cb) {
			BL.model.removeServiceContainers(soajs, criteria, cb);
		}

		function removeHostRecords(criteria, cb) {
			BL.model.removeServiceHosts(soajs, criteria, cb);
		}
	},
	...
	"streamLogs": function (config, soajs, res) {
		BL.model.getEnvironment(soajs, soajs.inputmaskData.env, function (error, envRecord) {
			checkIfError(soajs, res, {config: config, error: error, code: 402}, function () {
				...
				deployer.inspectHATask(deployerConfig, options, BL.model.getDB(), function (error, taskInfo) {
					checkIfError(soajs, res, {config: config, error: error || !taskInfo, code: ((error && error.code) ? error.code : 811)}, function () {
						options = {
							nodeId: taskInfo.NodeID,
							containerId: taskInfo.Status.ContainerStatus.ContainerID
						};

						deployer.getContainerLogs(deployerConfig, options, BL.model.getDB(), soajs, res);
					});
				});
			});
		});
