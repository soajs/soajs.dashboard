	...
	"listHostEnv": function (config, soajs, res) {
		...
		function checkACL(){
			...
			if(!ACL){
				if(soajs.tenant.application.acl_all_env && Object.keys(soajs.tenant.application.acl_all_env).length > 0){
					ACL = soajs.tenant.application.acl_all_env[process.env.SOAJS_ENV.toLowerCase()];
				}
					...
					ACL = soajs.tenant.application.package_acl_all_env[process.env.SOAJS_ENV.toLowerCase()];
				}
				else{
		...
		var _api = {
			"checkPermission": function (system, myGroup, route, method) {
				if(system[method] && Object.keys(system[method]).length > 0){
