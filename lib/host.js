...
var helpers = require("./helpers/host.js");
...
var BL = {
	...
	"list": function (config, soajs, res, cbMain) {
	 ...
	 * @ param response
	 */
	"listHostEnv": function (config, soajs, res, cbMain) {
		if (process.env.SOAJS_DEPLOY_HA) {
			BL.listHAhostEnv(config, soajs, cbMain);
		}
		else {
			data.getHostEnv(soajs, BL.model, function (error, envs) {
				checkReturnError(soajs, cbMain, {config: config, error: error, code: 600}, function () {
					checkReturnError(soajs, cbMain, {config: config, error: envs.length === 0, code: 621}, function () {
	...
	"listHAhostEnv": function (config, soajs, cbMain) {
		var output = {};
	...
	"maintenanceOperation": function (config, soajs, res, cbMain) {
