...
function deployContainerServiceOrDaemon(config, soajs, res, model, type) {
	var name, origin;
	var modelMethod = (type === 'service') ? "getService": "getDaemon";
			...
			checkIfError(soajs, res, {
				...
				getDashDbInfo(model, soajs, function (error, data) {
					checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
						...
						origin = name = soajs.inputmaskData.name;
						if (soajs.inputmaskData.gcName && type === 'daemon') {
							name = soajs.inputmaskData.gcName;
							origin = "gcs";
						}
						...
						model.getGitAccounts(soajs, repoLabel, function (error, accountRecord) {
							checkIfError(soajs, res, {
								...
								model[modelMethod](soajs, {name: name}, function (error, dbRecord) {
									checkIfError(soajs, res, {
										config: config,
										...
										if (dbRecord.src && dbRecord.src.cmd) {
											if (Array.isArray(dbRecord.src.cmd) && dbRecord.src.cmd.length > 0) {
												var commands = dbRecord.src.cmd.join("; ");
												dockerParams.Cmd[2] = commands + "; " + dockerParams.Cmd[2];
											}
										}

										if (dbRecord.src && dbRecord.src.main) {
											dockerParams.Cmd[2] = dockerParams.Cmd[2] + ' ' + dbRecord.src.main;
										}
										...
										checkIfError(soajs, res, {
											...
											soajs.log.debug("Calling create container with params:", JSON.stringify(deployerConfig), JSON.stringify(dockerParams));
											deployer.createContainer(deployerConfig, dockerParams, model.getDB(), function (error, data) {
												checkIfError(soajs, res, {
													config: config,
													error: error,
													code: (error && error.code === 741) ? error.code : 615
												}, function () {
													soajs.log.debug("Container Created, starting container with params:", JSON.stringify(deployerConfig), JSON.stringify(data));
													var info = {
														type: type,
														name: name
													};
													registerHost(soajs, res, model, config, data, deployerConfig, info, function () {
														deployer.start(deployerConfig, data.Id, model.getDB(), function (error, data) {
															checkIfError(soajs, res, {
																config: config,
																error: error,
																code: 615
