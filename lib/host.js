	...
	"redeployNginx": function(config, soajs, res){
		...
		function copyCertificates(cb){
			...
			function copyCerts(cb) {
				BL.model.getFiles(soajs, {}, function(error, files){
					checkIfError(soajs, res, { config: config, error: error || !files || files.length === 0, code: 600 }, function () {

						async.each(files, copyOneFile, function(error){
							checkIfError(soajs, res, { config: config, error: error, code: 600 }, function () {
								return cb();
							});
						});
			...
			function copyOneFile(oneFile, cb){
		...
		function reloadContainerAndUpdateDB(cb){
			...
			deployer.exec(context.deployerConfig, context.containerRecord.cid, BL.model.getDB(), context.dockerParams, function (error) {
				checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
					...
					var newPrams = context.containerRecord;
					newPrams.info.Config.Env = context.dockerParams.variables;
					BL.model.updateContainer(soajs, context.containerRecord.cid, newParams, function(error){
						checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
							return cb();
						});
					});
