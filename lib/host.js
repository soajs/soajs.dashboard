...
function deployContainerServiceOrDaemon(config, soajs, res, model, type) {
    ...
    function constructDeployerParams(cb) {
        var serviceParams = {
            ...
            "name": soajs.inputmaskData.envCode.toLowerCase() + "-" + context.name + "-v" + soajs.inputmaskData.version,
            "image": soajs.inputmaskData.imagePrefix + "/soajs",
            "variables": [
                "NODE_ENV=production",
            ...
            "labels": { //very useful when filtering
                ...
                "soajs.env.code": soajs.inputmaskData.envCode.toLowerCase(),
                ...
                "soajs.service.label": soajs.inputmaskData.envCode.toLowerCase() + "-" + context.name + "-v" + soajs.inputmaskData.version
            },
            "cmd": [
                'bash',
                '-c',
            ...
            "memoryLimit": soajs.inputmaskData.memoryLimit,
            "replicaCount": soajs.inputmaskData.haCount,
            ...
            "network": "soajsnet" //TODO: make dynamic or at least set in config file
        };
        ...
        if (type === 'daemon') {
            serviceParams.variables.push("SOAJS_DAEMON_GRP_CONF=" + soajs.inputmaskData.grpConfName);
        }
        ...
        if (soajs.inputmaskData.gcName) {
            serviceParams.variables.push("SOAJS_GC_NAME=" + soajs.inputmaskData.gcName);
            serviceParams.variables.push("SOAJS_GC_VERSION=" + soajs.inputmaskData.gcVersion);
        }
...
var BL = {
    ...
    "nginx": function (config, soajs, res) {
        ...
        function getCustomUIConfig(cb) {
            if (!soajs.inputmaskData.nginxConfig || (soajs.inputmaskData.nginxConfig && !soajs.inputmaskData.nginxConfig.customUIId)) return cb();
        ...
        function constructDeployerParams() {
            var nginxParams = {
                ...
                "name": soajs.inputmaskData.envCode.toLowerCase() + "-nginx",
                "image": soajs.inputmaskData.imagePrefix + "/nginx",
                "variables": [
                ...
                "targetPort": 80, //TODO: make dynamic
                "memoryLimit": soajs.inputmaskData.memoryLimit,
                "replicaCount": soajs.inputmaskData.haCount,
                "containerDir": config.imagesDir,
                "restartPolicy": {
            ...
            if (context.ui) {
                ...
                nginxParams.variables.push("SOAJS_GIT_BRANCH=" + soajs.inputmaskData.nginxConfig.branch);
                nginxParams.variables.push("SOAJS_GIT_COMMIT=" + soajs.inputmaskData.nginxConfig.commit);
                if (context.ui.token) {
    ...
    "redeployHAService": function (config, soajs, res) {
        data.getEnvironment(soajs, BL.model, soajs.inputmaskData.env, function (error, envRecord) {
            checkIfError(soajs, res, {config: config, error: error || !envRecord, code: 402}, function () {
                var options = buildDeployerOptions(envRecord, soajs);
                options.params = {
