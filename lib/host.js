...
// 	};
//
// 	model.insertContainer(soajs, BL.model, document, function (error) {
...
function inspectService(soajs, deployerConfig, options, dataSource, serviceCount, cb) {
	deployer.inspectHAService(soajs, deployerConfig, options, dataSource, function (error, serviceInfo) {
		if (error) {
			return cb(error);
		}
		...
		if (runningTasks.length !== serviceCount) {
			...
			setTimeout(function () {
				return inspectService(soajs, deployerConfig, options, dataSource, serviceCount, cb);
			}, 500);
		}
		else {
...
// 	}
// 	model.insertHost(soajs, BL.model, newHost, function (error) {
// 		checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
//
...
//
// 					model.updateContainer(soajs, BL.model, data.Id, data, function (error) {
// 						checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
//
...
function deployContainerServiceOrDaemon(config, soajs, res, model, type) {
	...
	function constructDeployerParams(cb) {
		var dockerParams = {
			...
			"Cmd": [
				'bash',
				'-c',
				'cd /opt/soajs/FILES/deployer/; ./soajsDeployer.sh -T service -X deploy -g ' + context.accountRecord.providerName + ' -G ' + context.accountRecord.domain
			]
		};
	...
	// 	deployer.createContainer(soajs, context.deployerConfig, context.dockerParams, BL.model, function (error, data) {
	// 		checkIfError(soajs, res, {
	// 			config: config,
	// 			error: error,
	...
	function createHAService(cb) {
		...
		deployer.deployHAService(soajs, context.deployerConfig, context.haServiceParams, BL.model, function (error, data) {
			checkIfError(soajs, res, {
				config: config,
				error: error,
				code: (error && error.code === 741) ? error.code : 615
	...
	// 	deployer.start(soajs, context.deployerConfig, context.data.Id, BL.model, function (error, data) {
	// 		checkIfError(soajs, res, {
	// 			config: config,
	// 			error: error,
	// 			code: 615
						...
						initDeployer(function () {
							...
							// else {
								...
								if (process.env.SOAJS_TEST) {
									...
									context.haServiceParams.TaskTemplate.ContainerSpec.Image = 'alpine:latest';
									context.haServiceParams.TaskTemplate.ContainerSpec.Command = ['sh'];
									context.haServiceParams.TaskTemplate.ContainerSpec.Args = ['-c', 'sleep 36000'];
								}

								createHAService(function () {
									inspectService(soajs, context.deployerConfig, {serviceName: context.haServiceParams.Name}, BL.model, soajs.inputmaskData.haCount, function (error, serviceInfo) {
										checkIfError(soajs, res, {
											config: config,
											error: error,
											code: 615
...
var BL = {
	...
	"nginx": function (config, soajs, deployNewNginx, res) {
		...
		// 			deployer.start(soajs, context.deployerConfig, data.Id, BL.model, function (error, data) {
		// 				checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
		//
		...
		// 						"info": data
		// 					};
		// 					data.insertContainer(soajs, BL.model, document, function (error) {
		// 						checkIfError(soajs, res, {
		// 							config: config,
		// 							error: error,
		// 							code: 615
		...
		// 	deployer.exec(soajs, containerRecord.deployer, containerRecord.cid, BL.model, dockerParams, function (error, exec) {
		// 		checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
		...
		// 					data.updateContainer(soajs, BL.model, containerRecord.cid, newParams.info, function (error) {
		// 						checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
		// 							return cb();
		// 						});
		// 					});
								...
								createHAService(function () {
									inspectService(soajs, context.deployerConfig, {serviceName: context.haServiceParams.Name}, BL.model, soajs.inputmaskData.haCount, function (error, serviceInfo) {
										checkIfError(soajs, res, {
											...
											findAndSaveContainers(context.deployerConfig, soajs, data, serviceInfo.tasks, 'nginx', context.dockerParams.env, function (error) {
												checkIfError(soajs, res, {
													config: config,
													error: error,
													code: 615
	...
	//
	// 	function getDashboardConnection(cb) {
	// 		getDashDbInfo(data, soajs, function (error, data) {
	// 			checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
	// 				context.mongoDbs = data.mongoDbs;
	// 				context.mongoCred = data.mongoCred;
	// 				context.clusterInfo = data.clusterInfo;
	...
	// 	function getGitInfo(cb) {
	// 		var repoLabel = soajs.inputmaskData.owner + '/' + soajs.inputmaskData.repo;
	// 		data.getGitAccounts(soajs, BL.model, repoLabel, function (error, accountRecord) {
	// 			checkIfError(soajs, res, {
	// 				config: config,
	// 				error: error || !accountRecord,
	// 				code: 600
	...
	// 		data.getService(soajs, BL.model, {name: 'controller'}, function (error, dbRecord) {
	// 			checkIfError(soajs, res, {
	// 				config: config,
	// 				error: error || !dbRecord,
	// 				code: 600
	...
	// 			deployer.createContainer(soajs, deployerConfig, dockerParams, BL.model, function (error, data) {
	// 				checkIfError(soajs, res, {
	// 					config: config,
	// 					error: error,
	// 					code: (error && error.code === 741) ? error.code : 615
	...
	// 					deployer.start(soajs, deployerConfig, data.Id, BL.model, function (error, data) {
	// 						checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
	...
	// 		};
	//
	// 		data.insertHost(soajs, BL.model, newHost, function (error) {
	// 			checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
	// 				return cb(null, true);
	// 			});
	// 		});
	...
	// 			}, function () {
	//
	// 				data.getOneContainer(soajs, BL.model, envRecord.code, soajs.inputmaskData.cid, function (error, record) {
	// 					checkIfError(soajs, res, {config: config, error: error || !record, code: 600}, function () {
	//
	// 						deployer.info(soajs, record.deployer, record.cid, res, BL.model);
	// 					});
	// 				});
	// 			});
	// 		});
	...
	// 			}, function () {
	//
	// 				data.getOneContainer(soajs, BL.model, envRecord.code, soajs.inputmaskData.cid, function (error, dockerRecord) {
	// 					checkIfError(soajs, res, {
	// 						config: config,
	// 						error: error || !dockerRecord,
	// 						code: 773
	...
	// 			}
	//
	// 			data.removeContainer(soajs, BL.model, env, soajs.inputmaskData.cid, function (error) {
	// 				checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
	...
	// 			}
	//
	// 			data.getContainers(soajs, BL.model, envRecord.code, {"$ne": "nginx"}, null, function (error, containers) {
	// 				checkIfError(soajs, res, {config: config, error: error, code: 772}, function () {
	//
	// 					data.getHosts(soajs, BL.model, envRecord.code, null, function (error, hosts) {
	// 						checkIfError(soajs, res, {config: config, error: error, code: 772}, function () {
	//
	...
	"addNode": function (config, soajs, res) {
		checkIfError(soajs, res, {
			...
			data.listNodes(soajs, BL.model, {role: 'manager'}, function (error, managerNodes) {
				checkIfError(soajs, res, {
					config: config,
					error: error || managerNodes.length === 0,
					code: 800
				}, function () {
								...
								checkIfError(soajs, res, {
									...
									deployer.addNode(soajs, deployerConfig, nodeInfo, BL.model, function (error, node) {
										checkIfError(soajs, res, {
											config: config,
											error: error,
											code: ((error && error.code) ? error.code : 801)
										}, function () {
											...
											data.addNode(soajs, BL.model, record, function (error) {
												checkIfError(soajs, res, {
													...
													if (record.role === 'manager') {
														data.updateDockerDeployerNodes(soajs, BL.model, 'add', record.name, function (error) {
															checkIfError(soajs, res, {
																config: config,
																error: error,
																code: 819
	...
	"removeNode": function (config, soajs, res) {
		checkIfError(soajs, res, {
			...
			soajs.inputmaskData.env = soajs.inputmaskData.env.toUpperCase();
			data.getOneNode(soajs, BL.model, criteria, function (error, nodeRecord) {
				checkIfError(soajs, res, {config: config, error: error || !nodeRecord, code: 803}, function () {
					checkIfError(soajs, res, {
						config: config,
						error: nodeRecord.role === 'manager',
						code: 821
					}, function () {
						data.getEnvironment(soajs, BL.model, soajs.inputmaskData.env, function (error, envRecord) {
							checkIfError(soajs, res, {
								...
								deployer.removeNode(soajs, deployerConfig, nodeRecord, BL.model, function (error) {
									checkIfError(soajs, res, {
										config: config,
										error: error,
										code: ((error && error.code) ? error.code : 804)
									}, function () {
								...
								checkIfError(soajs, res, {
									...
									deployer.updateNode(soajs, deployerConfig, options, BL.model, function (error, result) {
										checkIfError(soajs, res, {
											config: config,
											error: error,
											code: ((error && error.code) ? error.code : 806)
										}, function () {
						...
						checkIfError(soajs, res, {config: config, error: error, code: 402}, function () {
							...
							deployer.scaleHAService(soajs, deployerConfig, options, BL.model, function (error, result) {
								checkIfError(soajs, res, {
									config: config,
									error: error,
									code: ((error && error.code) ? error.code : 810)
								}, function () {
		...
		var scale = {
			...
			up: function (dockerRecords, serviceInfo, deployerConfig, cb) {
				...
				async.each(serviceInfo.tasks, function (oneTask, callback) {
					...
					return findAndSaveContainers(deployerConfig, soajs, data, newInstances, serviceType, soajs.inputmaskData.env, cb);
				});
			},
			...
			checkIfError(soajs, res, {config: config, error: error || !envRecord, code: 402}, function () {
				...
				deployer.deleteHAService(soajs, deployerConfig, options, BL.model, function (error, result) {
					checkIfError(soajs, res, {
						config: config,
						error: error,
						code: ((error && error.code) ? error.code : 813)
					}, function () {
						criteria = {
							serviceName: soajs.inputmaskData.name
						};
			...
			checkIfError(soajs, res, {config: config, error: error, code: 402}, function () {
				...
				deployer.inspectHATask(soajs, deployerConfig, options, BL.model, function (error, taskInfo) {
					checkIfError(soajs, res, {
						...
						code: ((error && error.code) ? error.code : 811)
					}, function () {
						options = {
