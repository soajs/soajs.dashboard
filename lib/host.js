...
var BL = {
	...
	"scaleHAService": function (config, soajs, res) {
					...
					BL.model.getEnvironment(soajs, 'DASHBOARD', function (error, envRecord) { //TODO: make dynamic
						checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
							...
							deployer.scaleHAService(deployerConfig, options, BL.model.getDB(), function (error, result) {
								checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
									inspectService(deployerConfig, options, BL.model.getDB(), scaling, soajs.inputmaskData.scale, function (error, serviceInfo) {
										checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
		...
		function inspectService(deployerConfig, options, dataSource, scaling, scale, cb) {
			deployer.inspectHAService(deployerConfig, options, dataSource, function (error, serviceInfo) {
				...
				if (scaling === 'up' && runningTasks.length !== scale) {
					//experimental
					//waiting until all new tasks are available
					setTimeout(function () {
						return inspectService(deployerConfig, options, dataSource, scaling, scale, cb);
					}, 500);
				}
				else {
		...
		var scale = {
			...
			up: function (dockerRecords, serviceInfo, deployerConfig, cb) {
				...
				async.each(serviceInfo.tasks, function (oneTask, callback) {
					if (oneTask.Status.State === 'running') { //TODO: check might no longer be needed, added in inspectService()
						var found = false;
					...
					soajs.log.debug('New instances detected: ' + newInstances);
					async.map(newInstances, function (oneInstance, callback) {
						var options = {
							nodeId: oneInstance.NodeID,
							containerId: oneInstance.Status.ContainerStatus.ContainerID
						};
						deployer.inspectContainer(deployerConfig, options, BL.model.getDB(), function (error, containerInfo) {
							if (error) {
								return callback(error);
							}

							var newRecord = {
								type: ((soajs.inputmaskData.name.indexOf('nginx') !== -1) ? 'nginx' : 'service'),
						...
						soajs.log.debug('Inserting new records into docker collection');
						soajs.log.debug(JSON.stringify(results, null, 2));
						BL.model.insertContainers(soajs, results, cb);
					});
				});
			},
