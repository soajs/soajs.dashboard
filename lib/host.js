...
function deployNginx(config, mongo, req, res) {
    ...
    function rebuildNginx(envRecord) {
        getRunningControllers(function (controllers) {
            var dockerParams = {
                ...
                "variables": [
                    ...
                    "SOAJS_NX_API_DOMAIN=dashboard-api." + envRecord.domain, //mydomain.com,

                    "SOAJS_NX_SITE_DOMAIN=dashboard." + envRecord.domain,
                    "SOAJS_NX_SITE_PORT=" + envRecord.port,
                    "SOAJS_NX_SITE_PATH=" + req.soajs.inputmaskData.sitePath
                ],
                "Cmd": [
                    'bash',
                    '-c',
            ...
            getDashUIConfigIfApplicable(function (error, dashData) {
                checkIfError(req, res, {config: config, error: error, code: 600}, function () {
                    if (dashData) {
                        dockerParams.variables.push("SOAJS_GIT_DASHBOARD_BRANCH=" + dashData.branch);
                    }

                    getCustomUIConfigIfApplicable(function (error, customData) {
                        checkIfError(req, res, {config: config, error: error, code: 600}, function () {
                            if (customData) {
                                dockerParams.variables.push("SOAJS_GIT_REPO=" + customData.repo);
                                dockerParams.variables.push("SOAJS_GIT_OWNER=" + customData.owner);
                                dockerParams.variables.push("SOAJS_GIT_BRANCH=" + customData.branch);
                                if (customData.token) {
        ...
        function getDashUIConfigIfApplicable (cb) {
            if (req.soajs.inputmaskData.nginxConfig.useDashUI) {
                mongo.findOne('services', {'name': 'dashboard'}, {'src': 1}, function (error, dashSrc) {
                    cb (error, dashSrc.src);
                });
            } else {
                cb (null, null);
            }
        ...
        function getCustomUIConfigIfApplicable (cb) {
            if (req.soajs.inputmaskData.nginxConfig.customUIId) {
...
function registerHost (req, res, mongo, config, data, deployerConfig, info) {
    var document = {
        "cid": data.Id,
        "env": req.soajs.inputmaskData.envCode.toLowerCase(),
    ...
    mongo.insert("docker", document, function (error) {
        checkIfError(req, res, {config: config, error: error, code: 615}, function () {
            ...
            if (data.NetworkSettings && data.NetworkSettings.Networks && data.NetworkSettings.Networks[deployerConfig.config.HostConfig.NetworkMode]) {
                newHost.ip = data.NetworkSettings.Networks[deployerConfig.config.HostConfig.NetworkMode].IPAddress; //ip address location for dockermachine deployment
            } else {
                newHost.ip = data.NetworkSettings.IPAddress; //ip address location for docker deployment and test cases
            }
                    ...
                    mongo.find(colName, {
                        ...
                        checkIfError(req, res, {config: config, error: error, code: 600}, function () {
                            ...
                            return res.json(req.soajs.buildResponse(null, {
                                'cid': data.Id,
                                'hostname': document.hostname, //data.Config.Hostname,
                                "ip": newHost.ip,
            ...
            mongo.findOne("environment", {code: req.soajs.inputmaskData.envCode.toUpperCase()}, function (err, envRecord) {
                checkIfError(req, res, {config: config, error: err || !envRecord, code: 600}, function () {
                    checkIfError(req, res, {config: config, error: envRecord.deployer.type === 'manual', code: 618}, function () {
                        ...
                        var mongoDbs = envRecord.dbs.clusters[clusterName].servers;
                        var clusterInfo = envRecord.dbs.clusters[clusterName];

                        mongo.findOne("services", {'name': 'controller'}, function (error, ctrlRecord) {
                            checkIfError(req, res, {config: config, error: error || !ctrlRecord, code: 600}, function () {
                                checkIfError(req, res, {
                                    config: config,
                                    error: !ctrlRecord.src || !ctrlRecord.src.type || !ctrlRecord.src.owner || !ctrlRecord.src.repo || !ctrlRecord.src.branch || !ctrlRecord.src.main,
                                    code: 741
                                }, function () {
                                    var dockerParams = {
                                        ...
                                        "variables": [
                                            ...
                                            "SOAJS_GIT_OWNER=" + ctrlRecord.src.owner,
                                            "SOAJS_GIT_REPO=" + ctrlRecord.src.repo,
                                            "SOAJS_GIT_BRANCH=" + ctrlRecord.src.branch
                                        ]
                                    };
                                    ...
                                    if (ctrlRecord.src.token) {
                                        dockerParams.variables.push("SOAJS_GIT_TOKEN=" + ctrlRecord.src.token);
                                    }

                                    //pulling service files, installing dependencies, and running it
                                    if (ctrlRecord.src.type === 'github') {
                                        var gitCommand;
                                        var runCommand =  'cd ' + ctrlRecord.src.repo + '/; npm install; node .';
                                        if (ctrlRecord.src.token) {
                                            gitCommand = 'git clone -b ' + ctrlRecord.src.branch + ' https://' + ctrlRecord.src.token + '@github.com/' + ctrlRecord.src.owner + '/' + ctrlRecord.src.repo + '.git;';
                                        } else {
                                            gitCommand = 'git clone -b ' + ctrlRecord.src.branch + ' https://github.com/' + ctrlRecord.src.owner + '/' + ctrlRecord.src.repo + '.git;';
                                        }

                                        dockerParams.Cmd = ['bash', '-c', 'cd /opt/soajs/node_modules/; ' + gitCommand + ' ' + runCommand];
                                    }
        ...
        function registerNewControllerHost(data, deployerConfig, cb) {
            ...
            mongo.insert("docker", document, function (error) {
                checkIfError(req, res, {config: config, error: error, code: 615}, function () {
                    ...
                    if (data.NetworkSettings && data.NetworkSettings.Networks && data.NetworkSettings.Networks[deployerConfig.config.HostConfig.NetworkMode]) {
                        newHost.ip = data.NetworkSettings.Networks[deployerConfig.config.HostConfig.NetworkMode].IPAddress;
                    } else {
                        newHost.ip = data.NetworkSettings.IPAddress;
                    }
                    newHost.hostname = newHost.hostname.replace("/", "");
                    mongo.insert(colName, newHost, function (error) {
                        checkIfError(req, res, {config: config, error: error, code: 615}, function () {
                ...
                checkIfError(req, res, {config: config, error: envRecord.deployer.type === 'manual', code: 618}, function () {
                    ...
                    mongo.findOne("services", {"name": serviceName}, function (err, serviceRecord) {
                        checkIfError(req, res, {config: config, error: err || !serviceRecord, code: 600}, function () {
                            checkIfError(req, res, {
                                config: config,
                                error: !serviceRecord.src || !serviceRecord.src.type || !serviceRecord.src.owner || !serviceRecord.src.repo || !serviceRecord.src.branch || !serviceRecord.src.main,
                                code: 741
                            }, function () {
                                ...
                                var dockerParams = {
                                    ...
                                    "variables": [
                                        "SOAJS_SRV_AUTOREGISTERHOST=false",
                                        "SOAJS_PROFILE=/opt/soajs/FILES/profiles/profile.js", //hardcoded for testing purposes

                                        "NODE_ENV=production",

                                        "SOAJS_MONGO_NB=" + mongoDbs.length,

                                        "SOAJS_GIT_OWNER=" + serviceRecord.src.owner,
                                        "SOAJS_GIT_REPO=" + serviceRecord.src.repo,
                                        "SOAJS_GIT_BRANCH=" + serviceRecord.src.branch
                                    ]
                                };
                                ...
                                //if private repo, add token to env variables
                                if (serviceRecord.src.token) {
                                    dockerParams.variables.push("SOAJS_GIT_TOKEN=" + serviceRecord.src.token);
                                }

                                //pulling service files, installing dependencies, and running it
                                if (serviceRecord.src.type === 'github') {
                                    var gitCommand;
                                    var runCommand =  'cd ' + serviceRecord.src.repo + '/; npm install; node .';
                                    if (serviceRecord.src.token) {
                                        gitCommand = 'git clone -b ' + serviceRecord.src.branch + ' https://' + serviceRecord.src.token + '@github.com/' + serviceRecord.src.owner + '/' + serviceRecord.src.repo + '.git;';
                                    } else {
                                        gitCommand = 'git clone -b ' + serviceRecord.src.branch + ' https://github.com/' + serviceRecord.src.owner + '/' + serviceRecord.src.repo + '.git;';
                                    }
                                    ...
                                    checkIfError(req, res, {config: config, error: error, code: 615}, function () {
                                        ...
                                        deployer.start(deployerConfig, data.Id, mongo, function (error, data) {
                                            checkIfError(req, res, {config: config, error: error, code: 615}, function () {
                                                ...
                                                registerHost(req, res, mongo, config, data, deployerConfig, info);
                                            });
                                        });
                                    });
                                });
        ...
        mongo.findOne("environment", {code: req.soajs.inputmaskData.envCode.toUpperCase()}, function (error, envRecord) {
            checkIfError(req, res, {config: config, error: error || !envRecord, code: 600}, function () {
                checkIfError(req, res, {config: config, error: envRecord.deployer.type === 'manual', code: 618}, function () {
                    ...
                    var folderPath = config.workDir + "soajs/open_source/services/" + daemonOrig;

                    mongo.findOne("daemons", {"name": daemonName}, function (error, daemonRecord) {
                        checkIfError(req, res, {config: config, error: error || !daemonRecord, code: 600}, function () {
                            checkIfError(req, res, {
                                config: config,
                                error: !daemonRecord.src || !daemonRecord.src.type || !daemonRecord.src.owner || !daemonRecord.src.repo || !daemonRecord.src.branch || !daemonRecord.src.main,
                                code: 741
                            }, function () {
                                if (daemonRecord.custom) {
                                    folderPath = daemonRecord.custom;
                                }
                                ...
                                var dockerParams = {
                                    ...
                                    "variables": [
                                        ...
                                        "SOAJS_GIT_OWNER=" + daemonRecord.src.owner,
                                        "SOAJS_GIT_REPO=" + daemonRecord.src.repo,
                                        "SOAJS_GIT_BRANCH=" + daemonRecord.src.branch
                                    ]
                                };
                                ...
                                //if private repo, add token to env variables
                                if (daemonRecord.src.token) {
                                    dockerParams.variables.push("SOAJS_GIT_TOKEN=" + daemonRecord.src.token);
                                }

                                //pulling service files, installing dependencies, and running it
                                if (daemonRecord.src.type === 'github') {
                                    var gitCommand;
                                    var runCommand =  'cd ' + daemonRecord.src.repo + '/; npm install; node .';
                                    if (daemonRecord.src.token) {
                                        gitCommand = 'git clone -b ' + daemonRecord.src.branch + ' https://' + daemonRecord.src.token + '@github.com/' + daemonRecord.src.owner + '/' + daemonRecord.src.repo + '.git;';
                                    } else {
                                        gitCommand = 'git clone -b ' + daemonRecord.src.branch + ' https://github.com/' + daemonRecord.src.owner + '/' + daemonRecord.src.repo + '.git;';
                                    }
