						...
						checkIfError(soajs, res, {config: config, error: error || !dockerRecord, code: 600}, function () {
							var dockerParams = {
								...
								"Cmd": [
									'bash',
									'-c',
									'cd /opt/soajs/node_modules/*/ && /usr/bin/npm list --json > packages.txt'
								]
							};

							deployer.exec(dockerRecord.deployer, dockerRecord.cid, BL.model.getDB(), dockerParams, function (error, exec) {
								checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
									exec.start({hijack: true, stdin: true}, function (error) {
										checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
											deployer.getFile(dockerRecord.deployer, dockerRecord.cid, soajs, '/opt/soajs/node_modules/*/packages.txt', BL.model.getDb(), function (error, data) {
												checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
