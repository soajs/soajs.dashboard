								...
								checkReturnError(req, cb, {
									...
									coreProvision.generateInternalKey(function (error, internalKey) {
										checkReturnError(req, cb, {
											...
											if(x.position[1] > 0){
												//check previous key
												if(tenantRecord.applications[x.position[0]].keys[x.position[1]-1]){
													previousKeyConfig = tenantRecord.applications[x.position[0]].keys[x.position[1]-1].config;
												}
											}
											else{
												//check previous app
												if(tenantRecord.applications[x.position[0]-1].keys[0]){
											...
											if(previousKeyConfig && Object.keys(previousKeyConfig).length > 0){
												for(let i in previousKeyConfig){
													if(previousKeyConfig[i].oauth){
														keyConfigClone.oauth = JSON.parse(JSON.stringify(previousKeyConfig.oauth));
													}
												}
											}
