	...
	"permissionsGet": function (config, req, res, cbMain) {
		...
		function getUserAllAclPerKey(cb) {
			...
			var opts = {
				...
				"conditions": {},
				"fields": {'code': 1, 'deployer': 1, '_id': 0, 'sitePrefix': 1, 'portalPrefix': 1},
				"options": null
			};
			...
			BL.model.findEntries(req.soajs, opts, function (error, envCodesRecords) {
				checkReturnError(req, cbMain, {config: config, error: error, code: 600}, function () {
					...
					envCodesRecords.forEach(function(oneEnv){
						if(oneEnv.code.toLowerCase() === process.env.SOAJS_ENV.toLowerCase()){
							sites.dashboard = oneEnv.sitePrefix;
							sites.portal = oneEnv.portalPrefix;
						}
					});
					
					return cb({"acl": ACL, "environments": envCodes, "sites": sites});
				});
			});
		}
	},
