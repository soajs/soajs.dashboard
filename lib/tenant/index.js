...
function saveTenantRecordAndExit(tenantRecord, config, req, code, msg, cb) {
	...
	BL.model.saveEntry(req.soajs, opts, function (error) {
		checkReturnError(req, cb, { model: BL.model, config: config, error: error, code: code }, function () {
			var response = {};
...
var BL = {
	...
	"delete": function (config, req, res, cb) {
		...
		validateId(req.soajs, function (error) {
			checkReturnError(req, cb, { model: BL.model, config: config, error: error, code: 438 }, function () {
				checkCanEdit(req.soajs, function (err) {
					checkReturnError(req, cb, { model: BL.model, config: config, error: err, code: err }, function () {
