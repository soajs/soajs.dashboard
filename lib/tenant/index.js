...
function saveTenantRecordAndExit(tenantRecord, config, req, code, msg, cb) {
	...
	console.log(currentAppId);
	console.log("----");
	console.log(JSON.stringify(tenantRecord.applications,null,2));
	console.log("----");
			...
			if (currentAppId) {
				...
				tenantRecord.applications.forEach(function (oneApp) {
					if(oneApp.appId.toString() === currentAppId){
						console.log("found currentAppId:"+currentAppId);
						console.log(currentAppId);
						if (oneApp.keys[0]) {
							console.log("ok!");
				...
				checkReturnError(req, cb, { config: config, error: error || !registry, code: 446 }, function () {
					...
					BL.model.findEntry(req.soajs, opts, function (error, tenantRecord) {
						checkReturnError(req, cb, {
							...
							console.log("+++ ok will getkeyintenant");
			...
			checkReturnError(req, cb, { config: config, error: !x.found, code: 440 }, function () {
				...
				BL.model.findEntry(req.soajs, opts, function (error, productRecord) {
					checkReturnError(req, cb, {
						...
						console.log("+++ ok will generateDashboardExtKeyAndLeave");
		...
		function generateDashboardExtKeyAndLeave(x, tenantRecord, registry, productLocked) {
			...
			coreProvision.generateExtKey(req.soajs.inputmaskData.key, registry.serviceConfig.key, function (error, extKeyValue) {
				checkReturnError(req, cb, { config: config, error: error, code: 440 }, function () {
					...
					console.log("+++ alright will saveTenantRecordAndExit ...");
