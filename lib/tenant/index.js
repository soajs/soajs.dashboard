...
function checkCanEditUsingIdOrCode(soajs, cb) {
	var myUrac;
	if (soajs.uracDriver && soajs.uracDriver.getProfile()) {
		myUrac = soajs.uracDriver.getProfile();
	}
	if (soajs.inputmaskData.id && myUrac && myUrac.tenant.id.toString() === soajs.inputmaskData.id.toString()) {
		return cb(null, {});
	}
	else {
		var opts = {};
...
var BL = {
	...
	"delete": function (config, req, res, cb) {
		...
		validateId(req.soajs, function (error) {
			...
			checkReturnError(req, cb, errorData, function () {
				checkCanEditUsingIdOrCode(req.soajs, function (err) {
		...
		validateId(req.soajs, function (err) {
			...
			checkReturnError(req, cb, errorData, function () {
				...
				BL.model.findEntry(req.soajs, opts, function (err, data) {
					checkReturnError(req, cb, { config: config, error: err || !data, code: req.soajs.inputmaskData.id?438:452 }, function () {
						//generate oauth authorization if needed.
	...
	// todo: check
	"addApplicationExtKeys": function (config, coreProvision, req, res, cb) {
				...
				BL.model.findEntry(req.soajs, opts, function (error, productRecord) {
					checkReturnError(req, cb, {
						...
						if (productRecord.locked) {
							...
							generateDashboardExtKeyAndLeave(x, tenantRecord, envRecord, true);
						}
						else {
							...
							if (req.soajs.inputmaskData.env.toUpperCase() === process.env.SOAJS_ENV.toUpperCase()) {
								...
								generateDashboardExtKeyAndLeave(x, tenantRecord, envRecord, locked);
							}
						}
					});
				});
		 ...
		 */
		function generateDashboardExtKeyAndLeave(x, tenantRecord, envRecord, productLocked) {
			//init a new coreProvision to generate ext key
			coreProvision.init(req.soajs.registry.coreDB.provision, req.soajs.log);
			coreProvision.generateExtKey(req.soajs.inputmaskData.key, envRecord.services.config.key, function (error, extKeyValue) {
