	...
	cleanEnvVarsList: function (vars) {
		...
		if (vars && vars.length > 0) {
			for (let x = vars.length - 1; x >= 0; x--) {
				if (vars[x] && preDefined.indexOf(vars[x].name) !== -1) {
					vars.splice(x, 1);
				}
			}
		}
...
var BL = {
			...
			checkIfError(req, cb, {config: config, error: error, code: 600}, function () {
				...
				accounts.forEach(function (oneAccount) {
					if (activeAccounts.indexOf(oneAccount) === -1) {
						if (oneAccount.variables && oneAccount.variables.length > 0) {
							oneAccount.variables.forEach((oneVariable) => {
								oneVariable.value = "********"
							});
						}
						...
						if (req.soajs.inputmaskData.variables) {
							async.each(accounts, function (oneAccount, callback) {
								helpers.buildEnvVarsList(oneAccount.gitToken, BL.model, config, req, function (error, soajsVars) {
									...
									let temp = {};
									if (oneAccount.variables && oneAccount.variables.length > 0){
										oneAccount.variables.forEach((oneVar)=>{
											temp[oneVar.name] = oneVar.value;
										});
									}
									oneAccount.variables = soajsVars;
									oneAccount.customeVariables = temp;
									return callback();
								});
					...
					BL.model.findEntry(req.soajs, opts, function (error, record) {
						...
						ciDriver.generateToken(settings, function (error, authToken) {
							checkIfError(req, cb, {config: config, error: error, code: 969}, function () {
								...
								if (Object.hasOwnProperty.call(req.soajs.inputmaskData, 'variables')) {
									req.soajs.inputmaskData.variables.forEach((oneVariable) => {
										if (oneVariable.value === "********" && record && record.variables) {
											record.variables.forEach((oldVariable) => {
												if (oneVariable.name === oldVariable.name) {
													oneVariable.value = oldVariable.value;
												}
											});
										}
									});
									opts.fields['$set'].variables = req.soajs.inputmaskData.variables;
								}
	...
	updateRepoSettings: function (config, req, ciDriver, cb) {
		...
		BL.model.findEntry(req.soajs, opts, function (error, record) {
			checkIfError(req, cb, {config: config, error: error || !record, code: 600}, function () {
				if (record.variables){
					record.variables.forEach((oneVar) => {
						oneVar.public = false;
					});
				}
				req.soajs.inputmaskData.variables = req.soajs.inputmaskData.variables.concat(record.variables);
				var options = {
					driver: record.provider,
					settings: {
						ciToken: record.ciToken,
