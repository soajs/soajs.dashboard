...
var BL = {
	...
	listUniqueProviders: function (config, req, cb) {
		var opts = {
			collection: colName,
			conditions: { "type": "recipe" }
		};

		BL.model.findEntries(req.soajs, opts, function (error, recipes) {
			checkIfError(req, cb, { config: config, error: error, code: 600 }, function () {
				async.groupBy(recipes, function (oneRecipe, callback) {
					return callback(null, oneRecipe.provider);
				}, function (error, allProviders) {
					if (!req.soajs.inputmaskData.owner) {
						return cb(null, allProviders);
					}
					else {
						//get all owner accounts and only return the providers activated for it
						opts.conditions.type = 'account';
						opts.conditions.owner = req.soajs.inputmaskData.owner;
						BL.model.findEntries(req.soajs, opts, function (error, accounts) {
							checkIfError(req, cb, { config: config, error: error, code: 600 }, function () {
								var ownerProviders = {};
								async.each(accounts, function (oneAccount, callback) {
									if (allProviders[oneAccount.provider]) {
										ownerProviders[oneAccount.provider] = allProviders[oneAccount.provider];
									}
									return callback();
								}, function () {
									return cb(null, ownerProviders);
								});
							});
						});
					}
	 ...
	 * @param cb
	 */
	addRecipe: function (config, req, res, cb) {
		var opts = {
			collection: colName,
			conditions: {
				"type": "recipe",
	 ...
	 * @param cb
	 */
	deleteRecipe: function (config, req, res, cb) {
		BL.model.validateId(req.soajs);
		var opts = {
			collection: colName,
