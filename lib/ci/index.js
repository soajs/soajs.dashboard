...
var BL = {
	...
	getConfig: function (config, req, cb) {
		...
		function listCiRepos(response) {
			buildEnvVarsList(response.settings.settings.gitToken, function(variables){
				//call the list
		...
		function buildEnvVarsList(gitToken, cb){
			var opts = {
	...
	/**
	 * Function that get the available environment variables for one repository
	 * @param  {Object}   config
	 * @param  {Request Object}   req
	 * @param  {Function} cb
	 *
	 */
	getRepoEnvVars: function (config, req, cb) {
		var opts = { collection: colName };
		BL.model.findEntry(req.soajs, opts, function (error, record) {
			checkIfError(req, cb, {config: config, error: error, code: 600}, function () {
	 ...
	 *
	 */
	getRepoSettings: function (config, req, cb) {
		var opts = { collection: colName };
		BL.model.findEntry(req.soajs, opts, function (error, record) {
			checkIfError(req, cb, {config: config, error: error, code: 600}, function () {
	 ...
	 *
	 */
	updateRepoSettings: function (config, req, cb) {
		var opts = { collection: colName };
		BL.model.findEntry(req.soajs, opts, function (error, record) {
			checkIfError(req, cb, {config: config, error: error, code: 600}, function () {
				...
				options.params = {
					...
					variables: req.soajs.inputmaskData.variables
				};
