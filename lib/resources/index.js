...
var BL = {
                ...
                BL.model.findEntries(req.soajs, opts, function (error, resources) {
                    checkIfError(req.soajs, cb, { config: config, error: error,  }, function () {
                        utils.checkIfOwner(req.soajs, BL.model, function (error, isOwner) {
                            checkIfError(req.soajs, cb, { config: config, error: error,  }, function () {
                            	console.log( "abel" ); // ToDelete #2del
     ...
     */
	addEditResource: function (config, req, res, deployBL, serviceBL, mainCb) {
		...
		async.series({
			...
			"deployService": (cb) => {
				if (req.soajs.inputmaskData.deployType && req.soajs.inputmaskData.deployType === 'saveAndDeploy') {
					let inputmaskData = JSON.parse(JSON.stringify(req.soajs.inputmaskData));
					
					deployBL.deployService(config, req, deployer, function (error, result) {
						
						if(error && !isOnEdit){
							
							req.soajs.inputmaskData = inputmaskData; // restore
				...
				if (req.soajs.inputmaskData.deployType && req.soajs.inputmaskData.deployType === 'saveAndRebuild') {
                    deployBL.redeployService(config, req, deployer, function (error, result) {
                        checkIfError(req.soajs, mainCb, {config: config, error: error}, function () {
                            return mainCb();
                        });
                    });
