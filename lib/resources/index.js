...
var BL = {
	...
	addEditResource: function (config, req, res, serviceBL, mainCb) {
		req.soajs.log.debug(1, "inside addeditresource :", req.soajs.inputmaskData);
		...
		async.auto({
			...
			"configUpdate": ['addEditResource', (info, cb) => {
				req.soajs.log.debug(2, "inside addeditresource :", req.soajs.inputmaskData);
				if (!isOnEdit) { // on new
			...
			"deployService": ['addEditResource', 'configUpdate', (info, cb) => {
				req.soajs.log.debug(3, "inside addeditresource :", req.soajs.inputmaskData);
				if (req.soajs.inputmaskData.deployType && req.soajs.inputmaskData.deployType !== 'save') {
					initBLModel(req, res, deployModule, dbModel, function (deployBL) {
						
						req.soajs.log.debug(4, "inside addeditresource :", req.soajs.inputmaskData);
