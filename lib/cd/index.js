	...
	saveConfig: function (config, req, helpers, cb) {
		...
		BL.model.findEntry(req.soajs, opts, function (error, record) {
			utils.checkErrorReturn(req.soajs, cb, { config: config, error: error, code: 600 }, function () {
				...
				if(req.soajs.inputmaskData.config.versions){
					for(var version in req.soajs.inputmaskData.config.versions){
						record[req.soajs.inputmaskData.config.env.toUpperCase()][req.soajs.inputmaskData.config.serviceName][version] = req.soajs.inputmaskData.config.versions[version];
					}
				}
