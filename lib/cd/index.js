...
var BL = {
	 ...
	 * @param  {Object}     config
	 * @param  {Object}     req
	 * @param  {Object}     cicdModel
	 * @param  {Object}     helpers
	 * @param  {Function}   cb
	 *
	 */
	getConfig: function (config, req, helpers, cicdModel, cb) {
		var opts = {collection: colName, conditions: {"type": "cd"}};
		cicdModel.findEntry(req.soajs, BL.model, opts, function (error, record) {
		...
		BL.model.findEntry(req.soajs, opts, function (error, record) {
			checkIfError(req.soajs, cb, {config: config, error: error, code: 600}, function () {
				...
				if (req.soajs.inputmaskData.config) {
					...
					let env = req.soajs.inputmaskData.config.env.toUpperCase();
					if (!record[env]) {
						record[env] = {};
					}
					
					if (!record[env][req.soajs.inputmaskData.config.serviceName]) {
						record[env][req.soajs.inputmaskData.config.serviceName] = {};
					}
					
					if (req.soajs.inputmaskData.config.default) {
						record[env][req.soajs.inputmaskData.config.serviceName] = req.soajs.inputmaskData.config.default;
						record[env][req.soajs.inputmaskData.config.serviceName].type = 'custom';
					} else if (req.soajs.inputmaskData.config.version
						&& req.soajs.inputmaskData.config.version.options
						&& req.soajs.inputmaskData.config.version.options.custom
						&& req.soajs.inputmaskData.config.version.options.custom.type === 'daemon') {
						let version = soajsLib.version.sanitize(req.soajs.inputmaskData.config.version.v);
						...
						record[env][req.soajs.inputmaskData.config.serviceName].type = 'daemon';
						
						if (!record[env][req.soajs.inputmaskData.config.serviceName][version]) {
							record[env][req.soajs.inputmaskData.config.serviceName][version] = {};
						}
						record[env][req.soajs.inputmaskData.config.serviceName][version][groupName] = req.soajs.inputmaskData.config.version;
						if (record[env][req.soajs.inputmaskData.config.serviceName][version][groupName]
							&& record[env][req.soajs.inputmaskData.config.serviceName][version][groupName].options
							&& record[env][req.soajs.inputmaskData.config.serviceName][version][groupName].options.custom
							&& record[env][req.soajs.inputmaskData.config.serviceName][version][groupName].options.custom.secrets
						) {
							record[env][req.soajs.inputmaskData.config.serviceName][version][groupName].options.custom.secrets.forEach((secret) => {
								delete secret.data;
							});
						}
					} else if (req.soajs.inputmaskData.config.version) {
						let version = soajsLib.version.sanitize(req.soajs.inputmaskData.config.version.v);
						delete req.soajs.inputmaskData.config.version.v;
						record[env][req.soajs.inputmaskData.config.serviceName][version] = req.soajs.inputmaskData.config.version;
						record[env][req.soajs.inputmaskData.config.serviceName].type = 'service';
						if (record[env][req.soajs.inputmaskData.config.serviceName][version]
							&& record[env][req.soajs.inputmaskData.config.serviceName][version].options
							&& record[env][req.soajs.inputmaskData.config.serviceName][version].options.custom
							&& record[env][req.soajs.inputmaskData.config.serviceName][version].options.custom.secrets
						) {
							record[env][req.soajs.inputmaskData.config.serviceName][version].options.custom.secrets.forEach((secret) => {
								delete secret.data;
							});
						}
					}
	 ...
	 *
	 */
	cdDeploy: function (config, req, deployer, helpers, cicdModel, cb) {
		//verify that the deploy_token is valid
		verifyDeployToken(function () {
			//get CD recipe
			BL.getConfig(config, req, helpers, cicdModel, function (error, record) {
				checkIfError(req.soajs, cb, {config: config, error: error, code: 600}, function () {
		...
		function getUndeployedServices(deployedServices, cdRecord, options, fcb) {
			...
			for (var envKey in cdRecord) {
				if (typeof cdRecord[envKey] === "object") {
			...
			async.mapValues(cdRecord, function (oneCDEnvConfig, envCode, callback) {
				async.detect(options.envs, function (oneEnvRecord, callback) {
					...
					return callback(null, {
						envConfig: oneCDEnvConfig,
						envRecord: (matchingEnvRecord && matchingEnvRecord.record) || {}
					});
				});
	...
	getUpdates: function (config, req, deployer, helpers, cloudServicesBL, cb) {
		...
		BL.model.findEntry(req.soajs, opts, (error, envRecord) => {
			checkIfError(req.soajs, cb, {config: config, error: error || !envRecord, code: 600}, function () {
									...
									BL.model.insertEntry(req.soajs, opts, (error) => {
										checkIfError(req.soajs, cb, {
											...
											if (req.soajs.inputmaskData.data.action === 'deploy') {
												let soajs_project;
												if (req.soajs.inputmaskData && req.soajs.inputmaskData.soajs_project) {
													soajs_project = req.soajs.inputmaskData.soajs_project;
												}
												req.soajs.inputmaskData = req.soajs.inputmaskData.deployOptions;
												if (soajs_project) {
													req.soajs.inputmaskData.soajs_project = soajs_project;
												}
