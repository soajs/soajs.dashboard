...
var BL = {
	...
	getLedger: function (config, req, cb) {
		...
		function doesServiceHaveUpdates(oneService, catalogs, repos, soajsImages, cb){
			...
			//todo: check if there are image updates
			//todd: if soajs is the image, you alread made the api call above, comapre the version only
			let imageInfo = myCatalog.recipe.deployOptions.image;
			if(imageInfo.name === 'soajs' && imageInfo.prefix === 'soajsorg'){
				let tag = imageInfo.tag;
				let imageUpdated;
				let latest;
				let myImage;
				let newVersion;
				
				soajsImages.forEach(function(oneImage){
					if(oneImage.name === tag){
						myImage = oneImage;
						let imageLastUpdated = new Date(oneimage.last_updated).getTime();
						imageUpdated = (imageLastUpdated > new Date(oneService.labels['soajs.image.ts']).getTime());
					}
				...
				updateObj.image = myImage;
				if(imageUpdated){ updateObj.image.update = true; }
				if(newVersion){ updateObj.image.upgrade = true; }
			}
			else{
				checkImageforUpdates(opts, null, cb);
			}
			
			return cb(null, true);
		}
		
		function checkImageforUpdates(opts, getImageInfo, cb){
			if(getImageInfo){
				let myUrl = config.docker.url;
			...
			function step2(results){
				...
				results.forEach(function(oneImage){
					if(oneImage.name === tag){
						myImage = oneImage;
						let imageLastUpdated = new Date(oneImage.last_updated).getTime();
						imageUpdated = (imageLastUpdated > new Date(opts.oneService.labels['soajs.image.ts']).getTime());
					}
					else if (oneImage.name === 'latest'){
						latest = oneImage;
					}
				...
				opts.updateObj.image = myImage;
				if(imageUpdated){ opts.updateObj.image.update = true; }
				if(newVersion){ opts.updateObj.image.upgrade = true; }
			}
		}
	}
