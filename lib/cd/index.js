...
var BL = {
	...
	saveConfig: function (config, req, helpers, cb) {
		...
		async.mapValues(req.soajs.inputmaskData.config, function (oneRecordEntry, key, callback) {
			if (['_id', 'type'].indexOf(key) !== -1 || typeof(oneRecordEntry) !== 'object') {
				return callback(null, oneRecordEntry);
			}

			return utils.normalizeKeyValues(oneRecordEntry, config.tokens.dotRegexString, config.tokens.dotToken, callback);
		}, function (error, updatedRecord) {
			//no error to be handled
			req.soajs.inputmaskData.config = updatedRecord;
