...
var BL = {
	...
	getConfig: function (config, req, helpers, cb) {
		...
		BL.model.findEntry(req.soajs, opts, function (error, record) {
			...
			async.mapValues(record, function (oneRecordEntry, key, callback) {
				...
				var envConfig = {};
				var dotReplacementRegex = new RegExp(config.CD.dotReplacement, 'g');
				async.eachOf(oneRecordEntry, function (oneEnvConfig, key, eachOfCB) {
					if(key.match(dotReplacementRegex)) {
						envConfig[key.replace(dotReplacementRegex, '.')] = oneEnvConfig;
					}
					else {
						envConfig[key] = oneEnvConfig;
					}
	...
	saveConfig: function (config, req, helpers, cb) {
		...
		async.mapValues(req.soajs.inputmaskData.config, function (oneRecordEntry, key, callback) {
			if (typeof(oneRecordEntry) !== 'object') {
				return callback(null, oneRecordEntry);
			}

			var envConfig = {};
			var dotReplacementRegex = new RegExp('\\.', 'g');
			async.eachOf(oneRecordEntry, function (oneEnvConfig, key, eachOfCB) {
				if(key.match(dotReplacementRegex)) {
					envConfig[key.replace(dotReplacementRegex, '__dot__')] = oneEnvConfig;
				}
				else {
					envConfig[key] = oneEnvConfig;
				}
