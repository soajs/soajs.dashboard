...
var BL = {
	...
	getConfig: function (config, req, helpers, cb) {
		...
		BL.model.findEntry(req.soajs, opts, function (error, record) {
			helpers.normalizeConfigValues(record, config.CD.dotToken, config.CD.dotValue, function (error, updatedRecord) {
	...
	saveConfig: function (config, req, helpers, cb) {
		...
		helpers.normalizeConfigValues(req.soajs.inputmaskData.config, config.CD.dotRegexString, config.CD.dotToken, function (error, updatedRecord) {
			//no error to be handled
