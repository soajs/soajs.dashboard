...
var BL = {
            ...
            async.each(servicesList, function (oneService, callback) {
                if (oneService.strategy === 'update') {
                    var opts = {
                        ...
                        "record": {
                            "serviceName": oneService.labels["soajs.service.name"],
                            "repo": oneService.env["SOAJS_GIT_REPO"],
                            "branch": oneService.env["SOAJS_GIT_BRANCH"],
                            "env": oneService.labels["soajs.env.code"],
                            "ts": new Date().getTime(),
                            "update": true
                        }
                    };
                    if(oneService.serviceVersion){
                        opts.record.serviceVersion = oneService.labels["soajs.service.version"];
                    }
                    ...
                    var opts = {
                        ...
                        "record": {
                            ...
                            "mode": oneService.labels["soajs.service.mode"],
                            "serviceName": oneService.labels["soajs.service.name"],
                            "repo": oneService.env["SOAJS_GIT_REPO"],
                            "branch": oneService.env["SOAJS_GIT_BRANCH"],
                            "env": oneService.labels["soajs.env.code"],
    ...
    getUpdates: function (config, req, cb) {
        ...
        function checkImageforUpdates(opts, cb){
            ...
            function deepVersionComparison(oneImage, tag, opts, newObj){
                ...
                else if(!isNaN(parseInt(oneImage.name)) && isNaN(tag)){
    ...
    getLedger: function (config, req, cb){
        let opts = {
        	"collection": "ledger",
            "conditions": {"env": req.soajs.inputmaskData.env},
            "options": {
                "sort": {"ts":-1},
     ...
     * @param cb
     */
    cdAction: function(config, req, cb){
        function initBLModel (BLModule, modelName, cb) {
            BLModule.init(modelName, cb);
        }

        function redeployService(cb) {
            var servicesBL = require("../cloud/deploy.js");
            initBLModel(servicesBL, 'mongo', function(error, BL){
                if(error){
                    return cb(error);
                }
                BL.model.validateId(req.soajs, (err, res) => {
                    if(err)
                        return cb(err);
                    ...
                    BL.model.findEntry(req.soajs, opts, (error, mongoRes) =>{
                        if(error)
                            return cb(error);

                        mongoRes.manual = true;
                        mongoRes.oldTs = mongoRes.ts;
                        mongoRes.ts = new Date().getTime();
                        opts = {
                            "collection": "ledger",
                            "record" : mongoRes
                        };
                        BL.model.saveEntry(req.soajs, opts, (error) => {
                            ...
                            req.soajs.inputmaskData.env = mongoRes.env;
                            req.soajs.inputmaskData.mode = mongoRes.mode;
                            req.soajs.inputmaskData.serviceId = mongoRes.serviceId;
                            BL.redeployService(config, req.soajs, cb);
                        });
                    });
                });
