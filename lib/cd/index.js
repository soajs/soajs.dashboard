...
var BL = {
	...
	cdDeploy: function (config, req, registry, deployer, helpers, cb) {
		...
		function getEnvs (fcb) {
			...
			BL.model.findEntries(req.soajs, opts, function (error, envs) {
				checkIfError(req.soajs, cb, { config: config, error: error, code: 600 }, function () {
					...
					helpers.getEnvsServices(envs, req, registry, deployer, BL, fcb);
				});
			});
		}
		...
		function getUndeployedServices(deployedServices, cdRecord, options, cb) {
			...
			async.mapValues(cdRecord, function (oneCDEnvConfig, envCode, callback) {
				async.detect(options.envs, function (oneEnvRecord, callback) {
					return callback(null, (oneEnvRecord.record.environment.toLowerCase() === envCode.toLowerCase()));
		...
		function processCD (servicesList, cb) {
			...
			async.eachSeries(servicesList, function (oneService, callback) {
				helpers.processOneService(req, BL, oneService, registry, deployer, options, callback);
			}, cb);
		}
	},
	 ...
	 * @param  {Function}           cb
	 */
	getUpdates: function (config, req, registry, deployer, helpers, cloudServicesBL, cb) {
		/**
		 * get all catalogs
		 * @param cbk
		 */
		...
		function getServices (cbk) {
			helpers.getServices(config, req, registry, deployer, cloudServicesBL, cbk);
		}
	 ...
	 * @param cb
	 */
	cdAction: function (config, req, registry, deployer, helpers, cb) {
