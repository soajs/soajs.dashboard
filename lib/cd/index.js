...
var BL = {
		...
		BL.model.findEntry(req.soajs, opts, function (error, record) {
			utils.checkErrorReturn(req.soajs, cb, { config: config, error: error, code: 600 }, function () {
				async.mapValues(record, function (oneRecordEntry, key, callback) {
					...
					if(updatedRecords[process.env.SOAJS_ENV.toUpperCase()]){
	 ...
	 *
	 */
	saveConfig: function (config, req, helpers, soajsUtils, cb) {
		var opts = { collection: colName, conditions: { "type": "cd" } };
		BL.model.findEntry(req.soajs, opts, function (error, record) {
			utils.checkErrorReturn(req.soajs, cb, { config: config, error: error, code: 600 }, function () {
				...
				if(req.soajs.inputmaskData.config.version && req.soajs.inputmaskData.config.version.options && req.soajs.inputmaskData.config.version.options.custom && req.soajs.inputmaskData.config.version.options.custom.type === 'daemon'){
					var groupName = req.soajs.inputmaskData.config.version.options.custom.daemonGroup;
					
					var temp = soajsUtils.cloneObj(record[req.soajs.inputmaskData.config.env.toUpperCase()][req.soajs.inputmaskData.config.serviceName][version]);
					
					record[req.soajs.inputmaskData.config.env.toUpperCase()][req.soajs.inputmaskData.config.serviceName][version] = {};
					record[req.soajs.inputmaskData.config.env.toUpperCase()][req.soajs.inputmaskData.config.serviceName][version][groupName] = temp;
	 ...
	 *
	 */
	cdDeploy: function (config, req, registry, deployer, helpers, cb) {
		...
		function verifyDeployToken (fcb) {
			...
			BL.model.findEntry(req.soajs, opts, function (error, record) {
				utils.checkErrorReturn(req.soajs, cb, { config: config, error: error, code: 600 }, function () {
					utils.checkErrorReturn(req.soajs, cb, { config: config, error: !record, code: 955 }, function () {
		...
		function getUndeployedServices(deployedServices, cdRecord, options, cb) {
			delete cdRecord._id;
			...
			async.mapValues(cdRecord, function (oneCDEnvConfig, envCode, callback) {
				async.detect(options.envs, function (oneEnvRecord, callback) {
					...
					return callback(null, { envConfig: oneCDEnvConfig, envRecord: matchingEnvRecord.record });
				});
			}, function (error, cdRecordWithEnvRecords) {
				var allServices = [];
				//check if CD script call includes services array and use its content to fetch undeployed services
				if (req.soajs.inputmaskData.services && Array.isArray(req.soajs.inputmaskData.services) && req.soajs.inputmaskData.services.length > 0) {
					allServices = req.soajs.inputmaskData.services;
					return helpers.processUndeployedServices(req, deployedServices, allServices, cdRecordWithEnvRecords, cb);
				}
				//if services array was not found, query services and daemons collection and try to find content that belongs to repo that triggered CD
				else {
					var opts = {
					...
					BL.model.findEntries(req.soajs, opts, function (error, services) {
						...
						BL.model.findEntries(req.soajs, opts, function (error, daemons) {
							...
							if (allServices.length > 0) {
								async.map(allServices, function (oneEntry, callback) {
		...
		function processCD (servicesList, cb) {
			var options = { config: config, registry: registry };
			async.eachSeries(servicesList, function (oneService, callback) {
				helpers.processOneService(req, BL, oneService, deployer, options, callback);
			}, cb);
		}
	},
