...
var helpers = {
			...
			async.concat(undeployedServices, function (oneService, callback) {
				...
				async.concat(cdInfo, function (oneCDEnvConfig, callback) {
					...
					if (oneCDEnvConfig.envConfig.pause) {
						...
						req.soajs.log.debug("CD strategies are paused for environment", oneCDEnvConfig.envRecord.environment);
					}
	...
	processOneService: function (req, BL, oneService, registry, deployer, options, callback) {
		...
		//check if the strategy of the service is "notify" or the CD strategies of the environemnt are paused
			...
			//otherwise, get required info from deploy options that will be used to deploy the service
				...
				if (oneService.envRecord && oneService.envRecord.environment) {
					opts.record.env = oneService.envRecord.environment.toLowerCase();
				}
				if (oneService.serviceVersion) {
		...
		//deploy: true flag is added only for services that go through processUndeployedServices()
			...
			var opts = {
				...
				"record": {
					...
					"env": oneService.envRecord.environment.toLowerCase(),
					"repo": oneService.repo,
					"branch": oneService.branch,
					"ts": new Date().getTime(),
					"deploy": true,
			...
			BL.model.insertEntry(req.soajs, opts, function (error) {
				...
				initBLModel(deployBL, modelName, function (error, deployBL) {
					...
					registry.loadByEnv({ envCode: oneService.envRecord.environment.toLowerCase() }, function(error, envRegistry) {
						if(error) return callback(error);

						deployBL.deployService(options.config, req.soajs, envRegistry, deployer, function (error, data) {
	...
	checkRecordConfig: function (req, envs, record, cb) {
		async.concat(envs, function (oneEnv, cback) {
			var cdEnvConfig = record[oneEnv.record.environment.toUpperCase()];
			if (!cdEnvConfig) {
				req.soajs.log.debug("No CD configuration found for environment", oneEnv.record.environment);
				return cback(null, []); // no CD config found for environment
			}

			if (!oneEnv.services || oneEnv.services.length === 0) {
				req.soajs.log.debug("No services matching repo name or branch found in environment", oneEnv.record.environment);
			...
			async.concat(oneEnv.services, function (oneService, callback) {
				...
				if (cdEnvConfig[serviceName]) {
					...
					if (cdEnvConfig.pause) {
						serviceCD.pause = true;
						req.soajs.log.debug("CD strategies are paused for environment", oneEnv.record.environment);
					}
					...
					if (req.soajs.inputmaskData.services && Array.isArray(req.soajs.inputmaskData.services) && req.soajs.inputmaskData.services.length > 0) {
						...
						if (found) {
							req.soajs.log.debug("found specific entry in CD configuration for service", serviceName, 'in environment', oneEnv.record.environment);
	...
	getEnvsServices: function (envs, req, registry, deployer, BL, fcb) {
		...
		async.map(envs, function (oneEnv, callback) {
			...
			registry.loadByEnv({ envCode: oneEnv.code.toLowerCase() }, function(error, envRegistry) {
				if (error) {
					return callback(error)
				}

				var options = utils.buildDeployerOptions(envRegistry, req.soajs, BL);
				...
				deployer.listServices(options, function (error, services) {
					...
					async.map(services, function (oneService, callback) {
						...
						async.filter(mappedServices, function (oneService, callback) {
							...
							return callback(null, { record: envRegistry, services: repoServices });
						});
					});
				});
			});
	...
	getServices: function (config, req, registry, deployer, cloudServices, cb) {
		initBLModel(cloudServices, 'mongo', function (error, BL) {
			...
			BL.listServices(config, req.soajs, registry, deployer, cb);
		});
	},
