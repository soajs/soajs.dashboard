...
var helpers = {
	...
	normalizeConfigValues: function (record, regex, replacementString, cb) {
		async.mapValues(record, function (oneRecordEntry, key, callback) {
			...
			var envConfig = {};
			var dotReplacementRegex = new RegExp(regex, 'g');
			async.eachOf(oneRecordEntry, function (oneEnvConfig, key, eachOfCB) {
				if(key.match(dotReplacementRegex)) {
					envConfig[key.replace(dotReplacementRegex, replacementString)] = oneEnvConfig;
				}
				else {
	...
	processUndeployedServices: function (req, deployedServices, allServices, cdInfo, cb) {
		...
		async.reject(allServices, function (oneCDService, callback) {
			...
			async.concat(undeployedServices, function (oneService, callback) {
				req.soajs.log.debug('Service ' + oneService.serviceName + ' is not deployed, checking CD strategy');
				async.concat(cdInfo, function (oneCDEnvConfig, callback) {
						...
						if (oneCDEnvConfig[oneService.serviceName] && oneCDEnvConfig[oneService.serviceName][srvVersionLabel]) {
							if (oneCDEnvConfig[oneService.serviceName][srvVersionLabel].branch === req.soajs.inputmaskData.branch) {
								if (oneCDEnvConfig[oneService.serviceName][srvVersionLabel].deploy && oneCDEnvConfig[oneService.serviceName][srvVersionLabel].options) {
									...
									serviceCD.deploy = oneCDEnvConfig[oneService.serviceName][srvVersionLabel].deploy;
									req.soajs.log.debug('Service ' + oneService.serviceName + ' v.' + oneService.serviceVersion + ' will be deployed');
					...
					if (oneCDEnvConfig[oneService.serviceName] && oneCDEnvConfig[oneService.serviceName].deploy && oneCDEnvConfig[oneService.serviceName].options) {
						if (oneCDEnvConfig[oneService.serviceName].branch === req.soajs.inputmaskData.branch) {
							...
							serviceCD.deploy = oneCDEnvConfig[oneService.serviceName].deploy;
							req.soajs.log.debug('Service ' + oneService.serviceName + ' will be deployed');
							return callback(null, [ serviceCD ]);
						}
					}

					//no deploy config detected, ignore entry
					req.soajs.log.debug('CD strategy for service ' + oneService.serviceName + ' is not configured to deploy');
