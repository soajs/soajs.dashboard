...
var helpers = {
	...
	processOneService: function (req, BL, oneService, deployer, callback) {
		...
		if (oneService.strategy === 'notify' || oneService.pause) {
			var opts = {
				"collection": "ledger",
				"record": {
		...
		//check if the strategy of the service is "update
		else if (oneService.strategy === 'update') {
			var opts = {
				"collection": "ledger",
				"record": {
					"serviceName": oneService.service.labels["soajs.service.name"],
		...
		else if (oneService.strategy == 'deploy') {
			var opts = {
				"collection": "ledger",
				//TODO: fill ledger record
				"record": {
					"serviceName": '',
					"env": '',
					"repo": '',
			...
			BL.model.insertEntry(req.soajs, opts, function (error) {
				...
				var options = utils.buildDeployerOptions(oneService.envRecord, req.soajs, BL);
				options.params = oneService.options;
				req.soajs.log.info('DEPLOY: Received CD trigger for ' + oneService.repo);
				return deployer.deployService(options, callback);
			});
		};
	},
