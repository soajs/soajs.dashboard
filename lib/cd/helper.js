...
var helpers = {
	...
	processOneService: function (req, BL, oneService, deployer, options, cdConfigRecord, callback) {
		...
		//check if the strategy of the service is "update
			...
			let serviceName = oneService.service.labels["soajs.service.name"];
			if (oneService.service.labels["soajs.service.type"] === 'daemon') {
				serviceName = serviceName.replace("-" + oneService.service.labels["soajs.daemon.group"], "")
			}
			if (oneService.service.labels["soajs.service.type"] === 'other') {
				serviceName = serviceName.replace(oneService.service.labels["soajs.env.code"] + "-", "");
			}
			opts = {
				"collection": "ledger",
				"record": {
			...
			BL.model.insertEntry(req.soajs, opts, (err, res) => {
				let serviceCustomInputs = cdConfigRecord[oneService.envRecord.code.toUpperCase()][serviceName];
				if(!serviceCustomInputs){
					req.soajs.log.warn(`Invalid CD Configuration for service ${oneService.service.labels["soajs.service.name"]} under environment ${oneService.envRecord.code.toUpperCase()} !!!`);
				...
				switch(serviceCustomInputs.type){
					case 'service':
						...
						if (serviceCustomInputs && serviceCustomInputs[version] && serviceCustomInputs[version].options && serviceCustomInputs[version].options.custom){
							serviceCustomInputs = serviceCustomInputs[version].options.custom;
						}
						break;
					case 'daemon':
						version = 'v' + oneService.service.labels['soajs.service.version'];
						let daemonGroup = oneService.service.labels['soajs.daemon.group'];
						if (serviceCustomInputs && serviceCustomInputs[version] && serviceCustomInputs[version][daemonGroup] &&
							serviceCustomInputs[version][daemonGroup].options && serviceCustomInputs[version][daemonGroup].options.custom){
							serviceCustomInputs = serviceCustomInputs[version][daemonGroup].options.custom;
						}
						break;
					default:
						if (serviceCustomInputs && serviceCustomInputs.options && serviceCustomInputs.options.custom){
							serviceCustomInputs = serviceCustomInputs.options.custom;
						}
						break;
				}
				if (!serviceCustomInputs){
					return callback(new Error("Custom service inputs not found!"));
				}
				oneService.options = {
			...
			async.concat(oneEnv.services, function (oneService, callback) {
				...
				if (oneService.labels && oneService.labels['soajs.service.type']) {
					if (oneService.labels['soajs.service.type'] === 'service') {
						...
						if (daemonGroup){
							serviceName = serviceName.replace("-"+daemonGroup,"");
						}
					}
				}
				...
				if (cdEnvConfig[serviceName]) {
					...
					if (req.soajs.inputmaskData.services && Array.isArray(req.soajs.inputmaskData.services) && req.soajs.inputmaskData.services.length > 0) {
						...
						for (let i = 0; i < req.soajs.inputmaskData.services.length; i++) {
							if (req.soajs.inputmaskData.services[i].serviceName === serviceName && req.soajs.inputmaskData.services[i].serviceVersion === parseFloat(oneService.labels['soajs.service.version'])) {
	...
	callDeployer: function (config, req, deployer, opName, cb) {
		...
		initBLModel(cloudDeploy, 'mongo', function (error, BL) {
			...
			BL[opName](config, req, deployer, cb);
		});
	}
