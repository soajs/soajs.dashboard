					...
					if (!oneService.custom && oneService.serviceVersion) {
						...
						if (oneCDEnvConfig[oneService.label] && oneCDEnvConfig[oneService.label][srvVersionLabel]) {
							if (oneCDEnvConfig[oneService.label].type === 'service') {
								...
								var daemonGroups = Object.keys(oneCDEnvConfig[oneService.label][srvVersionLabel]), oneGroupConfig;
								var daemonGroupsCD = [];
								for (var i = 0; i < daemonGroups.length; i++) {
									oneGroupConfig = oneCDEnvConfig[oneService.label][srvVersionLabel][daemonGroups[i]];
									if (oneGroupConfig.branch === req.soajs.inputmaskData.branch) {
										if (oneGroupConfig.deploy && oneGroupConfig.options) {
											var oneGroupCD = JSON.parse(JSON.stringify(serviceCD));
											oneGroupCD.serviceVersion = oneService.serviceVersion;
											oneGroupCD.strategy = oneGroupConfig.strategy;
											oneGroupCD.options = oneGroupConfig.options;
											if(oneGroupCD.options.gitSource && req.soajs.inputmaskData.commit) {
												oneGroupCD.options.gitSource.commit = req.soajs.inputmaskData.commit;
												req.soajs.log.debug('Adding commit to configuration');
											}

											oneGroupCD.deploy = oneGroupConfig.deploy;
											req.soajs.log.debug('Daemon ' + oneService.label + ' v.' + oneService.serviceVersion + ' using group ' + daemonGroups[i] + ' will be deployed');
											daemonGroupsCD.push(oneGroupCD);
										}
									}
								}

								return callback(null, daemonGroupsCD);
							}
						}
					}
