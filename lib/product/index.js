...
var BL = {
	...
	"listConsole": function (config, req, res, productModel, cb) {
		...
		productModel.findEntry(req.soajs, BL.model, opts, function (err, record) {
			checkReturnError(req, cb, {config: config, error: err, code: 412}, function () {
				if (record && record.packages.length > 0) {
	...
	"updateScopePerEnv": function (config, req, res, productModel, cb) {
		var opts = {};
		validateId(req.soajs, function (err) {
			checkReturnError(req, cb, {config: config, error: err, code: 409}, function () {
				productModel.sanitizePerEnv(req.soajs.inputmaskData.acl, function () {
					var s = {
						'$set': {
							["scope.acl." + req.soajs.inputmaskData.env]: req.soajs.inputmaskData.acl
						}
					};
					opts.collection = colName;
					opts.conditions = {'_id': req.soajs.inputmaskData.id};
					...
					productModel.updateEntry(req.soajs,  BL.model, opts, function (err) {
						checkReturnError(req, cb, {
							...
							return cb(null, "product scope by env updated successful");
						});
					});
				});
			});
	...
	"getPackagePreviewAClService": function (config, req, res, cb) {
		let opts = {};
		opts.collection = colName;
		opts.conditions = {"code": req.soajs.inputmaskData.productCode};
		BL.model.findEntry(req.soajs, opts, function (err, product) {
			checkReturnError(req, cb, {
				...
				checkReturnError(req, cb, {
					config: config,
					error: req.soajs.inputmaskData.secEnv && pck.aclTypeByEnv[req.soajs.inputmaskData.mainEnv] !== "granular" && (pck.aclTypeByEnv[req.soajs.inputmaskData.mainEnv] !== pck.aclTypeByEnv[req.soajs.inputmaskData.secEnv]),
					code: 525
				}, function () {
					opts.collection = "services";
					opts.conditions = {
						name: {
							$ne: req.soajs.registry.services.controller.name || "controller"
						}
					};
					opts.options = {
						"skip": 0,
					...
					if ( req.soajs.inputmaskData.page > 1){
						opts.options.skip = opts.options.limit * req.soajs.inputmaskData.page - 1;
					}
					checkReturnError(req, cb, {
						config: config,
						error: err || !product,
						...
						BL.model.findEntries(req.soajs, opts, function (err, services) {
							checkReturnError(req, cb, {
								config: config,
								error: err,
								code: 460
							}, function () {
								helper.buildAclServicePreview(services, pck, req.soajs.inputmaskData, (acl) => {
									let result = {
										package: pck.code,
	...
	"getPackagePreviewAClApi": function (config, req, res, cb) {
		let opts = {};
		opts.collection = colName;
		opts.conditions = {"code": req.soajs.inputmaskData.productCode};
		BL.model.findEntry(req.soajs, opts, function (err, product) {
					...
					checkReturnError(req, cb, {
						...
						BL.model.findEntries(req.soajs, opts, function (err, services) {
							checkReturnError(req, cb, {
								...
								helper.buildAclApiPreview(services, pck, req.soajs.inputmaskData, (acl) => {
									let result = {
										package: pck.code,
										product: req.soajs.inputmaskData.productCode,
										acl: acl,
	...
	"getScopePreviewAClService": function (config, req, res, cb) {
		let opts = {};
		opts.collection = colName;
		opts.conditions = {"code": req.soajs.inputmaskData.productCode};
		BL.model.findEntry(req.soajs, opts, function (err, product) {
				...
				checkReturnError(req, cb, {
					...
					BL.model.findEntries(req.soajs, opts, function (err, services) {
						checkReturnError(req, cb, {
							...
							helper.buildAclServicePreviewScope(services, product.scope, req.soajs.inputmaskData, (acl) => {
								let result = {
									product: req.soajs.inputmaskData.productCode,
									acl: acl,
								};
	...
	"getScopePreviewAClApi": function (config, req, res, cb) {
		let opts = {};
		opts.collection = colName;
		opts.conditions = {"code": req.soajs.inputmaskData.productCode};
		BL.model.findEntry(req.soajs, opts, function (err, product) {
				...
				checkReturnError(req, cb, {
					...
					BL.model.findEntries(req.soajs, opts, function (err, services) {
						checkReturnError(req, cb, {
							...
							helper.buildAclApiPreviewScope(services, product.scope, req.soajs.inputmaskData, (acl) => {
								let result = {
									product: req.soajs.inputmaskData.productCode,
									acl: acl,
								};
								...
								BL.model.findEntries(req.soajs, opts, function (error, environments) {
									checkReturnError(req, cb, {
										...
										if (req.soajs.inputmaskData.type) {
											newPackage.aclTypeByEnv = req.soajs.inputmaskData.type;
										}
										if (newPackage.acl) {
											let schema = {
												"type": "object",
												...
												"patternProperties": {
													"^[a-zA-Z0-9]+$": {
														"type": "object",
														"patternProperties": {
															"^[^\W\.]+$": newPackage.aclTypeByEnv === "granular" ? granularAcl : apiGroup
														},
											...
											if (!check.valid) {
												let message = `Invalid Acl of type ${newPackage.aclTypeByEnv === "granular" ? "Granular" : "Api Group"} provided!`;
									...
									checkReturnError(req, cb, {
										...
										for (let i = 0; i < productRecord.packages.length; i++) {
											if (productRecord.packages[i].code.toUpperCase() === prefix + req.soajs.inputmaskData.code) {
												...
												if (req.soajs.inputmaskData.aclTypeByEnv) {
													productRecord.packages[i].aclTypeByEnv = req.soajs.inputmaskData.aclTypeByEnv;
												}
												found = true;
												break;
											}
	...
	"updatePackageAcl": function (config, req, res, cb) {
		var opts = {};
		validateId(req.soajs, function (err) {
			checkReturnError(req, cb, {config: config, error: err, code: 409}, function () {
				req.soajs.inputmaskData.code = req.soajs.inputmaskData.code.toUpperCase();
							...
							checkReturnError(req, cb, {
								...
								BL.model.countEntries(req.soajs, opts, function (error, count) {
									checkReturnError(req, cb, {
										...
										for (let i = 0; i < productRecord.packages.length; i++) {
											if (productRecord.packages[i].code.toUpperCase() === prefix + req.soajs.inputmaskData.code) {
												productRecord.packages[i].acl[req.soajs.inputmaskData.env.toLowerCase()] = req.soajs.inputmaskData.acl;
												if (req.soajs.inputmaskData.type) {
													if (!productRecord.packages[i].aclTypeByEnv) {
														productRecord.packages[i].aclTypeByEnv = {};
													}
													
													productRecord.packages[i].aclTypeByEnv[req.soajs.inputmaskData.env.toLowerCase()] = req.soajs.inputmaskData.type;
												}
												else {
													if (productRecord.packages[i].aclTypeByEnv){
														delete productRecord.packages[i].aclTypeByEnv[req.soajs.inputmaskData.env.toLowerCase()];
													}
												...
												let schema = {
													...
													"required": false,
													"patternProperties": {
														"^[^\W\.]+$": req.soajs.inputmaskData.type === "granular" ? granularAcl : apiGroup
													},
													"additionalProperties": false
												};
												let check = validator.validate(req.soajs.inputmaskData.acl, schema);
												
												if (!check.valid) {
													let message = `Invalid Acl of type ${productRecord.packages[i].acl.aclTypeByEnv === "granular" ? "Granular" : "Api Group"} provided`;
													return cb({"code": "469", "msg": message});
												}
												found = true;
												break;
										...
										BL.model.saveEntry(req.soajs, opts, function (error) {
											checkReturnError(req, cb, {
												...
												return cb(null, `Product package for ${req.soajs.inputmaskData.env} updated successfully`);
											});
										});
									});
	...
	"updatePackageAclApi": function (config, req, res, productModel, cb) {
		let opts = {};
		opts.collection = colName;
		opts.conditions = {
			code: req.soajs.inputmaskData.productCode
		};
		BL.model.findEntry(req.soajs, opts, function (error, product) {
			checkReturnError(req, cb, {
				config: config,
				error: error || !product,
				code: 416
				...
				checkReturnError(req, cb, {
					...
					helper.updateApiAcl(pck, req.soajs.inputmaskData.acl, req.soajs.inputmaskData.env.toLowerCase(), (acl) => {
						opts.collection = colName;
						opts.conditions = {
							'code': req.soajs.inputmaskData.productCode,
							'packages.code': req.soajs.inputmaskData.packageCode
						};
						opts.fields = {
							$set: {
								'packages.$.acl': acl
	...
	"updatePackageAclService": function (config, req, res, productModel, cb) {
		let opts = {};
		opts.collection = colName;
		opts.conditions = {
			code: req.soajs.inputmaskData.productCode
		...
		BL.model.findEntry(req.soajs, opts, function (error, product) {
			checkReturnError(req, cb, {
				...
				checkReturnError(req, cb, {
					...
					helper.updateServiceAcl(pck, req.soajs.inputmaskData.acl, req.soajs.inputmaskData.env.toLowerCase(), (acl) => {
						opts.collection = colName;
						opts.conditions = {
							'code': req.soajs.inputmaskData.productCode,
							'packages.code': req.soajs.inputmaskData.packageCode
	...
	"updateScopeAclApi": function (config, req, res, productModel, cb) {
		let opts = {};
		opts.collection = colName;
		opts.conditions = {
			code: req.soajs.inputmaskData.productCode
		...
		BL.model.findEntry(req.soajs, opts, function (error, product) {
			checkReturnError(req, cb, {
				...
				helper.updateApiAclScope(product.scope, req.soajs.inputmaskData.acl, req.soajs.inputmaskData.env.toLowerCase(), (acl) => {
					opts.collection = colName;
					opts.conditions = {
						'code': req.soajs.inputmaskData.productCode,
					};
	...
	"updateScopeAclService": function (config, req, res, productModel, cb) {
		let opts = {};
		opts.collection = colName;
		opts.conditions = {
			code: req.soajs.inputmaskData.productCode
		...
		BL.model.findEntry(req.soajs, opts, function (error, product) {
			checkReturnError(req, cb, {
				...
				helper.updateServiceAclScope(product.scope, req.soajs.inputmaskData.acl, req.soajs.inputmaskData.env.toLowerCase(), (acl) => {
					opts.collection = colName;
					opts.conditions = {
						'code': req.soajs.inputmaskData.productCode,
					};
