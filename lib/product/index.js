								...
								BL.model.findEntries(req.soajs, opts, function (error, environments) {
									checkReturnError(req, cb, {
										...
										if (req.soajs.inputmaskData.type){
											newPackage.aclType = req.soajs.inputmaskData.type;
										}
										if (newPackage.acl){
											let schema = {
												"type": "object",
									...
									checkReturnError(req, cb, {
										...
										for (let i = 0; i < productRecord.packages.length; i++) {
											if (productRecord.packages[i].code.toUpperCase() === prefix + req.soajs.inputmaskData.code) {
												...
												if (req.soajs.inputmaskData.type === "granular"){
													productRecord.packages[i].aclType = "granular";
												}
												else {
													delete productRecord.packages[i].aclType;
												}
												if (productRecord.packages[i].acl){
													let schema = {
														"type": "object",
														"required": false,
														"patternProperties": {
															"^[a-zA-Z0-9]+$": {
																"type": "object",
																"patternProperties": {
																	"^[^\W\.]+$": productRecord.packages[i].aclType === "granular" ? granularAcl : apiGroup
																},
																"additionalProperties": false
															}
														},
													...
													let check = validator.validate(productRecord.packages[i].acl, schema);
													if (!check.valid){
														let message = `Invalid Acl of type ${productRecord.packages[i].acl.aclType === "granular" ? "Granular" : "Api Group"} provided"`;
														return cb({"code": "469", "msg": message});
													}
												}
												found = true;
