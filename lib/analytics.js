	...
	"setEsCluster": function (req, res, config, envRecord, settings, cb) {
		...
		function getCluster (call){
			...
			if (envRecord.deployer.selected.split('.')[1] === "kubernetes"){
				var namespace = envRecord.container["kubernetes"][envRecord.deployer.selected.split('.')[2]].namespace.default;
						...
						utils.checkIfError(soajs, res, {config: config, error: !deployerCheck, code: 909}, function () {
							...
							deployer.listNameSpaces(options, function (error, namespaces) {
								utils.checkIfError(soajs, res, {config: config, error: error}, function () {
									return res.jsonp(req.soajs.buildResponse(null, namespaces));
								});
							});
						});
						// var data = {};
