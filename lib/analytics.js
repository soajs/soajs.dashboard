							...
							lib.setEsCluster(req, res, config, envRecord, settings, function (err, esCluster) {
								utils.checkIfError(req.soajs, res, {
									...
									if (!(process.env.SOAJS_TEST_ANALYTICS === 'test')) {
										...
										settings.env[envRecord.code.toLowerCase()] = {
											"active": true
										};
										if (!settings.elasticsearch) {
											settings.elasticsearch = {};
										...
										settings.logstash[envRecord.code.toLowerCase()] = {
											"status": "deployed"
										...
										settings.filebeat[envRecord.code.toLowerCase()] = {
											"status": "deployed"
										...
										combo.options = {'upsert': true, 'safe': true};
										BL.model.updateEntry(req.soajs, combo, function (error) {
											utils.checkIfError(req.soajs, res, {
												config: config,
												error: error,
												code: 600
										...
										async.eachSeries(services, function (oneService, callback) {
											...
											if (oneService.labels["soajs.service.type"] === "elk"
												...
												if (activated && (oneService.labels["soajs.service.name"] === "metricbeat") ||
													...
													console.log(oneService.labels["soajs.service.name"], 'oneService.labels["soajs.service.name"]')
													return callback(null, true);
												}
												else {
													onsole.log("activated is false")
													console.log(oneService.labels["soajs.service.name"], 'oneService.labels["soajs.service.name"]')
													options.params = {
														id: oneService.id,
											...
											utils.checkIfError(req.soajs, res, {
												...
												if (settings.metricbeat && settings.metricbeat[env]) {
