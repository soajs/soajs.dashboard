	...
	"setEsCluster": function (req, res, config, envRecord, settings, cb) {
		...
		function getCluster(call) {
			...
			if (envRecord.deployer.selected.split('.')[1] === "kubernetes") {
				...
				es_analytics_cluster.servers[0].host += '.' + namespace;
				console.log(es_analytics_cluster.servers[0].host)
				console.log(7)
			}
		}
		
		getCluster(function () {
			console.log(es_analytics_db, "es_analytics_db")
			console.log(es_analytics_cluster_name, "es_analytics_cluster_name")
			console.log(es_analytics_cluster, "es_analytics_cluster")
									...
									utils.checkIfError(req.soajs, res, {config: config, error: error}, function () {
										...
										async.each(services, function (oneService, callback) {
											if (oneService.labels["soajs.service.type"] === "elk"
												...
												&& oneService.labels["soajs.service.name"] !== "elasticsearch"
												...
												deployer.deleteService(options, function (err) {
													if (err) {
														return callback(err);
													}
													else {
