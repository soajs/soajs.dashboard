...
var BL = {
	...
	"deleteGroupConfig": function (config, req, res, cloudBL, registry, deployer, cb) {
				...
				BL.model.findEntry(req.soajs, opts, function (error, record) {
					checkReturnError(req, cb, {config: config, error: error || !record, code: 716}, function () {
						...
						if(process.env.SOAJS_DEPLOY_HA){
							helpers.checkIfGroupIsDeployed(config, record, req, BL, cloudBL, registry, deployer, checkReturnError, cb, function(){
								//do remove
								doRemove();
							});
						}
