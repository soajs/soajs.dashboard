...
var helpers = {
	...
	checkIfGroupIsDeployed: function(config, record, req, BL, cloudBL, registry, deployer, checkReturnError, mainCb, cb){
		//get all env codes
		...
		BL.model.findEntries(req.soajs, opts, function(error, envCodes){
			checkReturnError(req, mainCb, {config: config, error: error, code: 701}, function () {
				async.eachSeries(envCodes, function(oneEnvCode, miniCb){
					...
					checkReturnError(req, mainCb, {config: config, error: error, code: 711}, function () {
						return cb();
					});
				});
			});
