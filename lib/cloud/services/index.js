...
var BL = {
					...
					checkIfError(soajs, cbMain, {config: config, error: !options, code: 825}, function () {
						...
						deployer.execute({'type': 'container', 'driver': options.strategy}, 'listServices', options, (error, services) => {
							checkIfError(soajs, cbMain, {config: config, error: error}, function () {
								...
								deployer.execute({'type': 'container', 'driver': options.strategy}, 'listServices', options, (error, customServices) => {
									checkIfError(soajs, cbMain, {config: config, error: error}, function () {
		...
		utils.getEnvironment(soajs, BL.model, soajs.inputmaskData.env.toUpperCase(), function (error, envRecord) {
			checkIfError(soajs, cbMain, {
				...
				checkIfError(soajs, cbMain, {
					...
					deployer.execute({'type': 'container', 'driver': options.strategy}, 'scaleService', options, (error) => {
						checkIfError(soajs, cbMain, {config: config, error: error}, function () {
				...
				checkIfError(req.soajs, cbMain, {
					...
					deployer.execute({'type': 'container', 'driver': options.strategy}, 'inspectService', options, (error, deployedServiceDetails) => {
						checkIfError(req.soajs, cbMain, {config: config, error: error}, function () {
							deployer.execute({'type': 'container', 'driver': options.strategy}, 'deleteService', options, (error) => {
								checkIfError(req.soajs, cbMain, {config: config, error: error}, function () {
			...
			checkIfError(soajs, cb, {
				...
				checkIfError(soajs, cb, {
					...
					checkIfError(soajs, cb, { config: config, error: !options, code: 600 }, function() {
						deployer.execute({'type': 'container', 'driver': options.strategy}, 'listKubeServices', options, (error, services) => {
	...
	"listVms": function (config, soajs, deployer, cbMain) {
		...
		//get infra from db
		utils.getInfra(soajs, BL.model, soajs.inputmaskData.infra, function (error, infraRecord) {
			checkIfError(soajs, cbMain, {
				config: config,
				error: error || !infraRecord,
				code: 600 //change this
			}, function () {
				checkIfError(soajs, cbMain, {
					config: config,
					error: !infraRecord,
					...
					checkIfError(soajs, cbMain, {config: config, error: !options, code: 825}, function () {
						//todo use infra record to call bridge and supple info
						options.auth = infraRecord.auth;
						deployer.listVMs(soajs, options, function (error, vms) {
							checkIfError(soajs, cbMain, {config: config, error: error}, function () {
