		...
		async.auto({
			...
			"getInfra": ["getEnvironment", (results, mCb) => {
				checkIfError(soajs, mCb, {
					config: config,
					error: !result.getEnvironment.restriction || !Object.keys(results.getEnvironment.restriction).length > 0,
					code: 520
				}, () => {
			...
			"callDeployer": ["getEnvironment", "getInfra", (info, mCb) => {
				...
				checkIfError(req.soajs, mCb, {
					...
					let infraId = Object.keys(info.getEnvironment.restriction)[0];
					checkIfError(soajs, cbMain, {
						...
						let region = Object.keys(info.getEnvironment.restriction[infraId])[0]
						options.params = {
							id: req.soajs.inputmaskData.serviceId,
							mode: req.soajs.inputmaskData.mode,
							group: info.getEnvironment.restriction[infraId][region].group,
							region: region
						};
