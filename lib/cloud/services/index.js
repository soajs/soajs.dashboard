		...
		async.auto({
			...
			"getInfra": ["getEnvironment", (results, mCb) => {
				checkIfError(req.soajs, mCb, {
					config: config,
					error: req.soajs.inputmaskData.technology === "vm" && !results.getEnvironment.restriction || !Object.keys(results.getEnvironment.restriction).length > 0,
					code: 520
				}, () => {
					req.soajs.inputmaskData.id = Object.keys(results.getEnvironment.restriction)[0];
			...
			"callDeployer": ["getEnvironment", "getInfra", (info, mCb) => {
				...
				checkIfError(req.soajs, mCb, {
					...
					if (technology === "vm"){
						infraId = Object.keys(info.getEnvironment.restriction)[0];
					}
