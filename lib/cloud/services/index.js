...
var BL = {
	 ...
	 */
	"listServices": function (config, soajs, registry, deployer, cbMain) {
	 ...
	 * @param {Callback Function} cbMain
	 */
	"scaleService": function (config, soajs, registry, deployer, cbMain) {
		utils.getEnvironment(soajs, BL.model, soajs.inputmaskData.env, function (error, envRecord) {
			checkIfError(soajs, cbMain, {config: config, error: error, code: 402}, function () {
				var options = utils.buildDeployerOptions(registry, soajs, BL);
				options.params = {
	 ...
	 * @param {Callback Function} cbMain
	 */
	"deleteService": function (config, soajs, registry, deployer, cbMain) {
		utils.getEnvironment(soajs, BL.model, soajs.inputmaskData.env, function (error, envRecord) {
			checkIfError(soajs, cbMain, {config: config, error: error || !envRecord, code: 402}, function () {
				var options = utils.buildDeployerOptions(registry, soajs, BL);
				options.params = {
	 ...
	 *
	 */
	checkResource: function(config, soajs, registry, deployer, cb) {
		utils.getEnvironment(soajs, BL.model, soajs.inputmaskData.env, function(error, envRecord) {
