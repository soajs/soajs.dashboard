			...
			async.parallel({
				"updateInfra": (mCb) => {
					if (soajs.inputmaskData.previousEnvironment) {
						...
						let uniqueEnvs = infraRecord.deployments[info[2]].environments.filter((item, pos) => { return infraRecord.deployments[info[2]].environments.indexOf(item)== pos; });
						...
						let uniqueEnvs = oneDeployment.environments.filter((item, pos) => { return oneDeployment.environments.indexOf(item)== pos; });
