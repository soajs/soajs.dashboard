...
const clustersModule = {
			...
			if (soajs.inputmaskData.previousEnvironment) {
				...
				BL.model.findEntry(soajs, opts2, (error, previousEnvRecord) => {
					checkIfError(soajs, cbMain, { config: config, error: error, code: 600 }, function () {
						...
						if (environmentRecord.deployer
						&& environmentRecord.deployer.container
						&& environmentRecord.deployer.container.kubernetes
						&& environmentRecord.deployer.container.kubernetes.local
						&& environmentRecord.deployer.container.kubernetes.remote){
							environmentRecord.deployer.container.kubernetes.local.namespace.default = soajs.inputmaskData.envCode.toLowerCase();
							environmentRecord.deployer.container.kubernetes.remote.namespace.default = soajs.inputmaskData.envCode.toLowerCase();
						}
						let options = utils.buildDeployerOptions(environmentRecord, soajs, BL);
						if (options.deployerConfig){
							options.deployerConfig.namespace = {
								default: soajs.inputmaskData.envCode.toLowerCase(),
								perService: false
							};
						}
						deployer.execute({
							...
							utils.checkErrorReturn(soajs, cbMain, {config: config, error: error}, () =>{
			...
			async.parallel({
				"updateInfra": (mCb) => {
					...
					if (soajs.inputmaskData.previousEnvironment) {
						...
						uniqueEnvs = infraRecord.deployments[info[2]].environments.filter((item, pos) => {
							return infraRecord.deployments[info[2]].environments.indexOf(item) == pos;
						});
						...
						uniqueEnvs = oneDeployment.environments.filter((item, pos) => {
							return oneDeployment.environments.indexOf(item) == pos;
						});
									...
									async.auto({
										"getCatalogRecipe": (vCb) => {
											BL.model.findEntry(options.soajs, {
												...
												if(error) return vCb(error);
												if(!recipeRecord) {
													// add it to db and use it
													soajs.log.debug("Machine Public IP White Lister recipe not found, it will be automatically added");
													let defaultRecipe = require(path.join(__dirname, '../../../schemas/ipWhitelisterKubernetes.js'));
													BL.model.insertEntry(options.soajs, {
														collection: "catalogs",
														record: defaultRecipe
													}, (error, result) => {
														if(error) return vCb(error);
														return vCb(null, result[0]);
													});
												}
												else {
													return vCb(null, recipeRecord);
												}
											});
										},
										"getResource": (vCb) => {
	...
	"removeEnvFromDeployment": function (config, req, soajs, BL, deployer, cbMain) {
		...
		let inputmaskData = soajsUtils.cloneObj(soajs.inputmaskData);
		
		if (!inputmaskData.envCode) {
			if (inputmaskData.env) {
				inputmaskData.envCode = inputmaskData.env;
			}
			else {
		...
		let options1 = {
			collection: "infra",
			conditions: { "deployments.environments": { "$in": [inputmaskData.envCode.toUpperCase()] } }
		};
		BL.model.findEntry(soajs, options1, (error, infraProvider) => {
			checkIfError(soajs, cbMain, { config: config, error: error, code: 600 }, function () {
				checkIfError(soajs, cbMain, { config: config, error: !infraProvider, code: 600 }, function () {
					...
					infraProvider.deployments.forEach((oneDeployment) => {
						if (oneDeployment.environments && Array.isArray(oneDeployment.environments) && oneDeployment.environments.length > 0) {
							for (let i = oneDeployment.environments.length - 1; i >= 0; i--) {
								if (oneDeployment.environments[i] === inputmaskData.envCode.toUpperCase()) {
									soajs.log.debug(`Found environment ${inputmaskData.envCode} in deployment ${oneDeployment.id}`);
					...
					soajs.log.debug(`Updating infra Record:`, infraProvider.deployments);
					...
					BL.model.saveEntry(soajs, { collection: "infra", record: infraProvider }, (error) => {
						checkIfError(soajs, cbMain, { config: config, error: error, code: 600 }, function () {
							if (deleteDeployment) {
								...
								options.infra.stack = soajsUtils.cloneObj(deleteDeployment);
								...
								deployer.execute({
									...
									checkIfError(soajs, cbMain, {
										...
										soajs.log.debug(`Removing deployment ${deleteDeployment.id} from infra record`);
										...
										soajs.log.debug(`Updating infra Record Deployments to:`, infraProvider.deployments);
