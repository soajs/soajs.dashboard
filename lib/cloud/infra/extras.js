	...
	getExtras: (config, req, soajs, BL, deployer, cbMain) => {
		function getEnvironment(cb) {
			...
			utils.getEnvironment(soajs, BL.model, soajs.inputmaskData.envCode.toUpperCase(), (error, envRecord) => {
				checkIfError(soajs, cb, {config: config, error: error || !envRecord, code: 600}, () => {
					return cb(null, envRecord);
				});
			});
		}

		function getInfra(info, cb) {
							...
							if(infraRecord.technologies.includes('vm')){
								...
								deployer.execute({
									...
									checkIfError(soajs, cb, {config: config, error: error, code: 600}, function () {
										if(!extras.technologies.includes('vm')){
		...
		async.auto({
			getEnvironment,
			getInfra: ['getEnvironment', getInfra],
