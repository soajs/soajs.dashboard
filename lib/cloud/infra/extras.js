			...
			checkIfError(soajs, cbMain, {config, error, code: 600}, () => {
				...
				deployer.execute({
					...
					if (error) {
						...
						soajs.log.info(`Creating/Updating ${soajs.inputmaskData.params.section} ${soajs.inputmaskData.params.name} done!`)
					}
				});
				...
				checkIfError(soajs, cbMain, {config, error, code: 600}, () => {
					...
					deployer.execute({
						...
						if (error) {
							...
							soajs.log.info(`Creating/Updating ${soajs.inputmaskData.section} ${soajs.inputmaskData.name} done!`)
						}
					});
					return cbMain(null, true);
				});
