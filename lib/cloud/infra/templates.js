					...
					checkIfError(soajs, cbMain, {
						...
						async.each(templates, (oneTemplate, lCb) => {
							if (oneTemplate.type && oneTemplate.type === 'template') {
								...
								if(soajs.inputmaskData.fullTemplate){
									myTemplate.content = oneTemplate.content;
								}
								...
								checkIfError(soajs, cbMain, {config: config, error: err, code: 600}, function () {
									...
									checkIfError(soajs, cbMain, {config: config, error: count !== 0, code: 480}, function () {
										...
										if (soajs.inputmaskData.template.inputs) {
											options.record.inputs = JSON.stringify(soajs.inputmaskData.template.inputs);
										}
										if (soajs.inputmaskData.template.display) {
											options.record.display = JSON.stringify(soajs.inputmaskData.template.display);
										}
										...
										if (soajs.inputmaskData.template.imfv) {
											options.record.imfv = JSON.stringify(soajs.inputmaskData.template.imfv);
										}
				...
				BL.model.findEntry(soajs, opts, function (err, templateRecord) {
					checkIfError(soajs, cbMain, {config: config, error: err, code: 600}, function () {
						checkIfError(soajs, cbMain, {
							...
							templateRecord.inputs = JSON.stringify(soajs.inputmaskData.template.inputs);
							templateRecord.display = JSON.stringify(soajs.inputmaskData.template.display);
							...
							templateRecord.imfv = JSON.stringify(soajs.inputmaskData.template.imfv);
						...
						checkIfError(soajs, cbMain, {
							...
							deployer.execute({
								...
								checkIfError(soajs, cbMain, {config: config, error: error, code: 600}, function () {
									...
									checkIfError(soajs, cbMain, {config: config, error: foundSameName, code: 480}, function () {
