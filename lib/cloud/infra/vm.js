...
const vmLayerModule = {
	...
	destroyVM: (config, req, soajs, BL, deployer, cbMain) =>{
		async.auto({
			...
			"getTemplate": (mCb) => {
				//TODO: collection & conditions
				let opts2 = {
					collection: "",
					conditions: {}
				};
				BL.model.findEntry(soajs, opts2, function (err, template) {
					checkIfError(soajs, mCb, {config: config, error: err, code: 600}, () => {
						checkIfError(soajs, mCb, {
							config: config,
							error: !template,
			...
			"getTemplateState": (mCb) => {
				//TODO: collection & conditions
				let opts3 = {
					collection: "",
					conditions: {}
				};
				BL.model.findEntry(soajs, opts3, function (err, state) {
					checkIfError(soajs, mCb, {config: config, error: err, code: 600}, () => {
						checkIfError(soajs, mCb, {
							config: config,
							error: !state,
			...
			"getLayer": (mCb) => {
				...
				BL.model.findEntry(soajs, opts4, function (err, layer) {
					checkIfError(soajs, mCb, {config: config, error: err, code: 600}, () => {
						checkIfError(soajs, mCb, {
							config: config,
							error: !layer,
							...
							return mCb(null, layer);
						});
					});
				});
			},
			"callDeployer": ["getEnvironment", "getInfras", "getTemplate", "getLayer", "getTemplateState", (info, mCb) => {
				//TODO: fix options
				let options = {
					"template": info.getTemplate.name,
					"templateState": info.getTemplateState.state,
					"layerNAme": info.getLayer.name
				};
