	...
	deployVM: (config, req, soajs, BL, deployer, cbMain) =>{
		async.auto({
			...
			checkIfError(soajs, cbMain, {config: config, error: error}, () => {
				...
				deployer.execute({ type: "infra", name: info.getInfras.name, technology: soajs.inputmaskData.technology || "vm" }, method, options, (error, response) => {
					checkIfError(soajs, cbMain, {config: config, error: error}, () => {
						let opts = {
							collection: templateStateColName,
							versioning: true
	...
	getDeployVMStatus: (config, req, soajs, BL, deployer, cbMain) =>{
		...
		BL.model.findEntry(soajs, opts, function (err, templateState) {
			checkIfError(soajs, cbMain, {config: config, error: err, code: 600}, () => {
				...
				return cbMain(null, { "name": templateState.name, "infraId": templateState.infraId, "inputs": templateState.input, "template": templateState.name });
			});
		});
	},
	...
	destroyVM: (config, req, soajs, BL, deployer, cbMain) =>{
		async.auto({
			...
			checkIfError(soajs, cbMain, {config: config, error: error}, () => {
				...
				options.params.templateState = result.getInfraCodeTemplateState;
				deployer.execute({ type: "infra", name: info.getInfras.name, technology: soajs.inputmaskData.technology || "vm" }, 'deleteCluster', options, (error, response) => {
					checkIfError(soajs, cbMain, {config: config, error: error}, () => {
						let opts = {
							collection: templateStateColName,
							versioning: true
