				...
				checkIfError(soajs, cbMain, {config: config, error: update &&!options.params.templateState, code: 495}, () => {
					...
					async.auto({
						...
						checkIfError(soajs, cbMain, {config: config, error: error, code: 600}, () => {
							return cbMain(null, {"id": result.createVMLayerRecord ? result.createVMLayerRecord._id.toString() : null , "name": inputmaskDataClone.layerName, "infraId": inputmaskDataClone.infraId});
						});
					});
				});
			});
