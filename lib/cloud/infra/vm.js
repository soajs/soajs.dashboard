...
const vmLayerModule = {
    ...
    onBoardVM: (config, req, soajs, BL, deployer, cbMain) => {
        ...
        async.auto({
            ...
            "onBoardVM": ['getInfra', (info, mCb) => {
                let names = [];
                async.forEach(soajs.inputmaskData.layer.list, (oneList, lCb) => {
                    names.push(oneList.name);
                    return lCb();
                }, () => {
                    let options = {
                        soajs: soajs,
                    ...
                    if (!soajs.inputmaskData.release) {
                        ...
                        options.params.labels = {
                            ...
                            'soajs.layer.name': soajs.inputmaskData.layer.list[0].layer,
                            'soajs.network.name': soajs.inputmaskData.layer.list[0].network,
                        }
                    }
