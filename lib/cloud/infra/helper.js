...
function validateCustomId(soajs, BL, cb) {
	BL.model.validateCustomId(soajs, soajs.inputmaskData.id, cb);
}

const helper = {
	"getCommonData": function (config, soajs, BL, cbMain, cb) {
		//validate id
		validateCustomId(soajs, BL, function (err, id) {
					...
					checkIfError(soajs, cbMain, {config: config, error: err, code: 600}, function () {
						checkIfError(soajs, cbMain, {
							...
							if (!soajs.inputmaskData.bypassInfoCheck && (!info || info.length === 0)) {
								return cbMain({
									code: 404,
									msg: `Driver ${InfraRecord.name} Configuration not found!`
								});
							}
