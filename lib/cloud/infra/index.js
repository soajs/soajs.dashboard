						...
						"updateExtra": (vCb) => {
							deployer.execute({
								...
								checkIfError(soajs, vCb, {config: config, error: error, code: 600}, function () {
									...
									if(
										hash(oneInfra.technologies) === hash(extras.technologies) &&
										hash(oneInfra.drivers) === hash(extras.drivers) &&
										hash(oneInfra.templates) === hash(extras.templates) &&
										hash(oneInfra.override) === hash(extras.override)
									){
