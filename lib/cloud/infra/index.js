...
var BL = {
	...
	"list": function (config, soajs, deployer, cbMain) {
		...
		if(soajs.inputmaskData.id){
			...
			if(soajs.inputmaskData.type){
				if(soajs.inputmaskData.type === 'technology'){
					opts.conditions = {"name": "local" };
				}
				else{
					opts.conditions = {"name": { "$ne": "local" } };
				}
			}
		...
		BL.model.findEntries(soajs, opts, function (err, records) {
			checkIfError(soajs, cbMain, {config: config, error: err, code: 600}, function () {
				...
				async.mapSeries(records, (oneInfra, mCb) => {
					...
					async.auto({
						"updateExtra": (vCb) => {
							//buypass this if local deployment
							if (oneInfra.name === "local") {
							...
							deployer.execute({
								...
								checkIfError(soajs, vCb, {config: config, error: error, code: 600}, function () {
									let technologies = false, drivers = false, templs = false, override = false;
									
									if(extras.technologies){
										if(oneInfra.technologies && hash(oneInfra.technologies) === hash(extras.technologies)){
											technologies = true;
										}
									}
									else{
									...
									if(extras.drivers){
										if(oneInfra.drivers && hash(oneInfra.drivers) === hash(extras.drivers)){
											drivers = true;
										}
									}
									else{
									...
									if(extras.templates){
										if(oneInfra.templates && hash(oneInfra.templates) === hash(extras.templates)){
											templs = true;
										}
									}
									else{
									...
									if(extras.override){
										if(oneInfra.override && hash(oneInfra.override) === hash(extras.override)){
											override = true;
										}
									}
									else{
									...
									//attach provider specific to the response
									if(extras.providerSpecific){
										oneInfra.providerSpecific = extras.providerSpecific;
									}
									
									if(technologies && drivers && templs && override){
										return vCb(null, true);
									}
									soajs.log.debug(`Updating technologies of infra ${oneInfra.label}`);
									//update infra record and return it
									oneInfra.technologies = extras.technologies;
									oneInfra.drivers = extras.drivers;
									oneInfra.templates = extras.templates;
									oneInfra.override = extras.override;
		...
		BL.list(config, soajs, deployer, (error, infraProviders) => {
			checkIfError(soajs, cbMain, {config: config, error: error, code: 600}, function () {
				async.each(infraProviders, (oneProvider, mCb) => {
					if (oneProvider.label && oneProvider.label === soajs.inputmaskData.label) {
					...
					checkIfError(soajs, cbMain, {config: config, error: error, code: 173}, function () {
						...
						if (['docker', 'kubernetes'].indexOf(soajs.inputmaskData.name) !== -1) {
							...
							options.params.technology = technology;
						}
									...
									checkIfError(soajs, cbMain, {config: config, error: error, code: 600}, function () {
										...
										let opts = {
											...
											record: {
												...
												override: (extras.override) ? extras.override : {},
												deployments: []
											}
										};
	...
	"modify": function (config, soajs, deployer, cbMain) {
		...
		validateId(soajs, function (err) {
			checkIfError(soajs, cbMain, {config: config, error: err, code: 490}, function () {
				...
				BL.model.findEntry(soajs, opts, function (err, record) {
					checkIfError(soajs, cbMain, {config: config, error: err || !record, code: 600}, function () {
						...
						let technology = (record.name === 'local' && record.technologies) ? record.technologies[0] : 'cluster';
						options.infra = {
							name: record.name,
							api: soajs.inputmaskData.api,
							stack: {
							...
							checkIfError(soajs, cbMain, {config: config, error: error, code: 600}, function () {
								...
								deployer.execute({
									...
									checkIfError(soajs, cbMain, {config: config, error: error, code: 600}, function () {
										...
										record.override = (extras.override) ? extras.override : {};
		...
		function updateEnvironmentsUsingThisInfra(infraRecord){
			...
			if(!infraRecord.deployments || infraRecord.deployments.length === 0){ return cbMain(null, true); }
						...
						deployer.execute({
							...
							async.series({
								"analyzeError": (aCb) => {
									...
									//error is valid return it to stop execution
									return aCb(error);
								},
								"updateDB": (aCb) => {
									...
									BL.model.saveEntry(soajs, opts, (err) => {
										checkIfError(soajs, aCb, {config: config, error: err, code: 600}, () => {
											...
											BL.model.updateEntry(soajs, opts, (err) => {
												checkIfError(soajs, aCb, {config: config, error: err, code: 600}, aCb);
											});
										});
									});
								}
