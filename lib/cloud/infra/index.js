						...
						"updateExtra": (vCb) => {
							deployer.execute({
								...
								checkIfError(soajs, vCb, {config: config, error: error, code: 600}, function () {
									let technologies = true, drivers = false, templs = false, override = false;
									if(oneInfra.technologies && extras.technologies && hash(oneInfra.technologies) === hash(extras.technologies)){
										technologies = true;
									}
									
									if(oneInfra.drivers && extras.drivers && hash(oneInfra.drivers) === hash(extras.drivers)){
										drivers = true;
									}
									
									if(oneInfra.templates && extras.templates && hash(oneInfra.templates) === hash(extras.templates)){
										templs = true;
									}
									
									if(oneInfra.override && extras.override && hash(oneInfra.override) === hash(extras.override)){
										override = true;
									}
