					...
					async.series({
						...
						"getRemoteTemplates": (vCb) => {
							...
							deployer.execute({
								...
								checkIfError(soajs, vCb, {config: config, error: error, code: 600}, function () {
									let templateInputs = {};
									async.each(templates, (oneTemplate, fCb) => {
										if(oneTemplate.tags && oneTemplate.tags.type === 'inputsAndDisplay'){
											...
											deployer.execute({
												...
												checkIfError(soajs, fCb, {
													...
													let tempFilePath = __dirname + oneTemplate.name + oneTemplate.id + ".json";
										...
										checkIfError(soajs, vCb, {
											...
											async.each(templates, (oneTemplate, fCb) => {
												if(oneTemplate.tags && oneTemplate.tags.type === 'template'){
													let myTemplate = {
														_id: oneTemplate.id,
														...
														inputs: templateInputs[oneTemplate.id]
													};
				...
				BL.model.findEntry(soajs, opts, function (err, InfraRecord) {
					checkIfError(soajs, cbMain, {config: config, error: err, code: 600}, function () {
						...
						deployer.execute({
							...
							checkIfError(soajs, cbMain, {config: config, error: error && error.code !== 'MissingRequiredParameter', code: 600}, function () {
		...
		function doDeploy(InfraRecord, environmentRecord, info){
			...
			if(InfraRecord.templates && InfraRecord.templates.indexOf("local") !== -1){
				BL.model.findEntry(soajs, {
					...
					checkIfError(soajs, cbMain, {config: config, error: error, code: 600}, function () {
						options.params.template = template.content;
						deployer.execute({
							'type': 'infra',
							'driver': InfraRecord.name,
							'technology': 'cluster'
						...
						checkIfError(soajs, cbMain, {config: config, error: InfraRecord.templates.indexOf('external') === -1, code: 494}, function () {
							...
							if (inputsAndDisplay && inputsAndDisplay.length > 0) {
								...
								fs.writeFile(complimentaryFilePath, inputsAndDisplay, (error) => {
									...
									compOptions.params = {
										name: (req.soajs.inputmaskData.name + '__comp'),
										description: '',
