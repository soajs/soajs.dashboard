 ...
 */
function deployServiceOrDaemon(config, soajs, registry, res, cbMain) {
    var context = {
        name: '',
        origin: ''
    };
    ...
    function getCatalogRecipe(cb) {
        BL.model.validateCustomId(soajs, soajs.inputmaskData.recipe, function (recipeId) {
            utils.checkIfError(soajs, res, {config: config, error: !recipeId, code: 701}, function () {
                var opts = {
    ...
    function getEnvInfo(cb) {
        utils.getEnvironment(soajs, BL.model, soajs.inputmaskData.env, function (error, envRecord) {
            utils.checkErrorReturn(soajs, cbMain, {config: config, error: error || !envRecord, code: 600}, function () {
                utils.checkErrorReturn(soajs, cbMain, {config: config, error: envRecord.deployer.type === 'manual', code: 618}, function () {
    ...
    function getGitInfo(cb) {
        getGitRecord(soajs, soajs.inputmaskData.gitSource.owner + '/' + soajs.inputmaskData.gitSource.repo, function (error, accountRecord) {
            utils.checkErrorReturn(soajs, cbMain, {config: config, error: error || !accountRecord, code: 600}, function () {
    ...
    function constructDeployerParams(cb) {
        ...
        var serviceParams = {
            ...
            "image": image,
            "variables": [
                "NODE_ENV=" + context.catalog.recipe.buildOptions.env['NODE_ENV'] || 'production',
        ...
        if (context.catalog.recipe.buildOptions.settings && context.catalog.recipe.buildOptions.settings.accelerateDeployment) {
        ...
        if (context.catalog.recipe.deployOptions.env && Object.keys(context.catalog.recipe.deployOptions.env).length > 0) {
            serviceParams.variables = serviceParams.variables.concat(normalizeEnvVars(context.catalog.recipe.deployOptions.env));
        }
    ...
    getEnvInfo(function () {
        getCatalogRecipe(function () {
            checkPort(function(){ //TODO: review
...
var BL = {
    ...
    "deployService": function (config, soajs, registry, res, cbMain) {
        if (soajs.inputmaskData.type === 'nginx') {
            ...
            deployServiceOrDaemon(config, soajs, registry, res, cbMain);
        }
    },
     ...
     * @param {Response Object} res
	 */
    "deployCustomService": function (config, soajs, res, cbMain) {
        soajs.inputmaskData.deployConfig.replication.mode = verifyReplicationMode(soajs);
        ...
        utils.getEnvironment(soajs, BL.model, soajs.inputmaskData.env, function (error, envRecord) {
            utils.checkErrorReturn(soajs, cbMain, {config: config, error: error, code: 600}, function () {
                utils.checkErrorReturn(soajs, cbMain, {config: config, error: !envRecord, code: 446}, function () {
	 ...
	 * @param {Response Object} res
	 */
    "deployNginx": function (config, soajs, res, cbMain) {
        /*
                    ...
                    BL.model.findEntry(soajs, opts, function (error, srcRecord) {
                        utils.checkErrorReturn(soajs, cbMain, {config: config, error: error, code: 600}, function () {
                            ...
                            getGitRecord(soajs, srcRecord.src.owner + '/' + srcRecord.src.repo, function (error, tokenRecord) {
                                utils.checkErrorReturn(soajs, cbMain, {config: config, error: error, code: 600}, function () {
                                    utils.checkErrorReturn(soajs, cbMain, {config: config, error: !tokenRecord, code: 600}, function () {
     ...
     * @param {Response Object} res
     */
    "redeployService": function (config, soajs, res, cbMain) {
        utils.getEnvironment(soajs, BL.model, soajs.inputmaskData.env, function (error, envRecord) {
        ...
        function getUIConfig (cb) {
            ...
            BL.model.validateCustomId(soajs, soajs.inputmaskData.ui.id, function (error, id) {
                utils.checkErrorReturn(soajs, cbMain, {config: config, error: error, code: 701}, function () {
                    var opts = {
                        collection: colls.staticContent,
                        condition: { _id: id }
                    };
                    BL.model.findEntry(soajs, opts, function (error, record) {
                        utils.checkErrorReturn(soajs, cbMain, {config: config, error: error, code: 600}, function () {
                            utils.checkErrorReturn(soajs, cbMain, {config: config, error: !record, code: 905}, function () {
                                getGitRecord(soajs, record.src.owner + '/' + record.src.repo, function (error, gitRecord) {
                                    utils.checkErrorReturn(soajs, cbMain, {config: config, error: error, code: 600}, function () {
                                        utils.checkErrorReturn(soajs, cbMain, {config: config, error: !gitRecord, code: 757}, function () {
                                            var result = {
                                                available: true,
                                                values: {
