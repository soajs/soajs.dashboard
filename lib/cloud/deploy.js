...
var BL = {
	...
	"deployService": function (config, soajs, registry, res, cbMain) {
		...
		function getAnalyticsEsInfo(cb) {
			utils.getEnvironment(soajs, BL.model, soajs.inputmaskData.env, function (error, envRecord) { //402
				if (error) {
					return cb(error);
				}

				if (!envRecord.dbs || !envRecord.dbs.databases) return cb();

				async.filter(envRecord.dbs.databases, function (oneDb, callback) {
		...
		function getLatestSOAJSImageInfo(cb){
			...
			myUrl = myUrl.replace("%organization%", context.catalog.recipe.deployOptions.image.prefix).replace("%imagename%", context.catalog.recipe.deployOptions.image.name);
			myUrl += context.catalog.recipe.deployOptions.image.tag;
			let opts = {
				method: 'GET',
				url: myUrl,
			...
			async.series([getDashboardConnection, getAnalyticsEsInfo, getControllerDomain, getGitInfo, getServiceDaemonInfo], function () {
				...
				switch (context.catalog.type) {
					...
					default:
						...
						var serviceLabel = context.envRecord.code.toLowerCase() + "-" + context.catalog.type + "-" + shortid.generate();
						context.labels = {
				...
				if(soajs.inputmaskData.action === 'rebuild'){
					if(context.platform === 'kubernetes' && context.catalog.recipe.deployOptions.image.pullPolicy !== 'Always'){
						...
						context.labels['service.image.ts'] = context.imageInfo.last_updated;
					}
				}
				else{
					context.labels['service.image.ts'] = context.imageInfo.last_updated;
