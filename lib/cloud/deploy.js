...
var helpers = require("../helpers/cloud/deploy.js");
		...
		function getServiceDaemonInfo(cb) {
			// if (context.catalog.type === 'soajs') {
				...
				BL.model.findEntry(soajs, opts, function (error, dbRecord) {
					utils.checkErrorReturn(soajs, cbMain, {config: config, error: error || (!dbRecord && context.catalog.type === 'soajs'), code: 600}, function () {
			...
			async.series([getDashboardConnection, getAnalyticsEsInfo, getControllerDomain, getGitInfo, getServiceDaemonInfo], function () {
				...
				switch (context.catalog.type) {
					...
					default:
						...
						if (context.serviceName) { //if a service record was found, use it to get service name
							serviceLabel = context.envRecord.code.toLowerCase() + "-" + context.serviceName;
						...
						context.labels = {
							...
							"soajs.service.version": soajs.inputmaskData.custom.version || "1"
						};
