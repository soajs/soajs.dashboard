...
function checkPort(options, cb) {
    ...
    async.each(ports, function (onePort, callback) {
        ...
        if (error && error.wrongPort) {
            ...
            return options.cbMain({"code": 824, "msg": errMsg});
        }
        else {
...
function deployServiceOrDaemon(config, soajs, registry, res, cbMain) {
    ...
    getEnvInfo(function () {
        getCatalogRecipe(soajs, res, config, function (catalogRecord) {
            ...
            checkPort({ soajs: soajs, cbMain: cbMain, res: res, config: config, context: context }, function() {
...
var BL = {
    ...
    "deployService": function (config, soajs, registry, res, cbMain) {
        if (soajs.inputmaskData.type === 'nginx') {
            BL.deployNginx(config, soajs, res, cbMain);
        }
