...
function computeCatalogEnvVars(context, soajs, cb) {
    ...
    async.map(catalogEnvs, function (oneEnvName, callback) {
        ...
        if (oneEnv.type === 'static') {
            return callback(null, oneEnvName + '=' + oneEnv.value);
        }
        // if env variable is of type userInput, get value from request body, if not found see use default value
            ...
            if (value) return callback(null, oneEnvName + '=' + value);
            ...
            if (oneEnv.value && context.variables[oneEnv.value]) {
                value = context.variables[oneEnv.value];
            }

            return callback(null, oneEnvName + '=' + value);
        }
    }, cb);
}
...
function deployServiceOrDaemon(config, soajs, registry, res) {
    ...
    function buildAvailableVariables(cb) {
        context.variables = {
            '$NODE_ENV': 'production',
        ...
        if (soajs.inputmaskData.type === 'daemon' && soajs.inputmaskData.contentConfig && soajs.inputmaskData.contentConfig.daemon) {
            context.variables['$SOAJS_DAEMON_GRP_CONF'] = soajs.inputmaskData.contentConfig.daemon.grpConfName;
        }
        ...
        if (context.dbRecord.src && context.dbRecord.src.main) {
            context.variables['$SOAJS_SRV_MAIN'] = context.dbRecord.src.main;
        }

        //adding info about database servers
        for (var i = 0; i < context.mongoDbs.length; i++) {
            context.variables["$SOAJS_MONGO_IP_" + (i + 1)] = context.mongoDbs[i].host;
            context.variables["$SOAJS_MONGO_PORT_" + (i + 1)] = context.mongoDbs[i].port;
        }

        //if database prefix exists, add it to env variables
        if (context.dbConfig && context.dbConfig.prefix) {
            context.variables["$SOAJS_MONGO_PREFIX"] = context.dbConfig.prefix;
        }
        ...
        if (context.mongoCred && context.mongoCred.username && context.mongoCred.password) {
            context.variables["$SOAJS_MONGO_USERNAME"] = context.mongoCred.username;
            context.variables["$SOAJS_MONGO_PASSWORD"] = context.mongoCred.password;
        }
        ...
        if (context.clusterInfo.URLParam && context.clusterInfo.URLParam.replicaSet && context.clusterInfo.URLParam.replicaSet) {
            context.variables["$SOAJS_MONGO_RSNAME"] = context.clusterInfo.URLParam.replicaSet;
        }

        //if authSource is set, add it to env variables
        if (context.clusterInfo.URLParam && context.clusterInfo.URLParam.authSource) {
            context.variables["$SOAJS_MONGO_AUTH_DB"] = context.clusterInfo.URLParam.authSource;
        }
        ...
        if (context.accountRecord.token) {
            ...
            context.variables["$SOAJS_GIT_TOKEN"] = context.accountRecord.token;
        }
        ...
        if (soajs.inputmaskData.contentConfig && soajs.inputmaskData.contentConfig.service && soajs.inputmaskData.contentConfig.service.gc) {
            context.variables["$SOAJS_GC_NAME"] = soajs.inputmaskData.contentConfig.service.gcName;
            context.variables["$SOAJS_GC_VERSION"] = soajs.inputmaskData.contentConfig.service.gcVersion;
        }
                        ...
                        getServiceDaemonInfo(function () {
                            checkMemoryRequirement(function () {
                                constructDeployerParams(function () {
                                    buildAvailableVariables(function () {
                                        computeCatalogEnvVars(context, soajs, function (error, variables) {
                                            if (error) {
    ...
    "deployNginx": function (config, soajs, res) {
        ...
        function constructDeployerParams() {
            ...
            if (context.catalog.recipe.buildOptions && context.catalog.recipe.buildOptions.env) {
                if ((context.catalog.recipe.buildOptions.env['SOAJS_NX_API_HTTPS'] && context.catalog.recipe.buildOptions.env['SOAJS_NX_API_HTTPS'].value === "1") ||
                    (context.catalog.recipe.buildOptions.env['SOAJS_NX_SITE_HTTPS'] && context.catalog.recipe.buildOptions.env['SOAJS_NX_SITE_HTTPS'].value === "1")) {
                    nginxParams.ssl = { "enabled": true };

                    if ((context.catalog.recipe.buildOptions.env['SOAJS_NX_CUSTOM_SSL'] && context.catalog.recipe.buildOptions.env['SOAJS_NX_CUSTOM_SSL'].value === "1") &&
                        (context.catalog.recipe.buildOptions.env['SOAJS_NX_SSL_SECRET'] && context.catalog.recipe.buildOptions.env['SOAJS_NX_SSL_SECRET'].value)) {
