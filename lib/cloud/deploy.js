			...
			async.each(ports, function (onePort, callback) {
				...
				if (error && error.wrongPort) {
					...
					async.map(ports, function (onePort, callback) {
						...
						context.catalog.recipe.deployOptions.ports = updatedPorts;
						return cb();
					});
				}
			});
				...
				switch (context.catalog.type) {
					case 'soajs':
						context.labels = {
							...
							"soajs.service.group": (context.serviceGroup) ? context.serviceGroup.toLowerCase().replace(/\s+/g, '-').replace(/_/g, '-') : context.catalog.labels['soajs.service.group'],
						...
						if(soajs.inputmaskData.custom && soajs.inputmaskData.custom.version){
							context.labels["soajs.service.version"] = soajs.inputmaskData.custom.version;
						}
					...
					case 'nginx':
						...
						break;
					default:
						context.labels = context.catalog.labels;
						break;
				}
				doDeploy();
			});
