...
var BL = {
    ...
    /**
    * Check if heapster is deployed and optionally build a catalog recipe for it
    * @param  {Object}   config
    * @param  {Object}   soajs
    * @param  {Object}   deployer
    * @param  {Function} cb
                        ...
                        deployer.listKubeServices(options, function(error, services) {
                            utils.checkErrorReturn(soajs, cb, { config: config, error: error }, function() {
                                async.detect(services, function(oneService, callback) {
                                    ...
                                    if(!heapsterService) return cb(null, true); //heapster is not deployed

                                    var selectors = '';
                                    if(heapsterService.spec && heapsterService.spec.selector) {
                                        selectors = helpers.buildSelectors(heapsterService.spec.selector);
                                    }

                                    //tools needed by helper functions
                                    var params = { BL: BL, soajs: soajs, config: config, utils: utils };

                                    options.params = { selectors: selectors };
                                    deployer.getKubeDeployment(options, function(error, deployment) {
                                        utils.checkErrorReturn(soajs, cb, { config: config, error: error }, function() {
                                            if(deployment && Object.keys(deployment).length > 0) {
                                                if(helpers.checkIfValid(deployment, 'deployment')) {
                                                    return cb(null, true); //heapster is deployed and its api server is exposed
                                                }
                                                else {
                                                    return helpers.buildCatalogRecipe(deployment, heapsterService, 'deployment', params, cb);
                                                }
                                            }
                                            else {
                                                options.params = { selectors: selectors };
                                                deployer.getKubePod(options, function(error, pod) {
                                                    utils.checkErrorReturn(soajs, cb, { config: config, error: error }, function() {
                                                        if(pod && Object.keys(pod).length > 0) {
                                                            if(helpers.checkIfValid(pod, 'pod')) {
                                                                return cb(null, true); //heapster is deployed and its api server is exposed
                                                            }
                                                            else {
                                                                return helpers.buildCatalogRecipe(pod, heapsterService, 'pod', params, cb);
                                                            }
                                                        }
                                                        else {
                                                            return cb({ code: 988, msg: config.errors[988] });
                                                        }
                                                    });
                                                });
                                            }
