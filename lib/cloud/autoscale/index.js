    ...
    set: function(config, soajs, deployer, cb) {
        ...
        function update(services, deployer, options, cb) {
            async.each(services, function (oneService, callback) {
	            ...
	            deployer.execute({'type': 'container', 'driver': options.strategy}, 'getAutoscaler', autoscaleOptions, (error, autoscaler) =>{
                    ...
                    if(!autoscaler || !autoscaler.replicas || !(autoscaler.replicas.min && autoscaler.replicas.max)) {
	                    deployer.execute({'type': 'container', 'driver': options.strategy}, 'createAutoscaler', autoscaleOptions, callback);
                    }
                    else {
                        if(!compareAutoscalerConfigs(soajs.inputmaskData.autoscaler, autoscaler)) {
	                        deployer.execute({'type': 'container', 'driver': options.strategy}, 'updateAutoscaler', autoscaleOptions, callback);
                        }
                        else {
        ...
        function turnOff(services, deployer, options, cb) {
            async.each(services, function (oneService, callback) {
	            ...
	            deployer.execute({'type': 'container', 'driver': options.strategy}, 'getAutoscaler', autoscaleOptions, (error, autoscaler) =>{
		            ...
		            deployer.execute({'type': 'container', 'driver': options.strategy}, 'deleteAutoscaler', autoscaleOptions, callback);
                });
            }, cb);
        }
