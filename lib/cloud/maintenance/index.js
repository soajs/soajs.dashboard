		...
		async.auto({
			...
			"callDeployer": ["checkIfServiceOrDaemon", "getEnvironment", "getInfra", (info, mCb) =>{
				...
				if(info.checkIfServiceOrDaemon && info.checkIfServiceOrDaemon.port){
					if (info.checkIfServiceOrDaemon.maintenance && info.checkIfServiceOrDaemon.maintenance.port){
						if (info.checkIfServiceOrDaemon.maintenance.port.type === "maintenance"){
							options.params.maintenancePort = info.checkIfServiceOrDaemon.port + info.getEnvironment.services.config.ports.maintenanceInc;
						}
						else if (info.checkIfServiceOrDaemon.maintenance.port.type === "inherit"){
							options.params.maintenancePort = info.checkIfServiceOrDaemon.port;
						}
						else if (info.checkIfServiceOrDaemon.maintenance.port.type === "custom"){
							options.params.maintenancePort =info.checkIfServiceOrDaemon.maintenance.port.value;
						}
					}
					else {
						options.params.maintenancePort = info.checkIfServiceOrDaemon.port + info.getEnvironment.services.config.ports.maintenanceInc;
				...
				soajs.log.info(`Maintenance operation ${soajs.inputmaskData.operation} on port ${options.params.maintenancePort} started!`);
				deployer.execute({ type: "infra", name: info.getInfra.name, technology: technology }, 'maintenance', options, mCb);
			}]
		}, (error, results) => {
			checkIfError(soajs, cbMain, { config: config, error: error, code: (error && error.code) ? error.code : 600 }, function () {
				return cbMain(error, results.callDeployer);
			});
		});
	},
