...
var BL = {
	...
	"deployService": function (config, soajs, deployer, cbMain) {
		...
		function getGitInfo(cb) {
								...
								if(dbRecord.repos[0].type === 'config'){
									...
									dbRecord.repos[0].configSHA.forEach((oneSubRepo) => {
										if(['custom','static','service','daemon'].indexOf(oneSubRepo.contentType) !== -1 && oneSubRepo.contentName === soajs.inputmaskData.custom.sourceCode[oneSourceCode].subName){
											handleOneRepoLevel(dbRecord, oneSourceCode, soajs.inputmaskData.custom.sourceCode[oneSourceCode].subName);
										}
									});
								}
								else if (dbRecord.repos[0].type !== 'config' && context.catalog.recipe.type === 'server' ){
			...
			function handleOneRepoLevel(dbRecord, oneSourceCode, subName){
				let mapping = {
				...
				let repoName = (subName) ? subName : soajs.inputmaskData.custom.sourceCode[oneSourceCode].repo.split("/")[1];
				context.variables['$SOAJS_GIT_REPO'] = repoName;
				if(!context.catalog.recipe.buildOptions.env['SOAJS_GIT_REPO']) {
					context.catalog.recipe.buildOptions.env['SOAJS_GIT_REPO'] = {
						"type": "computed",
						"value": "$SOAJS_GIT_REPO"
				...
				if(soajs.inputmaskData.custom.sourceCode[oneSourceCode].path){
					context.variables['$SOAJS_GIT_PATH'] = soajs.inputmaskData.custom.sourceCode[oneSourceCode].path;
