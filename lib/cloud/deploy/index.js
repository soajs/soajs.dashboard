												...
												checkIfError(soajs, cbMain, {
													...
													getCatalogRecipe (function (err) {
														checkIfError(soajs, cbMain, { config: config, error: err }, function () {
															...
															//pass commands and environment variables to vm driver
															if(context.catalog.recipe && context.catalog.recipe.buildOptions) {
																if(context.catalog.recipe.buildOptions.env) {
																	options.params.env = context.catalog.recipe.buildOptions.env;
																}
																if(context.catalog.recipe.buildOptions.cmd && context.catalog.recipe.buildOptions.cmd.deploy) {
																	options.params.command = context.catalog.recipe.buildOptions.cmd.deploy;
																}
															}

															//pass ports to vm driver
															if(context.catalog.recipe && context.catalog.recipe.ports && Array.isArray(context.catalog.recipe.ports)) {
																options.params.ports = context.catalog.recipe.ports;
															}
