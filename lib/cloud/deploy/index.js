				...
				helpers.registerCiCd(soajs, options, config, BL, (error) => {
					BL.checkIfError(soajs, cbMain, {config: config, error: error, code: 600}, () => {
						if (deployment === "vm") {
							...
							req.soajs.inputmaskData.catalog.recipe.buildOptions.cmd.deploy.env = helpers.computeCatalogEnvVars(options, req.soajs.inputmaskData.catalog.recipe.buildOptions.cmd.deploy.env);
							if (vms && vms.length > 0) {
