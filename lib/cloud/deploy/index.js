...
let BL = {
	...
	"deployService": (config, req, deployer, cbMain) => {
		...
		let deployment, vms;
		if (req.soajs.inputmaskData.deployConfig && req.soajs.inputmaskData.deployConfig.type === 'vm') {
			deployment = "vm";
			vms = req.soajs.inputmaskData.vms ? req.soajs.inputmaskData.vms : [];
		}
		else {
		...
		async.auto({
			getEnvInfo: (fCb) => {
				...
				utils.getEnvironment(soajs, BL.model, soajs.inputmaskData.env.toUpperCase(), (error, envRecord) => {
					BL.checkIfError(soajs, fCb, {config: config, error: error || !envRecord, code: 600}, () => {
						...
						options.params.data.variables['$SOAJS_PROFILE'] = envRecord.profile;
								...
								BL.model.findEntry(soajs, opts, (error, catalogRecord) => {
									BL.checkIfError(soajs, callback, {config: config, error: error, code: 600}, () => {
										BL.checkIfError(soajs, callback, {
											...
											if(catalogRecord.recipe && catalogRecord.recipe.deployOptions && catalogRecord.recipe.deployOptions.readinessProbe){
			...
			getDashboardConnectionAndExtKey: ['getCatalogAndInfra', (result, fCb) => {
				soajs.log.debug('get Dashboard ConnectionAndExtKey');
				async.parallel({
					//get tenant ext key
					getDefaultUIExtKey: (callback) => {
						helpers.getDefaultUIExtKey(soajs, config, BL, callback);
					},

					//get dashboard mongo connection
					getDashboardConnection: (callback) => {
			...
			getCDRecord: ['getDashboardConnectionAndExtKey', 'getEnvInfo', (result, fCb) => {
				soajs.log.debug('get cd record');
				if (deployment === "vm" || soajs.inputmaskData.action !== 'rebuild') {
					return fCb();
				}
				let recordType;
				if (req.soajs.inputmaskData.custom && (req.soajs.inputmaskData.custom.type === 'service' || req.soajs.inputmaskData.custom.type === 'daemon' || req.soajs.inputmaskData.custom.type === 'other')) {
					recordType = 'cd';
				}
				if (recordType) {
					let cond = {
					...
					BL.model.findEntry(soajs, cond, function (error, ciCdRecord) {
						BL.checkIfError(soajs, fCb, {
							...
							if (req.soajs.inputmaskData.custom) {
								let name = req.soajs.inputmaskData.custom.name;
								let version = 'v' + req.soajs.inputmaskData.custom.version;
								if (ciCdRecord && ciCdRecord[env] && ciCdRecord[env][name][version]
									...
									if (ciCdRecord[env][name][version].options.custom.exposeServicePort
										...
										req.soajs.inputmaskData.custom.exposeServicePort = ciCdRecord[env][name][version].options.custom.exposeServicePort;
										req.soajs.inputmaskData.custom.exposeServicePortValue = ciCdRecord[env][name][version].options.custom.exposeServicePortValue;
									}
								}
							}
							return fCb();
				...
				helpers.registerCiCd(soajs, options, config, BL, (error) => {
					BL.checkIfError(soajs, cbMain, {config: config, error: error, code: 600}, () => {
						...
						if (deployment === "vm") {
							...
							req.soajs.inputmaskData = options.params.inputmaskData;
							...
							req.soajs.inputmaskData.catalog.recipe.buildOptions.cmd.deploy.env = helpers.buildAvailableVariables(options);
							req.soajs.inputmaskData.catalog.recipe.buildOptions.cmd.deploy.env = helpers.computeCatalogEnvVars(config, options, req.soajs.inputmaskData.catalog.recipe.buildOptions.cmd.deploy.env);
							if (vms && vms.length > 0) {
								vmModule.init(dbModel, function (error, model) {
									BL.checkIfError(req.soajs, cbMain, {
										config: config,
										error: error ,
										code: 407
									}, () => {
										options.vms = vms;
										helpers.updateVmPortsAndDeployResource(deployer, model, options, req.soajs, config, function(error, result) {
							...
							async.auto({
											...
											if (recordType) {
												...
												BL.model.findEntry(soajs, cond, function (error, ciCdRecord) {
													BL.checkIfError(soajs, mCb, {
														...
														let version = 'v' + optionsCopy.inputmaskData.custom.version;
								...
								BL.checkIfError(soajs, cbMain, {config: config, error: error}, () => {
									req.soajs.log.info("Service Deployed Successfully!");
