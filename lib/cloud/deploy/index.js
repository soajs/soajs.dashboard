...
var BL = {
	...
	"deployService": function (config, soajs, registry, deployer, cbMain) {
		...
		function getDefaultUIExtKey(cb){
			...
			if(soajs.inputmaskData.env.toUpperCase() === 'PORTAL') {
				tenantCondition = { code: soajs.inputmaskData.env.toUpperCase() }; //tenant code = env code = PORTAL
				packageName = soajs.inputmaskData.env.toUpperCase() + '_MAIN';
			}
			else {
