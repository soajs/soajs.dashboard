								...
								checkIfError(soajs, cbMain, {
									...
									if (record.service.labels['soajs.service.type'] === 'service' || record.service.labels['soajs.service.type'] === 'daemon' || record.service.labels['soajs.service.type'] === 'other') {
										...
										async.each(record.service.env, function (oneEnv, callback) {
											...
											return BL.deployService(config, soajs, deployer, updateCiCdRecord.bind(this, soajs.inputmaskData, 'cd', cbMain));
										});
									}
									else {
										...
										return BL.deployService(config, soajs, deployer, updateCiCdRecord.bind(this, soajs.inputmaskData, 'resource', cbMain));
									}
									
									function updateCiCdRecord(opts, cbMain, recordType, err){
										if (err && err.code){
											return cbMain(err);
										}
										var cond = {
