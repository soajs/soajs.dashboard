...
var BL = {
	...
	"deployService": function (config, req, soajs, deployer, cbMain) {
		...
		console.log(JSON.stringify(soajs.inputmaskData, null, 2));
		if (soajs.inputmaskData.deployConfig.type && soajs.inputmaskData.deployConfig.type === "vm") {
			checkIfError(soajs, cbMain, {
				config: config,
				error: !soajs.inputmaskData.deployConfig.infra && !soajs.inputmaskData.deployConfig.location && !soajs.inputmaskData.deployConfig.vmConfiguration,
				code: 490
			}, function () {
										...
										checkIfError(soajs, cbMain, {
											...
											checkIfError(soajs, cbMain, {
												...
												options.params = {
													...
													instance: {
														name: soajs.inputmaskData.deployConfig.custom.name,
														size: soajs.inputmaskData.deployConfig.vmConfiguration.flavor,
														admin: soajs.inputmaskData.deployConfig.vmConfiguration.adminAccess
													}
												};
												option.infra = {
													api: infraRecord.api
