	...
	"deployService": function (config, req, deployer, cbMain) {
		...
		async.auto({
			...
			function doDeploy() {
				...
				deployer.execute({
					'type': 'infra',
					'driver': options.params.infra.name,
					...
					updateEnv: (callback) => {
						...
						BL.model.saveEntry(soajs, opts, function (error) {
							if (error) {
								...
								soajs.log.info("Environment record updated successfully!");
							}
							return callback();
						});
					},
					updateInfra: (callback) => {
						let infra = options.params.infra;
						delete infra.stack;
						let opts = {
							collection: colls.infra,
							record: infra
						...
						BL.model.saveEntry(soajs, opts, function (error) {
							if (error) {
								...
								soajs.log.info("Infra record updated successfully!");
							}
							return callback();
						});
					},
						...
						if (record.service.labels['soajs.resource.id']) {
							...
							BL.model.findEntry(soajs, resourceOpts, function (error, resource) {
								if (error) {
									...
									if (resource && resource.config && resource.config.servers
										&& resource.config.servers[0] && resource.config.servers[0].host) {
			...
			options.params.infra.stack = utils.getDeploymentFromInfra(options.params.infra, options.env);
			if (options.params.infra.stack) {
				options.params.infra.stack = options.params.infra.stack[0];
			}
