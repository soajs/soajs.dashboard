		...
		async.auto({
			...
			getCatalogAndInfra: (fCb) => {
				async.auto({
					...
					inspectService: function (callback){
						if (soajs.inputmaskData.action === 'rebuild'){
							...
							deployer.execute({'type': 'container', 'driver': options.strategy}, 'inspectService', options, (error, record) =>{
								checkIfError(soajs, callback, {config: config, error: error}, function () {
					...
					getCatalogRecipe: ['inspectService', function(callback) {
					...
					getInfraRecord: ['inspectService', function(callback) {
						...
						if (soajs.inputmaskData.infraId) {
							...
							BL.model.findEntry(soajs, opts, function (err, infraRecord) {
								checkIfError(soajs, callback, {config: config, error: err, code: 600}, () => {
									checkIfError(soajs, callback, {config: config, error: !infraRecord, code: 600}, () => {
										return callback(null, infraRecord);
									});
								});
							});
					...
					options.params.infra = result.getInfraRecord;
					options.params.inputmaskData = soajs.inputmaskData;
					return fCb(null, result);
				});
			},
			
			getDashboardConnectionAndExtKey: [ 'getCatalogAndInfra', (fCb) => {
				async.parallel({
					//get tenant ext key
			...
			getGitAndServiceDaemonInfo: [ 'getDashboardConnectionAndExtKey', (fCb) => {
				async.parallel({
					...
					getGitInfo: function(callback) {
							...
							helpers.getGitRecord(soajs, soajs.inputmaskData.gitSource.owner + '/' + soajs.inputmaskData.gitSource.repo, BL, function (error, accountRecord) {
								...
								checkIfError(soajs, cbMain, {
									...
									if (accountRecord.token) {
										...
										options.params.variables["$SOAJS_GIT_TOKEN"] = accountRecord.token;
									}
									options.params.variables['$SOAJS_GIT_PROVIDER'] = accountRecord.providerName;
									options.params.variables['$SOAJS_GIT_DOMAIN'] = accountRecord.domain;
									
									if (soajs.inputmaskData.gitSource && soajs.inputmaskData.gitSource.owner) {
										options.params.variables['$SOAJS_GIT_OWNER'] = soajs.inputmaskData.gitSource.owner;
									}
									if (soajs.inputmaskData.gitSource && soajs.inputmaskData.gitSource.repo) {
										options.params.variables['$SOAJS_GIT_REPO'] = soajs.inputmaskData.gitSource.repo;
									}
									if (soajs.inputmaskData.gitSource && soajs.inputmaskData.gitSource.branch) {
										options.params.variables['$SOAJS_GIT_BRANCH'] = soajs.inputmaskData.gitSource.branch;
									}
									if (soajs.inputmaskData.gitSource && soajs.inputmaskData.gitSource.commit) {
										options.params.variables['$SOAJS_GIT_COMMIT'] = soajs.inputmaskData.gitSource.commit;
									}
										...
										if (error) {
											...
											if (dbRecord.repos[0].type === 'config') {
												...
												for (let entry in mapping) {
													options.params.variables["$" + mapping[entry]] = dbRecord[entry];
												}
												
												options.params.variables['$SOAJS_CONFIG_REPO_NAME'] = soajs.inputmaskData.custom.sourceCode[oneSourceCode].repo.split("/")[1];
												
												options.params.variables['$SOAJS_CONFIG_REPO_BRANCH'] = soajs.inputmaskData.custom.sourceCode[oneSourceCode].branch;
												
												if (soajs.inputmaskData.custom.sourceCode[oneSourceCode].commit) {
													options.params.variables['$SOAJS_CONFIG_REPO_COMMIT'] = soajs.inputmaskData.custom.sourceCode[oneSourceCode].commit;
												}
												
												if (soajs.inputmaskData.custom.sourceCode[oneSourceCode].path) {
													options.params.variables['$SOAJS_CONFIG_REPO_PATH'] = soajs.inputmaskData.custom.sourceCode[oneSourceCode].path;
												}
											}
						...
						function handleOneRepoLevel(dbRecord, oneSourceCode, subPath) {
							...
							options.params.variables['$SOAJS_GIT_REPO'] = soajs.inputmaskData.custom.sourceCode[oneSourceCode].repo.split("/")[1];
							options.params.variables['$SOAJS_GIT_BRANCH'] = soajs.inputmaskData.custom.sourceCode[oneSourceCode].branch;
							
							if (soajs.inputmaskData.custom.sourceCode[oneSourceCode].commit) {
								options.params.variables['$SOAJS_GIT_COMMIT'] = soajs.inputmaskData.custom.sourceCode[oneSourceCode].commit;
							}
							...
							if (soajs.inputmaskData.custom.sourceCode[oneSourceCode].path || subPath) {
								...
								options.params.variables['$SOAJS_GIT_PATH'] = path;
							}
						}
					},
					
					// service and daemon information
					getServiceDaemonInfo: function(callback) {
							...
							checkIfError(soajs, cbMain, {
								...
								if (!dbRecord) {
									...
									if (options.params.catalog.type === 'soajs' || ((options.params.catalog.type === 'service' || options.params.catalog.type === 'daemon') && options.params.catalog.subtype === 'soajs')) {
										...
										options.params.variables['$SOAJS_SRV_PORT_MAINTENANCE'] = dbRecord.maintenancePort;
									}
									...
									var platform = options.soajs.registry.deployer.selected.split('.')[1];
			...
			portComputing: ['getGitAndServiceDaemonInfo', (result, fCb) => {
				...
				soajs.log.info('stepfive start');
				var ports = options.params.catalog.recipe.deployOptions.ports || [];
				if (ports.length === 0 && result.getGitAndServiceDaemonInfo.getServiceDaemonInfo) {
					...
					async.each(ports, function (onePort, callback) {
						...
						if (result.additionalInformation.getServiceDaemonInfo) {
							if (result.additionalInformation.getServiceDaemonInfo.servicePort
								&& result.additionalInformation.getServiceDaemonInfo.servicePort.target === onePort.target) {
								servicePort = true;
							}
							if (result.additionalInformation.getServiceDaemonInfo.maintenancePort) {
						...
						if (result.additionalInformation.getServiceDaemonInfo && result.additionalInformation.getServiceDaemonInfo.servicePort && !servicePort) {
							ports.push(result.additionalInformation.getServiceDaemonInfo.servicePort);
						}
						if (result.additionalInformation.getServiceDaemonInfo && result.additionalInformation.getServiceDaemonInfo.maintenancePort && !maintenancePort) {
							ports.push(result.additionalInformation.getServiceDaemonInfo.servicePort);
						}
			...
			//call cloostro and return callback
			function doDeploy() {
				deployer.execute({'type': 'infra', 'driver': options.infra.name, 'technology': technology}, 'deployService', options, (error, data) =>{
					utils.checkErrorReturn(soajs, cbMain, { config: config, error: error }, function() {
						...
						setTimeout(function(){
							...
							deployer.execute({'type': 'infra', 'driver': options.infra.name, 'technology': technology}, 'inspectService', opts, (error, record) =>{
								utils.checkErrorReturn(soajs, cbMain, { config: config, error: error }, function() {
				...
				async.parallel({
					updateEnv: (callback) => {
						...
						BL.model.saveEntry(soajs, opts, function (error) {
							if (error) {
								...
								soajs.log.info("Resource record updated successfully!");
							}
							return callback();
						});
					},
					...
					updateResource: (callback) => {
						if (record.service.labels['soajs.resource.id']){
							...
							BL.model.findEntry(soajs, resourceOpts, function (error, record) {
								if (error) {
									soajs.log.error(error);
									return callback();
								}
								else {
									if (record && record.config && record.config.servers
										&& record.config.servers[0] && record.config.servers[0].host) {
										record.config.servers[0].host = ip.toString();
										resourceOpts = {
											collection: colls.resources,
											record: record
										};
										BL.model.saveEntry(soajs, resourceOpts, function (error) {
											if (error) {
												soajs.log.error(error);
											}
											else {
					...
					soajs.log.info("Update successfully!");
				});
			}
			function updateCiCdRecord(opts, recordType, callback) {
				...
				BL.model.findEntry(soajs, cond, function (error, record) {
					checkIfError(soajs, cbMain, {config: config, error: error}, function () {
						...
						if (recordType === 'cd'){
							gitSource = {
								owner: record.service.labels['service.owner'] || null,
								repo: record.service.labels['service.repo'] || null,
								branch:  soajs.inputmaskData.custom.branch || null,
								commit: soajs.inputmaskData.custom.commit || null
							};
						}
						if (record && record[env] && record[env][name]) {
							if (record[env][name]["v" + version]) {
								...
								if (record[env][name]["v" + version].options && opts.custom) {
									record[env][name]["v" + version].options.custom = opts.custom;
								}
								
								if (record[env][name]["v" + version].options && opts.gitSource) {
									record[env][name]["v" + version].options.gitSource = gitSource;
								}
								
								if (record[env][name]["v" + version].options && opts.deployConfig) {
									record[env][name]["v" + version].options.deployConfig = opts.deployConfig;
								}
							}
							else {
								...
								if (opts.custom) {
									record[name][name].options.custom = opts.custom;
								}
								if (opts.gitSource) {
									record[name][name].options.gitSource = opts.gitSource;
								}
								if (opts.deployConfig) {
									record[name][name].options.deployConfig = opts.deployConfig;
								}
							}
			...
			options.params.data.action = soajs.inputmaskData.action;
			if (!options.params.data.action) {
				options.params.data.action = 'deploy';
			}
		...
		if (soajs.inputmaskData.action === 'redeploy') {
			utils.getEnvironment(soajs, BL.model, soajs.inputmaskData.env.toUpperCase(), function (error, envRecord) {
				checkIfError(soajs, cbMain, {
					...
					deployer.execute({'type': 'infra', 'driver': options.strategy, 'technology': options.strategy}, 'redeployService', options, (error) =>{
						checkIfError(soajs, cbMain, {config: config, error: error}, function () {
							return cbMain(null, true);
						});
					});
