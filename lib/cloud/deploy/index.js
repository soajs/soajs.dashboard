...
var BL = {
	...
	"deployService": function (config, soajs, registry, deployer, cbMain) {
		...
		function getEnvInfo(cb) {
			...
			//NOTE: keeping call to get environment record, some required values are not found in registry record (profile info)
			utils.getEnvironment(soajs, BL.model, context.env.toUpperCase(), function (error, envRecord) {
				checkIfError(soajs, cbMain, {
					...
					checkIfError(soajs, cbMain, {
						...
						options = utils.buildDeployerOptions(registry, soajs, BL);
						return cb();
					});
				});
			});
					...
					async.detect(newPorts, function(oneNewPort, callback) {
						...
						if(onePort.published === oneNewPort.published) {
							...
							if(soajs.inputmaskData.action === 'rebuild' && soajs.inputmaskData.custom && soajs.inputmaskData.custom.name) {
								if(soajs.inputmaskData.custom.name === deployedService.name || (deployedService.labels && deployedService.labels['soajs.service.label'] === soajs.inputmaskData.custom.name)) {
									conflict = false;
								}
							}
						}
		...
		function getDefaultUIExtKey(cb){
			...
			BL.model.findEntry(soajs, { "collection": "tenants", "conditions": tenantCondition }, function (error, tenantRecord) {
				checkIfError(soajs, cbMain, {
					config: config,
		...
		function doDeploy() {
			helpers.deployContainer(config, context, soajs, registry, deployer, BL, cbMain);
		}
	},
	...
	"redeployService": function (config, soajs, registry, deployer, cbMain) {
		...
		if (soajs.inputmaskData.action === 'redeploy') {
			...
			options.params = { id: soajs.inputmaskData.serviceId, mode: soajs.inputmaskData.mode, excludeTasks: true };
	 ...
	 *
	 */
	"deployPlugin": function (config, soajs, registry, deployer, cb) {
		var options = utils.buildDeployerOptions(registry, soajs, BL);
