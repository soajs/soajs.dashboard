				...
				async.auto({
					...
					getInfraRecord: ['inspectService', (result, callback) => {
						...
						if (soajs.inputmaskData.infraId) {
							...
							helpers.getInfraRecord(soajs, opts, config, BL, callback);
						}
						else {
							let opts = {
								collection: colls.infra,
					...
					BL.checkIfError(soajs, cbMain, {config: config, error: error, code: 600}, () => {
						async.auto({
							...
							if (error) {
								...
								utils.updateLedger(soajs, options.env.toLowerCase(), ledgerEntry, BL);
								BL.checkIfError(soajs, cbMain, {config: config, error: error}, cbMain);
							}
							else {
		...
		if (soajs.inputmaskData.action === 'redeploy') {
			...
			async.auto({
				...
				getInfraRecord: (fCb) => {
					...
					if (soajs.inputmaskData.infraId) {
						opts.conditions = {_id: req.soajs.inputmaskData.id};
						helpers.getInfraRecord(soajs, opts, config, BL, fCb);
					}
					else {
						let opts = {
							collection: colls.infra,
