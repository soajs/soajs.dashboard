...
var BL = {
	...
	"deployService": function (config, soajs, deployer, cbMain) {
		...
		function getGitInfo(cb) {
			...
			else if (soajs.inputmaskData && soajs.inputmaskData.custom && soajs.inputmaskData.sourceCode && Object.keys(soajs.inputmaskData.sourceCode).length > 0){
				let sourceCodeEntries = Object.keys(soajs.inputmaskData.sourceCode);
				async.eachSeries(sourceCodeEntries, (oneSourceCode, mCb) => {
					helpers.getGitRecord(soajs, soajs.inputmaskData.sourceCode[oneSourceCode].repo, BL, function (error, dbRecord) {
						soajs.log.info('get Git Info of: ', oneSourceCode);
						checkIfError(soajs, mCb, { config: config, error: error || !dbRecord, code: 600 }, function () {
							if (dbRecord.token) {
							...
							if(dbRecord.type === 'config'){
								...
								context.variables['$SOAJS_CONFIG_REPO_NAME'] = soajs.inputmaskData.sourceCode[oneSourceCode].repo.split("/")[1];
								...
								context.variables['$SOAJS_CONFIG_REPO_BRANCH'] = soajs.inputmaskData.sourceCode[oneSourceCode].branch;
								...
								if(soajs.inputmaskData.sourceCode[oneSourceCode].commit){
									context.variables['$SOAJS_CONFIG_REPO_COMMIT'] = soajs.inputmaskData.sourceCode[oneSourceCode].commit;
								...
								if(soajs.inputmaskData.sourceCode[oneSourceCode].path){
									context.variables['$SOAJS_CONFIG_REPO_PATH'] = soajs.inputmaskData.sourceCode[oneSourceCode].path;
							...
							if (dbRecord.type !== 'config' && context.catalog.recipe.type === 'server' ){
								mapping = {
								...
								context.variables['$SOAJS_GIT_REPO'] = soajs.inputmaskData.sourceCode[oneSourceCode].repo.split("/")[1];
								if(!context.catalog.recipe.buildOptions.env['SOAJS_GIT_REPO']) {
									context.catalog.recipe.buildOptions.env['SOAJS_GIT_REPO'] = {
										"type": "computed",
										"value": "$SOAJS_GIT_REPO"
								...
								context.variables['$SOAJS_GIT_BRANCH'] = soajs.inputmaskData.sourceCode[oneSourceCode].branch;
								if(!context.catalog.recipe.buildOptions.env['SOAJS_GIT_BRANCH']) {
									context.catalog.recipe.buildOptions.env['SOAJS_GIT_BRANCH'] = {
										"type": "computed",
										"value": "$SOAJS_GIT_BRANCH"
								...
								if(soajs.inputmaskData.sourceCode[oneSourceCode].commit){
									context.variables['$SOAJS_GIT_COMMIT'] = soajs.inputmaskData.sourceCode[oneSourceCode].commit;
									if(!context.catalog.recipe.buildOptions.env['SOAJS_GIT_COMMIT']) {
										context.catalog.recipe.buildOptions.env['SOAJS_GIT_COMMIT'] = {
											"type": "computed",
											"value": "$SOAJS_GIT_COMMIT"
								...
								if(soajs.inputmaskData.sourceCode[oneSourceCode].path){
									context.variables['$SOAJS_GIT_PATH'] = soajs.inputmaskData.sourceCode[oneSourceCode].path;
			...
			BL.model.findEntry(soajs, opts, function (error, dbRecord) {
				checkIfError(soajs, cbMain, {
					...
					if (!dbRecord && (context.catalog.type === 'soajs' || ((context.catalog.type === 'service' || context.catalog.type === 'daemon') && context.catalog.subtype === 'soajs'))) {
						return cbMain({ "code": 600, "msg": "This Repository does not contain a service or a daemon code that can be matched to the API catalog or the Daemon Catalog!" });
					}
								...
								checkIfError(soajs, cbMain, {
									...
									if (record.service.labels['soajs.service.type'] === 'service' || record.service.labels['soajs.service.type'] === 'daemon' || record.service.labels['soajs.service.type'] === 'other') {
										...
										async.each(record.service.env, function (oneEnv, callback) {
											...
											return BL.deployService(config, soajs, deployer, updateCiCdRecord.bind(this, soajs.inputmaskData, cbMain));
										});
									}
									else {
										...
										return BL.deployService(config, soajs, deployer, cbMain);
									}
									
									function updateCiCdRecord(opts, cbMain, err){
										if (err && err.code){
											return cbMain(err);
										}
										var cond = {
										...
										BL.model.findEntry(soajs, cond, function(error, record){
											checkIfError(soajs, cbMain, {config: config, error: error}, function () {
												if (record && record[opts.env] && record[opts.env][opts.custom.name] && record[opts.env][opts.custom.name]["v" + opts.custom.version]) {
													...
													BL.model.saveEntry(soajs, cond, cbMain);
												}
												else {
													return cbMain(null, true);
												}
