...
var BL = {
	...
	"deployService": function (config, req, deployer, cbMain) {
		...
		async.auto({
			...
			checkIfError(soajs, cbMain, {config: config, error: err, code: 600}, () => {
				...
				function registerCiCd(mCb){
					let cond = {
						collection: colls.cicd,
						conditions: {
							type: (options.params.inputmaskData.custom.resourceId) ? 'resource' : 'cd'
						}
					};
					BL.model.findEntry(soajs, cond, function (error, record) {
						checkIfError(soajs, mCb, {config: config, error: error, code: 600}, () => {
							...
							let version = options.params.inputmaskData.custom.version.toString();
							if (record && record[env] && record[env][name]) {
								...
								if (record[env][name]["v" + version]) {
									...
									if (options.params.inputmaskData.custom) {
										record[name][name].options.custom = options.params.inputmaskData.custom;
									}
									if (options.params.inputmaskData.gitSource) {
										record[name][name].options.gitSource = options.params.inputmaskData.gitSource;
									}
									if (options.params.inputmaskData.deployConfig) {
										record[name][name].options.deployConfig = options.params.inputmaskData.deployConfig;
									}
								}
								...
								BL.model.saveEntry(soajs, cond, mCb);
							}
							else {
					...
					checkIfError(soajs, cbMain, {config: config, error: error, code: 600}, () => {
						async.auto({
							...
							if (error) {
								...
								soajs.log.error(error);
								ledgerEntry.error = true;
								ledgerEntry.message = error.message || error.msg;
							}
							else {
								//generate success audit log message
								ledgerEntry.message = "Cloostro: " + cloostroAction + " Operation Completed.";
							}
							
							//insert ledger entry for audit log message
							let opts = {
								collection: colls.ledger,
	 ...
	 */
	"redeployService": function (config, req, deployer, cbMain) {
		let soajs = req.soajs;
		if (soajs.inputmaskData.action === 'redeploy') {
			...
			async.auto({
				...
				//wait until inspectService service and then get infra record
				getInfraRecord: function (fCb) {
					//if infra id provided, use it
					//else pull based on env code
					if (soajs.inputmaskData.infraId) {
						BL.model.validateCustomId(soajs, soajs.inputmaskData.infraId, (err) => {
							checkIfError(soajs, fCb, {config: config, error: err, code: 490}, () => {
								let opts = {
									collection: colls.infra,
									conditions: {
										_id: req.soajs.inputmaskData.id
								...
								BL.model.findEntry(soajs, opts, function (err, infraRecord) {
									checkIfError(soajs, fCb, {config: config, error: err, code: 600}, () => {
										checkIfError(soajs, fCb, {
											config: config,
											error: !infraRecord,
											code: 600
										}, () => {
											return fCb(null, infraRecord);
										});
									});
								});
							});
