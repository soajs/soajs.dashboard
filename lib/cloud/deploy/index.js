...
var BL = {
	...
	"deployService": function (config, req, soajs, deployer, cbMain) {
		...
		if (soajs.inputmaskData.deployConfig.type && soajs.inputmaskData.deployConfig.type === "vm") {
			checkIfError(soajs, cbMain, {
				config: config,
				error: !soajs.inputmaskData.deployConfig.infra && !soajs.inputmaskData.deployConfig.region && !soajs.inputmaskData.deployConfig.vmConfiguration,
				code: 490
			}, function () {
				utils.getEnvironment(soajs, BL.model, soajs.inputmaskData.env.toUpperCase(), function (error, envRecord) {
					checkIfError(soajs, cbMain, {
						...
						checkIfError(soajs, cbMain, {
							...
							soajs.inputmaskData.id = soajs.inputmaskData.deployConfig.infra;
									...
									BL.model.findEntry(soajs, opts, function (err, infraRecord) {
										checkIfError(soajs, cbMain, {
											...
											checkIfError(soajs, cbMain, {
												...
												options.params = {
													location: soajs.inputmaskData.deployConfig.region,
													instance: {
												...
												checkIfError(soajs, cbMain, {
													...
													if (soajs.inputmaskData.deployConfig.vmConfiguration.adminAccess.password){
														options.params.instance.admin.password = soajs.inputmaskData.deployConfig.vmConfiguration.adminAccess.password;
													}
													else {
														options.params.instance.admin.token = soajs.inputmaskData.deployConfig.vmConfiguration.adminAccess.token;
													}
													...
													if (soajs.inputmaskData.custom && soajs.inputmaskData.custom.resourceId) {
														options.params.labels["soajs.resource.id"] = soajs.inputmaskData.custom.resourceId;
													}
													...
													getCatalogRecipe(function (err) {
														checkIfError(soajs, cbMain, {
															...
															if (context.catalog.recipe && context.catalog.recipe.deployOptions && context.catalog.recipe.deployOptions.ports && Array.isArray(context.catalog.recipe.deployOptions.ports)) {
																options.params.instance.ports = context.catalog.recipe.deployOptions.ports;
															}
															...
															helpers.computeCatalogEnvVars(context, soajs, config, function (error, envVariables) {
																...
																soajs.log.debug(options.params);
																...
																BL.model.findEntry(soajs, {
																	...
																	checkIfError(soajs, cbMain, {
																		...
																		if (!record[soajs.inputmaskData.env.toUpperCase()]) {
																			record[soajs.inputmaskData.env.toUpperCase()] = {};
																		}
																		
																		if (!record[soajs.inputmaskData.env.toUpperCase()][soajs.inputmaskData.custom.name]) {
																			record[soajs.inputmaskData.env.toUpperCase()][soajs.inputmaskData.custom.name] = {};
																		}
																		record[soajs.inputmaskData.env.toUpperCase()][soajs.inputmaskData.custom.name].status = "pending";
																		opts = {
																			collection: colls.cicd,
																			record: record
																		};
																		BL.model.saveEntry(soajs, opts, function (error) {
																			checkIfError(soajs, cbMain, {
																				...
																				//call deploy vm and return callback
																				deployVM();
																	...
																	deployer.execute({
																		...
																		BL.model.findEntry(soajs, opts, function (error, record) {
																			if (error) {
																				...
																				//remove record if vm wasn't created
																				if (vmError) {
																					delete record[soajs.inputmaskData.env.toUpperCase()][soajs.inputmaskData.custom.name];
																				}
																				// add deploy
																				else {
																					record[soajs.inputmaskData.env.toUpperCase()][soajs.inputmaskData.custom.name].status = "ready";
																				}
																				opts = {
																					collection: colls.cicd,
																					record: record
																				...
																				BL.model.saveEntry(soajs, opts, function (error) {
																					if (error) {
																						...
																						soajs.log.info("CiCd record updated successfully!");
																						//inspectVM
																						options.params = {
																							vmName: soajs.inputmaskData.custom.name
																						};
																						updateIP();
																					}
																				});
																...
																function updateIP() {
																	soajs.log.info("Inspecting VM to get IP address!");
																	inspectVm(function (err, inspect) {
																		if (err) {
																			soajs.log.error(err);
																		}
																		else {
																			if (inspect && inspect.ip) {
																				updateResources(inspect.ip);
																			}
																			else {
																				setTimeout(function () {
																					updateIP();
																...
																function inspectVm(miniCb) {
																	deployer.execute({
																		type: "infra",
																		name: infraRecord.name,
																		technology: "vm"
																	}, 'inspectService', options, miniCb);
																}
																
																function updateResources(ip) {
																	var resourceOpts = {
