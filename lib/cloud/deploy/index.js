...
var BL = {
	...
	"deployService": function (config, soajs, deployer, cbMain) {
		...
		async.series([getEnvInfo, getCatalogRecipe, checkPort, checkClusterPorts, getLatestSOAJSImageInfo, getDefaultUIExtKey], function () {
			var platform = context.envRecord.deployer.selected.split('.')[1];
			...
			async.series([getDashboardConnection, getControllerDomain, getGitInfo, getServiceDaemonInfo], function () {
				...
				if (context.catalog.type === 'soajs' || (context.catalog.subtype === 'soajs' && (context.catalog.type === 'service' || context.catalog.type === 'daemon'))) {
					...
					//if ports are exposed and not nginx, use nodePort
					if(context.ports && context.platform === 'kubernetes'){
						context.options.deployerConfig.nginxDeployType = 'NodePort';
					}
					...
					if (context.catalog && context.catalog.recipe && context.catalog.recipe.buildOptions && context.catalog.recipe.buildOptions.env) {
						for (let i = 0, oneEnvKey = '', oneEnv = {}; i < Object.keys(context.catalog.recipe.buildOptions.env).length; i++) {
							...
							if (['computed', 'userInput'].indexOf(oneEnv.type) !== -1) {
								switch (oneEnv.value) {
									case '$SOAJS_GIT_REPO':
							...
							if (context.labels['service.repo'] && context.labels['service.branch'] && context.labels['service.commit']) {
								break;
							}
						}
					}
					...
					if (context.catalog && context.catalog.recipe && context.catalog.recipe.buildOptions && context.catalog.recipe.buildOptions.env) {
						for (let i = 0, oneEnvKey = '', oneEnv = {}; i < Object.keys(context.catalog.recipe.buildOptions.env).length; i++) {
							...
							if (oneEnvKey === 'SOAJS_GIT_REPO') {
								...
								var branchName;
								if (oneEnv.type === 'static') {
									branchName = oneEnv.value;
								}
								else if (oneEnv.type === 'userInput') {
									branchName = soajs.inputmaskData.custom.env['SOAJS_GIT_BRANCH'] || oneEnv.default;
								}
								context.labels['service.branch'] = branchName;
							}
						...
						deployer.inspectService(options, function (error, record) {
							checkIfError(soajs, cbMain, { config: config, error: error }, function () {
								checkIfError(soajs, cbMain, {
									...
									soajs.inputmaskData.custom.name = record.service.labels['soajs.service.label'];
									...
									if (record.service.labels['soajs.service.type'] === 'service' || record.service.labels['soajs.service.type'] === 'daemon') {
										...
										async.each(record.service.env, function (oneEnv, callback) {
											...
											return BL.deployService(config, soajs, deployer, updateCiCdRecord.bind(null, soajs.inputmaskData, cbMain));
										});
									}
									else {
									...
									function updateCiCdRecord(opts, cbMain){
										var cond = {
										...
										BL.model.findEntry(soajs, cond, function(error, record){
											checkIfError(soajs, cbMain, {config: config, error: error}, function () {
												if (record && record[opts.env] && record[opts.env][opts.custom.name]
													&& record[opts.env][opts.custom.name]["v" + opts.custom.version]) {
