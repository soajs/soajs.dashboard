...
var helpers = {
	...
	checkPort: function (context, config, soajs, cbMain, cb) {
		...
		async.each(ports, function (onePort, callback) {
			...
			if(soajs.inputmask.custom && soajs.inputmask.custom.ports && soajs.inputmask.custom.ports.length > 0){
				soajs.inputmask.custom.ports.forEach((oneInputPort) => {
