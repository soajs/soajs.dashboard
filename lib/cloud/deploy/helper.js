...
var helpers = {
	...
	checkPort: function (context, config, soajs, cbMain, cb) {
		...
		async.each(ports, function (onePort, callback) {
			 ...
			 */
			var temp;
			if (!Object.hasOwnProperty.call(onePort, "isPublished")) {
				return callback({invalidPorts: true});
			}
			else if (Object.hasOwnProperty.call(onePort, "isPublished") && onePort.isPublished === false) {
				temp = "cluster";
			}
			else if (Object.hasOwnProperty.call(onePort, "isPublished") && onePort.isPublished === true) {
			...
			if ((Object.hasOwnProperty.call(onePort, "isPublished") && !onePort.isPublished && onePort.published )){
			...
			if (error) {
				if (error.wrongPort){
					...
					soajs.log.error({"code": 824, "msg": config.errors[824]});
					return cbMain({"code": 824, "msg": errMsg});
				}
				if (error.invalidPorts) {
					soajs.log.error({"code": 826, "msg": config.errors[826]});
					return cbMain({"code": 826, "msg": config.errors[826]});
				}
			}
			else {
	...
	deployContainer: function (config, context, soajs, deployer, BL, cbMain) {
		...
		function updateEnvSettings(cb) {
			if(context.catalog.type ==='nginx' || (context.catalog.subtype ==='nginx' && context.catalog.type ==='server')) {
				...
				if(process.env.SOAJS_SAAS && context.envRecord.deployer && context.envRecord.deployer.selected === 'container.kubernetes.remote'){
					if(context.envRecord.deployer.container && context.envRecord.deployer.container.kubernetes && context.envRecord.deployer.container.kubernetes.remote && context.envRecord.deployer.container.kubernetes.remote.nginxDeployType === 'LoadBalancer'){
						port = 80;
					}
				}
	...
	checkVolumes: function (context, config, soajs, cbMain, cb) {
		if (!context.catalog.recipe.deployOptions
			&& (context.catalog.recipe.deployOptions && !context.catalog.recipe.deployOptions.voluming)) {
			return cb();
		}
		var check;
		var deployType = context.envRecord.deployer.selected.split('.')[1];
		...
		if (check.errors && check.errors.length > 0) {
			soajs.log.error({"code": 827, "msg": config.errors[827]});
			return cbMain({"code": 827, "msg": config.errors[827]});
		}
		else {
			return cb();
		}
