	...
	getAnalyticsEsInfo: function (soajs, context, model, cb) {
		...
		utils.getEnvironment(soajs, model, env, function (error, envRecord) {
			...
			async.detect(envRecord.dbs.databases, function (oneDb, callback) {
				...
				//old cluster style
				if(envRecord.clusters){
					var analyticsCluster = envRecord.dbs.clusters[analyticsDb.cluster];
					buildContext(analyticsCluster);
				}
				else{
					helpers.getClusterResource(soajs, {"model": model}, analyticsDb.cluster, function(error, resource) {
						if (error) {
							return cb(error);
						}
						if (!resource) {
						...
						var analyticsCluster = resource.config;
						buildContext(analyticsCluster);
					});
				}
				
				
				function buildContext(analyticsCluster){
	...
	getDashDbInfo: function (soajs, BL, cb) {
		utils.getEnvironment(soajs, BL.model, 'DASHBOARD', function (error, envRecord) {
			...
			if(envRecord.dbs.clusters){
				...
				helpers.getClusterResource(soajs, BL, "dash_cluster", function(error, resource){
					if(error){
						return cb(error);
					}
					data = {
						mongoDbs: resource.config.servers,
						mongoCred: resource.config.credentials,
						clusterInfo: resource.config,
						dbConfig: envRecord.dbs.config
					};
					return cb(null, data);
				});
