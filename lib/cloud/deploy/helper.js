...
let helpers = {
				...
				helpers.getGitRecord(soajs, soajs.inputmaskData.custom.sourceCode[oneSourceCode].repo, BL, (error, dbRecord) => {
					if (error) {
						...
						if (dbRecord.repos[0].type === 'config') {
							...
							helpers.handleOneRepoLevel(soajs, options, dbRecord, oneSourceCode);
						}
						return mCb();
					}
				});
	...
	makePortsUnique: function (options, cb) {
		let portTargets = [];
		console.log(options.params.inputmaskData);
		if (options.params.inputmaskData.custom && options.params.inputmaskData.custom.ports) {
			...
			options.params.inputmaskData.custom.ports = portTargets;
			console.log(options.params.inputmaskData.custom.ports);
		}
		return cb();
	}
};
