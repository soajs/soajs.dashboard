...
let helpers = {
	...
	computeInspectService: function (soajs, record, cb) {
		...
		if(soajs.inputmaskData.deployConfig.memoryLimit !== 0){
	...
	computeReadinessProbe: function (soajs, options, result, cb) {
		...
		if (result.getCatalogRecipe && result.getCatalogRecipe.recipe.deployOptions
			&& result.getCatalogRecipe.recipe.deployOptions.readinessProbe
			&& result.getCatalogRecipe.recipe.deployOptions.readinessProbe.httpGet
			&& result.getCatalogRecipe.recipe.deployOptions.readinessProbe.httpGet.port) {
			if (result.getCatalogRecipe.recipe.deployOptions.readinessProbe.port === "maintenance") {
				result.getCatalogRecipe.recipe.deployOptions.readinessProbe.port = options.params.data.variables['$SOAJS_SRV_PORT'] + result.getEnvInfo.services.config.ports.maintenanceInc;
			}
			if (result.getCatalogRecipe.recipe.deployOptions.readinessProbe.port === "inherit") {
				result.getCatalogRecipe.recipe.deployOptions.readinessProbe.port = options.params.data.variables['$SOAJS_SRV_PORT'];
			}
		}
		return cb();
	},
