		...
		async.each(ports, function (onePort, callback) {
			...
			if(soajs.inputmaskData.custom && soajs.inputmaskData.custom.ports && soajs.inputmaskData.custom.ports.length > 0){
				soajs.inputmaskData.custom.ports.forEach((oneInputPort) => {
					if(onePort.isPublished && oneInputPort.name === onePort.name && oneInputPort.port){
						onePort.published = oneInputPort.port;
					}
				});
			}
			...
			if(process.env.SOAJS_SAAS && req.soajs.inputmaskData.soajs_project){
				let options = {
					...
					"qs": {
						...
						"mode": deployedServiceDetails.labels['soajs.service.mode'],
						"serviceId": deployedServiceDetails.id,
						"env": context.envRecord.code.toLowerCase(),
						"name": deployedServiceDetails.labels['soajs.service.name']
					},
					"json": true
				};
				...
				if (deployedServiceDetails.namespace) {
					options.qs.namespace = deployedServiceDetails.namespace;
				}
