...
var helpers = {
	...
	deployContainer: function (config, context, req, soajs, deployer, BL, cbMain) {
		...
		function checkWithInfra(deployedServiceDetails, cb){
			if(process.env.SOAJS_SAAS && req.soajs.inputmaskData.soajs_project){
				...
				let options = {
					...
					'form':{
						'deployOptions':{
							'serviceId': deployedServiceDetails.service.id,
							'custom': {
								'type': deployedServiceDetails.service.labels['soajs.service.subtype'],
								'name': deployedServiceDetails.service.labels['soajs.service.name'],
				...
				req.soajs.log.debug(`Invoking bridge to update firewall rules...`);
	...
	checkVolumes: function (context, config, soajs, cbMain, cb) {
		if (!context.catalog.recipe.deployOptions || !context.catalog.recipe.deployOptions.voluming || context.catalog.recipe.deployOptions.voluming.length === 0) {
			return cb();
		}
	...
	"getSecrets": function(context, soajs, deployer, BL, cbMain){
		...
		deployer.execute({'type': 'container', 'driver': options.strategy}, 'listSecrets', options, cbMain);
	}
};
