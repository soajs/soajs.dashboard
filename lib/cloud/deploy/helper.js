...
var helpers = {
	...
	checkVolumes: function (context, config, soajs, cbMain, cb) {
		if (!context.catalog.recipe.volume) {
			return cb();
		}
		var check;
		var deployType = context.envRecord.deployer.selected.split('.')[1];
		...
		if (deployType === 'kubernetes') {
			check = myValidator.validate(context.catalog.recipe.volume, volumeSchema.kubernetes);
		}
		else {
			check = myValidator.validate(context.catalog.recipe.volume, volumeSchema.docker);
		}
