	...
	"listVMs": function (config, soajs, deployer, cbMain) {
		...
		async.auto({
			"getEnvironment": (mCb) =>{
				...
				utils.getEnvironment(soajs, BL.model, soajs.inputmaskData.env.toUpperCase(), (error, envRecord) => {
					checkIfError(soajs, mCb, { config: config, error: error || !envRecord, code: 600 }, () => {
						soajs.inputmaskData.infraId = Object.keys(envRecord.restriction);
			...
			"callDeployer": ["getEnvironment", "getInfras", (info, mCb) => {
				let region = Object.keys(info.getEnvironment.restriction[soajs.inputmaskData.infraId.toString()])[0];
				let network = info.getEnvironment.restriction[soajs.inputmaskData.infraId.toString()][region].network;
				...
				if(Object.keys(info.getEnvironment.restriction[soajs.inputmaskData.infraId.toString()][region]).length > 1){
			...
			checkIfError(soajs, cbMain, {config: config, error: error}, () => {
				...
				async.each(result.callDeployer, function(vm, mCB) {
					...
					let data = {[result.getInfras.name]: result.callDeployer};
