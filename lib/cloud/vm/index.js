			...
			"callDeployer": ["getEnvironment", "getInfras", (info, mCb) => {
				...
				deployer.execute({ type: "infra", name: info.getInfras.name, technology: "vm" }, 'listServices', options, (error, vms) => {
					checkIfError(soajs, mCb, {config: config, error: error}, () => {
						async.filter(vms, (oneVM, vCb) => {
							if(!oneVM.labels || !soajs.inputmaskData.env){
								return vCb(null, true);
							}
							else if(oneVM.labels['soajs.env.code'] && soajs.inputmaskData.env && oneVM.labels['soajs.env.code'].toLowerCase() === soajs.inputmaskData.env.toLowerCase()){
								return vCb(null, true);
							}
							return vCb(null, false);
						}, (error, results) => {
							return mCb(null, results);
						})
					});
				});
			}]
