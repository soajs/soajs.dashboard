...
var BL = {
	 ...
	 * @param owner, repo, filepath, branch
	 * @param req
	 * @param res
	 */
	'getYaml': function (config, req, res) {
		data.getAccount(req.soajs, BL.model, {owner: req.soajs.inputmaskData.owner, repo: req.soajs.inputmaskData.repo}, function (error, account) {
			checkIfError(req, res, {config: config, error: error || !account, code: 757}, function () {
				var options = {
					...
					ref: req.soajs.inputmaskData.branch,
					token: account.token,
