...
module.exports = {
    ...
    'login': function (mongo, config, req, res) {
        ...
        git.login(mongo, record, function (error, result) {
    ...
    'logout': function (mongo, config, req, res) {
        validateId(mongo, req, function (error) {
            checkIfError(req, res, {config: config, error: error, code: 701}, function () {
                ...
                git.logout(mongo, options, function (error, result) {
    ...
    'getRepos': function (mongo, config, req, res) {
        validateId(mongo, req, function (error) {
            checkIfError(req, res, {config: config, error: error, code: 701}, function () {
                ...
                git.getRepos(mongo, options, function (error, repos) {
    ...
    'getBranches': function (mongo, config, req, res) {
        if (req.soajs.inputmaskData.type === 'repo') {
            validateId(mongo, req, function (error) {
                checkIfError(req, res, {config: config, error: error, code: 701}, function () {
                    ...
                    checkIfError(req, res, {config: config, error: !req.soajs.inputmaskData.provider, code: 775}, function () {
                        ...
                        git.getBranches(mongo, options, function (error, result) {
            ...
            getContentInfo(function (error, info) {
                ...
                var repoLabel = info.src.owner + '/' + info.src.repo;
                //we have to make a call to git_accounts in this case in order to search for account provider of specified service/daemon/static
    ...
    'activateRepo': function (mongo, config, req, res) {
        ...
        function getConfigFile(token, path, flags, cb) {
            ...
            git.getContent(gitConfigObj, function (error, repoConfig, configSHA) {
                if (error) {
                    return cb(error);
                }
    ...
    'deactivateRepo': function (mongo, config, req, res) {
        validateId(mongo, req, function (error) {
            checkIfError(req, res, {config: config, error: error, code: 701}, function () {
                ...
                mongo.findOne(collName, {_id: req.soajs.inputmaskData.id, 'repos.name': repoLabel}, {'repos.$': 1}, function (error, accountRecord) {
                    checkIfError(req, res, {config: config, error: error, code: 765}, function () {
                        remove(accountRecord.repos[0].type, function (error, result) {
                            checkIfError(req, res, {
                                config: config,
                                error: error,
