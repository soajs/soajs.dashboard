...
function checkIfError(req, res, data, cb) {
    if (data.error) {
        ...
        if(data.error.message && data.error.message.indexOf('getaddrinfo EAI_AGAIN ') != -1) {
            data.code = 763;
...
module.exports = {
    ...
    'login': function (mongo, config, req, res) {
        ...
        req.soajs.inputmaskData.domain = req.soajs.inputmaskData.domain.replace(/http[s]{0,1}:\/\//i, '');

        if (req.soajs.inputmaskData.domain.lastIndexOf('/') === req.soajs.inputmaskData.domain.length - 1) {
            req.soajs.inputmaskData.domain = req.soajs.inputmaskData.domain.substring(0, req.soajs.inputmaskData.domain.length - 1);
        }
