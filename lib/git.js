...
var configGenerator = require('./helpers/configGenerator.js');
...
function doGetFile (req, git, gitModel, account, branch, cbMain) {
	var options = {
...
var BL = {
	...
	'activateRepo': function (config, req, git, helpers, gitModel, cbMain) {
		...
		function getConfigFile (token, path, flags, cb) {
			...
			git.getJSONContent(req.soajs, gitModel, BL.model, gitConfigObj, function (error, repoConfig, configSHA) {
				if (error) {
					...
					if (error.code === 761) {
						return getCustomRepoFiles({ gitConfig: gitConfigObj, path: path, flags: flags }, cb);
					}
					else {
						return cb(error);
					}
		...
		function getCustomRepoFiles(options, cb) {
			// fetch soa.js config file
			options.gitConfig.path = config.gitAccounts[options.gitConfig.provider].customConfigFilesPath.soajsFile;
			git.getJSONContent(req.soajs, gitModel, BL.model, options.gitConfig, function (error, repoConfig, configSHA) {
				if (error) {
					if (error.code === 761) {
						//soa.js file not found, fallback to activation of repo as type 'other'
						return activateMiscRepo(options, cb);
					}
					else {
						return cb(error);
					}
				...
				options.gitConfig.path = config.gitAccounts[options.gitConfig.provider].customConfigFilesPath.swaggerFile;
				git.getAnyContent(req.soajs, gitModel, BL.model, options.gitConfig, function (error, fileData) {
					if (error) {
						if (error.code === 761) {
							//swagger file not found, fallback to activation of repo as type 'other'
							return activateMiscRepo(options, cb);
						}
						else {
							return cb(error);
					...
					//write swagger file
					var swaggerFilePath = path.join(config.gitAccounts[options.gitConfig.provider].repoConfigsFolder, config.gitAccounts[options.gitConfig.provider].customConfigFilesPath.swaggerFile);
					var soajsFilePath = path.join(config.gitAccounts[options.gitConfig.provider].repoConfigsFolder, config.gitAccounts[options.gitConfig.provider].customConfigFilesPath.soajsFile);
					var configFilePath = path.join(config.gitAccounts[options.gitConfig.provider].repoConfigsFolder, config.gitAccounts[options.gitConfig.provider].defaultConfigFilePath);
					fs.writeFile(swaggerFilePath, fileData.content, function (error) {
						...
						configGenerator.generate(soajsFilePath, swaggerFilePath, function (error, config) {
							if (error) {
								req.soajs.log.error(error);
							...
							fs.writeFile(configFilePath, config, function (error) {
								...
								try {
									options.repoConfig = require(configFilePath);
								}
								catch(e) {
									return cb(e);
								}

								return analyzeConfigFile(options.repoConfig, configFilePath, options.gitConfig.token, options.flags, cb);
							});
						});
					});
				});
		...
		function addNewStaticContent (type, staticContentInfo, cb) {
			helpers.addNewStaticContent(req.soajs, BL.model, configSHA, type, staticContentInfo, function (error, added) {
				checkMongoError(req, error, cb, function () {
					return cb(null, added);
				});
			});
	...
	'deactivateRepo': function (config, req, git, helpers, gitModel, cbMain) {
		validateId(req.soajs, function (error) {
			checkReturnError(req, cbMain, { config: config, error: error, code: 701 }, function () {
		...
		gitModel.getAccount(req.soajs, BL.model, {
			...
			checkReturnError(req, cbMain, { config: config, error: error || !account, code: 757 }, function () {
				if (process.env.SOAJS_DEPLOY_HA) {
					...
					doGetFile(req, git, gitModel, account, "master", cbMain);
				}
			});
		});
	},
	...
	'getHaFile': function (account, config, req, git, deployer, helpers, gitModel, cbMain) {
		...
		BL.model.findEntry(req.soajs, opts, function (error, oneEnvironment) {
			checkReturnError(req, cbMain, {
				config: config,
				error: error || !account,
				code: 600
					...
					deployer.findService(options, function (error, oneService) {
						checkReturnError(req, cbMain, { config: config, error: error }, function () {
							checkReturnError(req, cbMain, {
								...
								doGetFile(req, git, gitModel, account, branch, cbMain);
							});
						});
					});
				});
