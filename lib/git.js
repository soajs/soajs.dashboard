...
var helpers = {
	...
	buildDeployerOptions: function (envRecord, soajs) {
...
var BL = {
	...
	'login': function (config, req, res, cbMain) {
		var record = {
	...
	'logout': function (config, req, res, cbMain) {
	...
	'listAccounts': function (config, req, res, cbMain) {
	...
	'getRepos': function (config, req, res, cbMain) {
		validateId(req.soajs, function (error) {
			checkReturnError(req, cbMain, {config: config, error: error, code: 701}, function () {
				var options = {
					provider: req.soajs.inputmaskData.provider,
	...
	'getBranches': function (config, req, res, cbMain) {
	...
	'activateRepo': function (config, req, res, cbMain) {
		var configSHA = [];
		validateId(req.soajs, function (error) {
			checkReturnError(req, cbMain, {config: config, error: error, code: 701}, function () {
		...
		function analyzeConfigFile(repoConfig, path, token, flags, cb) {
			...
			helpers.validateFileContents(req, res, repoConfig, function (error) {
				if (error) {
					return cb(error);
				}
				var info = {};
	...
	'deactivateRepo': function (config, req, res, cbMain) {
		validateId(req.soajs, function (error) {
			checkReturnError(req, cbMain, {config: config, error: error, code: 701}, function () {
	...
	'syncRepo': function (config, req, res, cbMain) {
		validateId(req.soajs, function (error) {
			checkReturnError(req, cbMain, {config: config, error: error, code: 701}, function () {
	 ...
	 * @param res
	 */
	'getFile': function (config, req, res, cbMain) {
		data.getAccount(req.soajs, BL.model, {
			owner: req.soajs.inputmaskData.owner,
			repo: req.soajs.inputmaskData.repo
				...
				if (process.env.SOAJS_DEPLOY_HA) {
					BL.model.findEntry(req.soajs, {
						...
						checkReturnError(req, cbMain, {
							...
							var options = helpers.buildDeployerOptions(oneEnvironment, req.soajs);
							checkReturnError(req, cbMain, {
								config: config,
								error: !options,
								code: 825
