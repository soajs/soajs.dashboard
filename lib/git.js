...
function getCustomRepoFiles(options, req, cb) {
	...
	options.git.getJSONContent(req.soajs, options.gitModel, BL.model, options.gitConfig, function (error, repoConfig, configSHA) {
		if (error) {
			error.reason = 'soa.js';
			req.soajs.log.error(error);
			return cb(error);
		...
		options.git.getAnyContent(req.soajs, options.gitModel, BL.model, options.gitConfig, function (error, fileData) {
			if (error) {
	...
	'activateRepo': function (config, req, git, helpers, gitModel, cbMain) {
			...
			git.getJSONContent(req.soajs, gitModel, BL.model, gitConfigObj, function (error, repoConfig, configSHA) {
				if (error) {
					...
					if (error.code === 761) {
						...
						return getCustomRepoFiles(options, req, function (error, configData) {
							if (error) {
		...
		function activateMiscRepo(options, cb) {
			...
			BL.model.insertEntry(req.soajs, opts, function (error) {
				var newRepo = { name: options.gitConfig.user + '/' + options.gitConfig.repo, type: 'custom' };
				gitModel.addRepoToAccount(req.soajs, BL.model, {accountId: req.soajs.inputmaskData.id, repo: newRepo}, function (error) {
	...
	'deactivateRepo': function (config, req, git, helpers, gitModel, cbMain) {
		...
		function remove (type, cb) {
			switch (type) {
				...
				default:
				req.soajs.log.error("Invalid type detected, must be [service || daemon || static || multi || custom]");
		...
		function removeCustom(cb) {
			var opts = {
				collection: reposColl,
				conditions: { accountId: req.soajs.inputmaskData.id, 'owner': req.soajs.inputmaskData.owner, 'repo': req.soajs.inputmaskData.repo }
			};
			BL.model.removeEntry(req.soajs, opts, function (error) {
				checkMongoError(req, error, cb, function () {
					var repoLabel = req.soajs.inputmaskData.owner + '/' + req.soajs.inputmaskData.repo;
					gitModel.removeRepoFromAccount(req.soajs, BL.model, { accountId: req.soajs.inputmaskData.id, repoLabel: repoLabel }, function (error) {
				...
				if (error) {
					...
					if (error.code === 761) {
						...
						return getCustomRepoFiles(options, req, function (error, configData) {
							...
							return analyzeConfigSyncFile(configData.content, configData.path, token, {}, options.flags, cb);
						});
					}
					else {
						return cb(error);
