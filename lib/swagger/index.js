		...
		function generateModule(cb) {
			...
			function writeFiles(mCb) {
				var files = [
					...
					{file: tempDir + "/helper.js", data: fs.readFileSync(tmplDir + "index.txt", "utf8")},
					{file: tempDir + "/cd.js", data: fs.readFileSync(ciDir + "cd.js", "utf8")},
				...
				async.each(files, function (fileObj, mCb) {
					...
					checkReturnError(req, cbMain, {config: config, error: error, code: 854}, function () {
						downloadAndAddCiRecipe(function(){
							return mCb(null, true);
						});
					});
				});
			...
			function downloadAndAddCiRecipe(cb){
				var opts = { collection: 'ci' };
				BL.model.findEntry(req.soajs, opts, function (error, record) {
					checkReturnError(req, cb, {config: config, error: error, code: 600}, function () {
						if(!record || !record.driver || !record.recipe){
							return cb();
						}
						
						var driver = record.driver, recipe = record.recipe;
						fs.writeFile(tempDir + "/" + driver + ".yml", recipe, "utf8", cb);
					});
				});
			}
