    ...
    function constructDeployerParams(cb) {
        var serviceParams = {
            ...
            "labels": { //very useful when filtering
                ...
                "soajs.service.group": ((context.dbRecord.group) ? context.dbRecord.group.replace(' ', '-') : ''),
    ...
    "deployNginx": function (config, soajs, res) {
        ...
        function constructDeployerParams() {
            var nginxParams = {
                ...
                "network": config.network,
                "ports": [ //NOTE: an https port is automatically exposed with a random port
                    {
                        "isPublished": true,
                        "target": 443
                    }
            ...
            if (soajs.inputmaskData.deployConfig.ports) {
                //NOTE: only one http exposed port is permitted for now for nginx deployment
                nginxParams.ports.push({
                    "isPublished": true,
                    "target": 80,
                    "published": soajs.inputmaskData.deployConfig.ports[0].published
                });
            }
        ...
        getEnvInfo(function () {
            ...
            checkPort(function() {
                console.log (soajs.inputmaskData.deployConfig.exposedPort);
