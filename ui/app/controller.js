...
/* App Module */
var soajsApp = angular.module('soajsApp', ['ui.bootstrap', 'ui.bootstrap.datetimepicker', 'luegg.directives', 'angular-sortable-view', 'ngRoute', 'ngCookies', 'ngStorage', 'textAngular', "ngFileUpload", "ng.jsoneditor"]);
		...
		function doEnvPerNav(cb) {
			...
			for (var i = 0; i < $scope.navigation.length; i++) {
				if ($scope.currentDeployerType) {
					if ($scope.currentDeployerType === 'manual' && $scope.navigation[i].id === 'environments-hacloud' ||
						$scope.currentDeployerType !== 'manual' && $scope.navigation[i].id === 'environments-hosts') {
