...
contentManagementApp.controller("ContentManagementModuleStgCtrl", ['$scope', 'ngDataApi', '$compile', '$timeout', '$modal', 'injectFiles', 'cmModuleStgService', function ($scope, ngDataApi, $compile, $timeout, $modal, injectFiles, cmModuleStgService) {
			...
			actions: [
				{
					...
					'action': function (formData) {
						...
						var files = cmModuleStgService.extractFilesFromPostedData($scope, config, formData);
						if (files === false) {
							...
							getSendDataFromServer($scope, ngDataApi, {
								...
								if (error) {
									...
									if (typeof(files) === 'object' && Object.keys(files).length > 0) {
										cmModuleStgService.UploadFile($scope, config, 'add', files, response, '/' + $scope.selectedService.name + "/upload", function (error) {
											if (error) {
								...
								getSendDataFromServer($scope, ngDataApi, {
									...
									if (error) {
										...
										if (hasContentToUpload) {
											cmModuleStgService.UploadFile($scope, config, 'edit', files, [response], '/' + $scope.selectedService.name + "/upload", function (error) {
												if (error) {
													$scope.form.displayAlert('danger', error);
												}
												else {
	...
	$scope.downloadFile = function (oneEntry, mediaType) {
		cmModuleStgService.downloadFile($scope, oneEntry, mediaType);
	};
	
	cmModuleStgService.loadServices($scope);
	injectFiles.injectCss("modules/stg/contentManagement/contentManagement.css");
}]);