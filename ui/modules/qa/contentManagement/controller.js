...
contentManagementApp.controller("ContentManagementModuleQaCtrl", ['$scope', 'ngDataApi', '$compile', '$timeout', '$modal', 'injectFiles', 'cmModuleQaService', function ($scope, ngDataApi, $compile, $timeout, $modal, injectFiles, cmModuleQaService) {
			...
			actions: [
				{
					...
					'action': function (formData) {
						...
						var files = cmModuleQaService.extractFilesFromPostedData($scope, config, formData);
						if (files === false) {
								...
								getSendDataFromServer($scope, ngDataApi, {
									...
									if (error) {
										...
										if (hasContentToUpload) {
											cmModuleQaService.UploadFile($scope, config, 'edit', files, [response], '/' + $scope.selectedService.name + "/upload", function (error) {
												if (error) {
													$scope.form.displayAlert('danger', error);
												}
												else {
	...
	$scope.downloadFile = function (oneEntry, mediaType) {
		cmModuleQaService.downloadFile($scope, oneEntry, mediaType);
	};
	
	cmModuleQaService.loadServices($scope);
	injectFiles.injectCss("modules/qa/contentManagement/contentManagement.css");
}]);